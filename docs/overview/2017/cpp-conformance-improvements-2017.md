...
Visual Studio 2017 バージョン 15.9 では、このコードでは次のエラーが発生します: *エラー C3615: constexpr 関数 'S<int>::f' の結果が定数式になることはありません。注: エラーの原因は未定義関数の呼び出しまたは 'constexpr' が宣言されていない呼び出しです。注: 'g' の使い方を参照してください*。 このエラーを回避するには、関数 f() の明示的なインスタンス化から `constexpr` 修飾子を削除します。 
