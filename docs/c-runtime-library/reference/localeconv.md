...
指定したメンバーのない限り、 **lconv**構造を持つ`char *`と`wchar_t *`バージョンは、文字列へのポインター。 これらに相当する **""** (または**L""** の**wchar_t \*** ) 長さが 0 のいずれかまたは現在のロケールでサポートされていません。 なお**decimal_point**と **_W_decimal_point**は常にサポートされており、長さが 0 以外。
