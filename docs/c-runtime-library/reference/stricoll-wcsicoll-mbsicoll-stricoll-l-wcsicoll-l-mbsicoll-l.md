---
description: 詳細については、_stricoll、_wcsicoll、_mbsicoll、_stricoll_l、_wcsicoll_l、_mbsicoll_l を参照してください。
...
これらの各関数は **_NLSCMPERROR** を返します。 **_NLSCMPERROR** を使用するに \<string.h> は、またはを含め \<mbstring.h> ます。 *string1* または *string2* に照合シーケンスのドメイン外のワイド文字コードが含まれている場合、 **_wcsicoll** は失敗する可能性があります。 エラーが発生した場合、 **_wcsicoll** は **errno** を **EINVAL** に設定することがあります。 **_Wcsicoll** の呼び出し時にエラーが発生していないかどうかを確認するには、 **errno** を0に設定し、 **_wcsicoll** を呼び出した後に **errno** をチェックします。
