...
**freopen_s**は、通常、事前に開かれたファイル ( **stdin**、 **stdout**、 **stderr** ) をユーザーが指定したファイルにリダイレクトするために使用されます。 *ストリーム*に関連付けられている新しいファイルは、次のように、ファイルに対して要求されるアクセスの種類を指定する文字列である*モード*で開かれます。
