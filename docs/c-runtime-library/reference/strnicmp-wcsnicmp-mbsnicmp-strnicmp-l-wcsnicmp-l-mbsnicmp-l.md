...
パラメーターの検証エラーが発生した場合 **_NLSCMPERROR**、これらの関数は _NLSCMPERROR \<を返します。これ\<は、.h> と mbstring.h> で定義されています。
