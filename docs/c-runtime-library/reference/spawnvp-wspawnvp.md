...
新しいプロセスを作成および実行し、使用してコマンドライン引数へポインターの配列を渡しますをこれらの各関数、**パス**環境変数を実行するファイルを検索します。
