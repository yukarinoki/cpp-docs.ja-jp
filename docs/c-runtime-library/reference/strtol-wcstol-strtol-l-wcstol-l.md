...
**strtol**、 **wcstol**、 **_strtol_l**、 **_wcstol_l**は、*文字列*で表される値を返します。 変換できない場合は0を返します。 この表現でオーバーフローが発生すると、 **LONG_MAX**または**LONG_MIN**が返されます。
...
**wcstol**と **_wcstol_l**は、 **strtol**と **_strtol_l**のワイド文字バージョンです。 *文字列*引数はワイド文字列です。 これらの関数は**strtol**と同じように動作し、それ以外の場合は **_strtol_l**します。 ロケールの**LC_NUMERIC**カテゴリの設定によって、*文字列*の小数点文字 (小数のマーカーまたは小数点) の認識が決まります。 関数**strtol**と**wcstol**は、現在のロケールを使用します。 **_strtol_l**と **_wcstol_l**渡されたロケールを代わりに使用します。 詳細については、「[setlocale]」および「 [Locale](../../c-runtime-library/locale.md)」を参照してください。
...
関数**_strtol_l** と**_wcstol_l** 関数は、標準 C ライブラリの一部ではなく、Microsoft 固有のものです。 互換性の詳細については、「[互換性](../compatibility.md)」を参照してください。

## <a name="example"></a>例

の[strtod](strtod-strtod-l-wcstod-wcstod-l.md)例を参照してください。
