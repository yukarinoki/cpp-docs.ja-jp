---
description: '詳細については、次を参照してください: _recalloc'
title: _recalloc
ms.date: 4/2/2020
api_name:
- _recalloc
...
**_Recalloc** 関数は、割り当てられたメモリブロックのサイズを変更します。 *Memblock* 引数は、メモリブロックの先頭を指します。 *Memblock* が **NULL** の場合、 **_recalloc** は [calloc](calloc.md)と同じように動作し、新しいサイズのバイト *数のブロック* を割り当て  *  ます。 各要素は 0 で初期化されます。 *Memblock* が **NULL** でない場合は、 **calloc**、 [malloc](malloc.md)、または [realloc](realloc.md)の前の呼び出しによって返されたポインターである必要があります。
