---
description: '詳細については、次を参照してください: _sopen、_wsopen'
...
整数式 *oflag* は、で定義されている次のマニフェスト定数の1つ以上を組み合わせることによって形成され \<fcntl.h> ます。 2つ以上の定数が引数 *oflag* を形成する場合、それらはビットごとの or 演算子 ( **&#124;** ) と組み合わされます。
 ...
 **_O_WRONLY**  |  **_O_APPEND** (追加モード) と **_O_WTEXT**、 **_O_U16TEXT**、または **_O_U8TEXT** を使用して _sopen が呼び出された場合、まず読み取りと書き込みのためにファイルを開き、BOM を読み取り、書き込み専用で再度開くように試みます。 読み取りおよび書き込み用にファイルを開くのに失敗した場合、書き込み専用でファイルを開き、Unicode モード設定の既定値を使用します。
...
*Pmode* 引数は **_O_CREAT** が指定されている場合にのみ必要です。 ファイルが存在しない場合は、ファイルのアクセス許可の設定 *を指定し* ます。これは、新しいファイルが最初に閉じられたときに設定されます。 それ以外の場合、 *pmode* は無視されます。 *pmode* は、で定義されているマニフェスト定数 **_S_IWRITE** と **_S_IREAD** の一方または両方を含む整数式です \<sys\stat.h> 。 両方の定数が指定されると、これらはビットごとの OR 演算子を使用して組み合わされます。 *Pmode* の意味は次のとおりです。
