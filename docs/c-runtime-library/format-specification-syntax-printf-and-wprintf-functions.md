...
@No__t_0、`int`、`long`、`long long`、およびその `unsigned` バリアントなどの整数型は、 **d**、 **i**、 **o**、 **u**、 **x**、および**x**を使用して指定します。@No__t_11、2、3 などの浮動小数点型は、、 **a**、 **e**、 **e**、 **f**、 **f**、 **g**、および**g** **を使用し**て指定します。既定では、*サイズ*プレフィックスによって変更されない限り、整数引数は 3 型に強制変換され、浮動小数点引数は 4 に強制的に変換されます。 64 ビット システム上では、`int` は 32 ビット値です。そのため、*size* プレフィックスに **ll** または **I64** を使用しない限り、64 ビット整数は、出力のために書式設定される際に切り捨てが行われます。 **p** で指定されるポインター型では、プラットフォームの既定のポインター サイズが使用されます。
