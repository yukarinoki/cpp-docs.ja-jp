...
書式設定文字列に続く引数は、対応する *type* 文字と、省略可能な [size](#size) プレフィックスに従って解釈されます。 文字型およびの **`char`** 変換 **`wchar_t`** は、 **c**または**c**を使用して指定します。また、1バイトと複数バイトの文字列またはワイド文字列は、使用する書式設定関数に応じて**s**または**s**を使用して指定します。 **C**および**s**を使用して指定された文字引数と文字列引数は、 **`char`** ファミリ関数ではおよびとして解釈され **`char*`** `printf` 、ファミリ関数ではとして解釈され **`wchar_t`** `wchar_t*` `wprintf` ます。 **C**および**S**を使用して指定された文字引数と文字列引数は、 **`wchar_t`** ファミリ関数ではおよびとして解釈され `wchar_t*` `printf` 、ファミリ関数ではとして解釈され **`char`** **`char*`** `wprintf` ます。 この動作は、Microsoft 固有の動作です。

**`short`**、、 **`int`** **`long`** 、 **`long long`** 、およびそのバリアントなどの整数型 **`unsigned`** は、 **d**、 **i**、 **o**、 **u**、 **x**、および**x**を使用して指定します。、、などの浮動小数点型は、、 **`float`** **`double`** **`long double`** **a**、 **e**、 **e**、 **f**、 **f**、 **g**、 **g**を使用して指定します。 **a**既定では、*サイズ*プレフィックスによって変更されない限り、整数引数は型に強制変換され、 **`int`** 浮動小数点引数はに強制変換され **`double`** ます。 64ビットシステムで **`int`** は、は32ビットの値であるため、 **I64**の*サイズ*プレフィックスが使用されない限り、64ビット整数は出力用に書式**ll**設定されると切り捨てられます。 **p** で指定されるポインター型では、プラットフォームの既定のポインター サイズが使用されます。
...
||**E**、 **e**、 **f**、 **f**、 **a** **、または形式と共**に使用する場合、 **#** フラグは出力値に小数点を含めるように強制します。|小数点は、数字が続く場合にのみ表示されます。|
...
幅の指定がアスタリスク () の場合 `*` 、 **`int`** 引数リストから引数を指定すると値が提供されます。 次の例で示すように、引数リストでは、書式設定される値の前に *width* 引数を指定する必要があります。
...
精度指定がアスタリスク () の場合は、 \* **`int`** 引数リストの引数によって値が提供されます。 次の例に示すように、引数リストでは、書式設定される値の前に *precision* 引数を指定する必要があります。
...
`ptrdiff_t`型と `size_t` 型は **`__int32`** 、 **`unsigned __int32`** 32 ビットプラットフォームの場合は、または64ビットプラットフォームの場合は **`__int64`** **`unsigned __int64`** です。 サイズ プレフィックス **I** (大文字の i)、**j**、**t**、**z** は、プラットフォームに適切な引数の幅を受け取ります。

Visual C++ では、 **`long double`** は別個の型ですが、内部表現はと同じ **`double`** です。

型指定子 **hc** または **hC** は、`printf` 関数の **c** および `wprintf` 関数の **C** と同じ意味です。 **Lc**、 **lc**、 **wc**、または**wc**型指定子は、関数の**c** `printf` および関数の**c**と同義です `wprintf` 。 型指定子 **hs** または **hS** は、`printf` 関数の **s** および `wprintf` 関数の **S** と同じ意味です。 型指定子 **ls**、**lS**、**ws**、**wS** は、`printf` 関数の **S** および `wprintf` 関数の **s** と同じ意味です。

> [!NOTE]
> **Microsoft 固有:**\
> 引数サイズ修飾子 **I** (小文字の i)、**I32**、**I64**、および **w** は Microsoft の拡張機能で、ISO C とは互換性がありません。 型のデータと共に使用する場合の**h**プレフィックス、 **`char`** および**l** (小文字の l) プレフィックス (型のデータと共に使用する場合) **`double`** は Microsoft 拡張機能です。
