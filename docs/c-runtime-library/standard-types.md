---
description: 詳細については、「標準型」を参照してください。
title: 基本データ型
ms.date: 11/04/2016
f1_keywords:
- __time64_t
...
|`size_t` (対象のプラットフォームによって、符号なし __int64 または符号なし整数)|演算子の結果 **`sizeof`** 。|CRTDEFS.H およびその他のインクルード ファイル|
...
[C Run-Time ライブラリリファレンス](../c-runtime-library/c-run-time-library-reference.md)
