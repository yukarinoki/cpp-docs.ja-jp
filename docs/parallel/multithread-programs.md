...
プロセス内の各スレッドは独立して動作します。 設定して相互に表示されるが、いないスレッドは個別に実行し、プロセス内の他のスレッドに対応していません。 ただし、共通のリソースを共有するスレッドは、セマフォ、またはプロセス間通信の別のメソッドを使用して作業を調整する必要があります。 スレッドの同期の詳細については、次を参照してください。[マルチ スレッド Win32 プログラムの作成](../parallel/writing-a-multithreaded-win32-program.md)です。  
