...
`priority_buffer`クラスは、要素と要素を含む std::[tuple](../../standard-library/tuple-class.md)型のメッセージを受け取り `PriorityType` `Type` ます。 `PriorityType` は各メッセージの優先順位を保持する型を表し、`Type` はメッセージのデータ部分を表します。 `priority_buffer` クラスは、`Type` 型のメッセージを送信します。 `priority_buffer`また、クラスは2つのメッセージキューを管理します。これは、受信メッセージ用の[std::p riority_queue](../../standard-library/priority-queue-class.md)オブジェクトと、送信メッセージ用の std::[queue](../../standard-library/queue-class.md)オブジェクトです。 `priority_buffer` オブジェクトが複数のメッセージを同時に受信する場合、またはコンシューマーがまだメッセージを読み取っていないときに複数のメッセージを受信する場合、メッセージを優先順位に従って並べ替えると便利です。
...
1. C++ ヘッダーファイルを作成し、という名前を指定 `priority_buffer.h` します。 または、プロジェクトに含まれる既存のヘッダー ファイルを使用することもできます。
...
1. **`private`** クラスのセクションに、 `priority_buffer` 次のメンバー変数を追加します。
...
1. セクションで、 **`private`** コピーコンストラクターと代入演算子を定義します。 これにより、`priority_queue` オブジェクトが割り当て可能になるのを防ぎます。

    [!code-cpp[concrt-priority-buffer#7](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-custom-message-block_6.h)]

1. セクションで **`public`** 、多くのメッセージブロックの型に共通するコンストラクターを定義します。 デストラクターも定義します。

    [!code-cpp[concrt-priority-buffer#4](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-custom-message-block_7.h)]

1. セクションで、 **`public`** メソッド `enqueue` およびを定義し `dequeue` ます。 これらのヘルパー メソッドによって、`priority_buffer` オブジェクトとの間でメッセージを送受信する別の手段が提供されます。

    [!code-cpp[concrt-priority-buffer#5](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-custom-message-block_8.h)]

1. セクションで、 **`protected`** メソッドを定義し `propagate_to_any_targets` ます。

    [!code-cpp[concrt-priority-buffer#9](../../parallel/concrt/codesnippet/cpp/walkthrough-creating-a-custom-message-block_9.h)]

   `propagate_to_any_targets` メソッドは、入力キューの先頭にあるメッセージを出力キューに転送し、出力キュー内のすべてのメッセージを伝達します。

1. セクションで、 **`protected`** メソッドを定義し `accept_message` ます。
...
1. セクションで、 **`protected`** メソッドを定義し `reserve_message` ます。
...
1. セクションで、 **`private`** メソッドを定義し `propagate_priority_order` ます。
...
1. セクションで、 **`protected`** メソッドを定義し `propagate_message` ます。
...
コード例をコピーし、Visual Studio プロジェクトに貼り付けるか、という名前のファイルにクラスの定義を貼り付け、テストプログラムをという名前のファイルに貼り付け `priority_buffer` `priority_buffer.h` `priority_buffer.cpp` てから、Visual Studio のコマンドプロンプトウィンドウで次のコマンドを実行します。
