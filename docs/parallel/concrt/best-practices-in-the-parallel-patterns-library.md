...
- [小さなループ本体は並列化しない](#small-loops)

- [可能な限り高いレベルで並列化を表現する](#highest)

- [parallel_invoke を使用して分割と整理の問題を解決する](#divide-and-conquer)

- [取り消し処理または例外処理を使用して並列ループから抜ける](#breaking-loops)

- [取り消し処理および例外処理がオブジェクトの破棄に及ぼす影響について](#object-destruction)

- [並列ループではブロッキング操作を繰り返さない](#repeated-blocking)

- [並列処理を取り消すときにブロッキング操作を実行しない](#blocking)

- [並列ループでは共有データへの書き込みは行わない](#shared-writes)

- [可能な限り偽共有を避ける](#false-sharing)
...
このパターンを回避するようにコードをリファクタリングすることをお勧めします。 この例では、シリアルループでを呼び出すことによって、追加のスレッドを作成しないようにすることができ `send` **`for`** ます。
...
次の例について考えます。この例では、提供された述語関数を満たす指定の配列の要素を検索する `parallel_find_answer` 関数を定義します。 述語関数がを返すと **`true`** 、並列処理関数はオブジェクトを作成 `Answer` し、タスク全体を取り消します。
