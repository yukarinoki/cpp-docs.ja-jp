---
description: '詳細情報: concurrency 名前空間列挙型'
...
## <a name="message_status-enumeration"></a><a name="message_status"></a> message_status 列挙型
...
## <a name="policyelementkey-enumeration"></a><a name="policyelementkey"></a> PolicyElementKey 列挙型
...
|Name|説明|
|----------|-----------------|
|`ContextPriority`|スケジューラ内の各コンテキストのオペレーティングシステムスレッドの優先順位。 このキーが値に設定されている場合、スケジューラのコンテキストは、 `INHERIT_THREAD_PRIORITY` スケジューラを作成したスレッドの優先順位を継承します。<br /><br /> 有効な値: Windows `SetThreadPriority` 関数と特別な値の有効な値のいずれか `INHERIT_THREAD_PRIORITY`<br /><br /> 既定値: `THREAD_PRIORITY_NORMAL`|
|`ContextStackSize`|スケジューラの各コンテキストの予約済みスタックサイズ (kb 単位)。<br /><br /> 有効な値: 正の整数<br /><br /> 既定値: は `0` 、スタックサイズのプロセスの既定値が使用されることを示します。|
|`DynamicProgressFeedback`|スケジューラから収集された統計情報に従ってスケジューラのリソースを再分配するか、基になるハードウェアスレッドのサブスクリプションレベルに基づいて作成するかを決定します。 詳細については、「 [Dynamicのフィードの Backtype](#dynamicprogressfeedbacktype)」を参照してください。<br /><br /> 有効な値: `DynamicProgressFeedbackType` またはのいずれかの列挙体のメンバー。 `ProgressFeedbackEnabled``ProgressFeedbackDisabled`<br /><br /> 既定値: `ProgressFeedbackEnabled`|
|`LocalContextCacheSize`|`SchedulingProtocol`ポリシーキーが値に設定されている場合は `EnhanceScheduleGroupLocality` 、仮想プロセッサのローカルキューごとにキャッシュできる実行可能なコンテキストの最大数を指定します。 通常、このようなコンテキストは、実行可能になる原因となった仮想プロセッサの後入れ先出し (LIFO) の順序で実行されます。 `SchedulingProtocol`キーが値に設定されている場合、このポリシーキーは意味を持たないことに注意して `EnhanceForwardProgress` ください。<br /><br /> 有効な値: 負でない整数<br /><br /> 既定値: `8`|
|`MaxConcurrency`|スケジューラが必要とする最大同時実行レベル。 リソースマネージャーは、この多くの仮想プロセッサを最初に割り当てようとします。 特別な値の [Maxexecutionresources](concurrency-namespace-constants1.md#maxexecutionresources) は、目的の同時実行レベルがコンピューター上のハードウェアスレッドの数と同じであることを示します。 に指定された値 `MinConcurrency` がコンピューター上のハードウェアスレッドの数より大きく、として指定されている場合、の値はに設定されている `MaxConcurrency` ものと `MaxExecutionResources` `MaxConcurrency` 一致するように発生し `MinConcurrency` ます。<br /><br /> 有効な値: 正の整数と特別な値 `MaxExecutionResources`<br /><br /> 既定値: `MaxExecutionResources`|
...
## <a name="winrtinitializationtype-enumeration"></a><a name="winrtinitializationtype"></a> WinRTInitializationType 列挙型
