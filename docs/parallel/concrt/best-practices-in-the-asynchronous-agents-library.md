...
- [スロットリング機構を使用してデータ パイプラインのメッセージの数を制限する](#throttling)

- [データ パイプラインでは粒度の細かい処理を実行しない](#fine-grained)

- [大きなメッセージ ペイロードの値渡しは行わない](#large-payloads)
...
通常、エージェントで状態を分離するに **`private`** は、エージェントクラスのまたはセクションにデータメンバーを保持 **`protected`** し、メッセージバッファーを使用して状態の変更を通知します。 次の例は、 `basic_agent` [concurrency:: agent](../../parallel/concrt/reference/agent-class.md)から派生するクラスを示しています。 `basic_agent` クラスは、2 つのメッセージ バッファーを使用して外部コンポーネントと通信します。 2 つのメッセージ バッファーにはそれぞれ受信メッセージと送信メッセージが保持されます。
