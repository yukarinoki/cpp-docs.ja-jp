...
*section-name*  
初期化セクションの明示的な指定を許可します。 ユーザー指定のオブジェクト*セクション名*暗黙的に構築しないによってという名前のセクションでは、アドレスは、ただし、*セクション名*します。  
...
*func-name*  
プログラムの終了時に `atexit` の代わりに呼び出される関数を指定します。 このヘルパー関数も呼び出します[atexit](../c-runtime-library/reference/atexit.md)グローバル オブジェクトのデストラクターへのポインター。 次の形式のプラグマで関数の識別子を指定した場合、  
