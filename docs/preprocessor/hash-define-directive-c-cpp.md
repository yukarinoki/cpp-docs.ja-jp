---
title: '#ディレクティブ (C/C++) |Microsoft Docs'
ms.custom: ''
ms.date: 11/04/2016
ms.technology:
- cpp-tools
...
`#define` *識別子* `(` *識別子*opt`,`*.* `,`*識別子*opt`)`*トークン文字列*選択  
...
2 番目の構文形式は、パラメーターを受け取る関数に似たマクロを定義します。 この形式では、かっこで囲んだパラメーターのリストを使用できます (省略可能)。 マクロが、以降の各定義に出現する*識別子*(*識別子*... を選択します*識別子*opt) のバージョンに置き換え、  *。トークン文字列*仮パラメーターの代わりに使用する実際の引数を持つ引数。  
