...
- Visual Studio 2012 の C++ コンパイラでは、Visual C++ 2010 で実装された _ITERATOR_DEBUG_LEVEL の不一致の検出に加え、ランタイム ライブラリの不一致も検出します。 これらの不一致は、コンパイラ オプション `/MT` (静的なリリース)、`/MTd` (静的なデバッグ)、`/MD` (動的なリリース)、および `/MDd` (動的なデバッグ) が混在する場合に発生します。
