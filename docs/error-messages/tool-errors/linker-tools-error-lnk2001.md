...
- 関数プロトタイプをクラス定義に含めたが、関数の[実装が含まれて](../../error-messages/tool-errors/missing-function-body-or-variable.md)いない場合は、を呼び出す C++ コード。 この問題を解決するには、呼び出すすべてのクラスメンバーの定義を指定してください。
