...
|[コンパイラの警告 (レベル 1) C4805](compiler-warning-level-1-c4805.md)|'*operation*': 操作中の型 '*type1*' と型 'type1 ' の混合が安全ではありません|
|[コンパイラの警告 (レベル 1) C4806](compiler-warning-level-1-c4806.md)|'*operation*': 安全でない操作: 型 ' type1 ' に昇格される型'*type1*' の値が、指定された定数と等しくなることはできません|
|[コンパイラの警告 (レベル 1) C4807](compiler-warning-level-1-c4807.md)|'*operation*': 型 ' type1 ' と型 '*type1*' の符号付きビットフィールドの混合が安全ではありません|
...
|コンパイラの警告 C4843|'*type1*':配列または関数の型への参照の例外ハンドラーに到達できません。代わりに ' ' ' を使用してください|
...
|コンパイラの警告 (レベル 4) C5044|コマンドラインオプションの引数が、存在しないパス '*path*' を指しています。|
...
| コンパイラの警告 (レベル 1) C5105 | ' defined ' を生成するマクロ拡張の動作が定義されていません |
