...
|名前|説明|
|----------|-----------------|
|[COleServerItem:: AddOtherClipboardData](#addotherclipboarddata)|オブジェクトにプレゼンテーション形式と変換形式を配置し `COleDataSource` ます。|
...
|[COleServerItem:: OnRenderFileData](#onrenderfiledata)|`CFile`遅延レンダリングの一部として、オブジェクトにデータを取得します。|
...
*pDataSource*<br/>
`COleDataSource`サポートされているすべての形式で OLE 項目のデータを受け取るオブジェクトへのポインター。
...
*nDrawAspect*<br/>
OLE 項目のどの側面が変更されたかを示す DVASPECT 列挙の値。 このパラメーターには次の値を指定できます。
...
*nDrawAspect*<br/>
範囲を取得する OLE 項目の側面を指定します。 このパラメーターには次の値を指定できます。
...
*rSize*<br/>
`CSize`OLE 項目のサイズを受け取るオブジェクトへの参照。
...
*nDrawAspect*<br/>
境界が指定されている OLE 項目の側面を指定します。 このパラメーターには次の値を指定できます。
