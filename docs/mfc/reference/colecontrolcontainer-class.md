...
これを行うには、1つ以上の ActiveX コントロールサイト (によって実装される) をサポートし `COleControlSite` ます。 `COleControlContainer`[IOleInPlaceFrame](/windows/win32/api/oleidl/nn-oleidl-ioleinplaceframe)インターフェイスと[IOleContainer](/windows/win32/api/oleidl/nn-oleidl-iolecontainer)インターフェイスを完全に実装します。これにより、含まれている ActiveX コントロールは、インプレース項目としての条件を満たすことができます。

一般的に、このクラスはと共に使用 `COccManager` して `COleControlSite` 、カスタム activex コントロールコンテナーと、1つ以上の activex コントロール用のカスタムサイトを実装します。
...
- BST_INDETERMINATE は、ボタンの状態を淡色表示に設定し、不確定な状態を示します。 この値は、ボタンのスタイルが BS_3STATE または BS_AUTO3STATE の場合にのみ使用してください。
...
BS_AUTOCHECKBOX、BS_AUTORADIOBUTTON、BS_AUTO3STATE、BS_CHECKBOX、BS_RADIOBUTTON、または BS_3STATE スタイルで作成されたボタンからの戻り値。 以下のいずれかを指定できます。

- BST_CHECKED] ボタンがオンになっています。

- BST_INDETERMINATE ボタンは淡色表示になっており、不確定状態を示します (ボタンが BS_3STATE または BS_AUTO3STATE スタイルを持っている場合にのみ適用されます)。
