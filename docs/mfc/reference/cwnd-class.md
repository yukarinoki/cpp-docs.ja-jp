...
|[CWnd::GetSafeHwnd](#getsafehwnd)|はを返し `m_hWnd` **`this`** ます。ポインターが null の場合は null を返します。|
...
|[CWnd:: Getwindowの Childcount](#getwindowlesschildcount)|関連付けられているウィンドウなしの子ウィンドウの数を返します。|
...
|[CWnd::UnsubclassWindow](#unsubclasswindow)|ウィンドウをオブジェクトからデタッチします。 `CWnd`|
...
|[CWnd::GetCurrentMessage](#getcurrentmessage)|ウィンドウが現在処理しているメッセージへのポインターを返します。 は、 `On` *メッセージ* メッセージハンドラーのメンバー関数の場合にのみ呼び出す必要があります。|
...
|[CWnd::OnVKeyToItem](#onvkeytoitem)|WM_KEYDOWN メッセージへの応答として、によって所有されているリストボックスによって呼び出され `CWnd` ます。 [](#onkeydown)|
...
子ウィンドウは 2 つのステップで作成します。 まず、コンストラクターを呼び出してオブジェクトを構築した `CWnd` `CWnd` 後、 [create](#create) member 関数を呼び出して子ウィンドウを作成し、オブジェクトにアタッチし `CWnd` ます。
...
*varStart*<br/>
開始オブジェクトを指定します。 Windows SDK の「 *Varstart* 」を参照してください `IAccessible::accNavigate` 。

*pvarEndUpAt*<br/>
対象のユーザーインターフェイスオブジェクトに関する情報を受け取ります。 Windows SDK の「 *pvarEnd* 」を参照してください `IAccessible::accNavigate` 。
...
のデバイスコンテキストを識別 `CWnd` します。 ポインターは一時的なものであり、 [Endpaint](#endpaint)の範囲を超えて格納することはできません。
...
*pDSCWnd*<br/>
プロパティがバインドされるデータソースコントロールをホストするウィンドウを指します。 `GetDlgItem`このポインターを取得するには、dc のホストウィンドウのリソース ID を使用してを呼び出します。
...
*hWndNext*<br/>
クリップボードビューアーのチェーン内の次のウィンドウを識別 `CWnd` します。
...
ポイントを含む子ウィンドウを識別します。 指定されたポイントがクライアント領域の外部にある場合は NULL になります。 ポイントがクライアント領域内にあり、子ウィンドウ内に含まれていない場合 `CWnd` は、が返されます。

このメンバー関数は、指定されたポイントを含む、非表示または無効な子ウィンドウを返します。

複数のウィンドウに、指定したポイントが含まれている場合があります。 ただし、この関数は、 `CWnd` ポイントを含む最初のウィンドウの * だけを返します。

返される * は一時的なものである `CWnd` 可能性があり、後で使用するために格納することはできません。
...
`ClientToScreen`このメンバー関数は、 `POINT` または構造体、また `RECT` `CPoint` は `CRect` *lppoint* または *lpRect* が指すオブジェクトまたはオブジェクトのクライアント座標を使用して、新しい画面座標を計算します。その後、構造内の座標を新しい座標に置き換えます。 新しい画面座標は、システム画面の左上隅を基準としています。
...
*pszClass*<br/>
この文字列には、クラスの OLE "short name" (ProgID) を含めることができます (例: "CIRC3")。Circ3Ctrl "です。 名前は、コントロールによって登録されている名前と同じである必要があります。 また、文字列には、かっこで囲まれた CLSID の文字列形式を含めることができます。たとえば、"{9DBAFCCF-59208CEC297B}" のようになります。 どちらの場合も、は `CreateControl` 文字列を対応するクラス ID に変換します。
...
*pSize*<br/>
[](/windows/win32/api/windef/ns-windef-size) `CSize` コントロールのサイズを格納しているサイズ構造体またはオブジェクトをポイントします。
...
`CWnd`メンバー関数によって作成された一時オブジェクトを削除 `FromHandle` します。
...
`DestroyWindow`また、メンバー関数は、 [CDialog:: Create](../../mfc/reference/cdialog-class.md#create)によって作成されたモードレスダイアログボックスを破棄します。

`CWnd`破棄されるが子ウィンドウであり、 [WS_EX_NOPARENTNOTIFY](../../mfc/reference/styles-used-by-mfc.md#extended-window-styles)スタイルが設定されていない場合[、WM_PARENTNOTIFY](/windows/win32/inputmsg/wm-parentnotify)メッセージは親に送信されます。
...
*nIDStaticPath*<br/>
現在のドライブとディレクトリを表示するために使用する静的テキストコントロールの識別子を指定します。 *NIDStaticPath* が0の場合、はその `DlgDirList` ようなテキストコントロールが存在しないことを前提としています。
...
- LB_DIR フラグを DDL_POSTMSGS します。 LB_DIR フラグが設定されている場合、によって生成されるメッセージは `DlgDirList` アプリケーションのキューに格納されます。それ以外の場合は、ダイアログボックスのプロシージャに直接送信されます。

- DDL_DRIVES ドライブ。 DDL_DRIVES フラグが設定されている場合、DDL_EXCLUSIVE フラグは自動的に設定されます。 したがって、ドライブとファイルを含むディレクトリの一覧を作成するには、を2回呼び出す必要があります。1回は DDL_DRIVES フラグを設定し、もう1つは `DlgDirList` リストの残りの部分にフラグを設定します。
...
*nIDStaticPath*<br/>
現在のドライブとディレクトリを表示するために使用する静的テキストコントロールの識別子を指定します。 *NIDStaticPath* が0の場合、はその `DlgDirListComboBox` ようなテキストコントロールが存在しないことを前提としています。
...
- CB_DIR フラグを DDL_POSTMSGS します。 CB_DIR フラグが設定されている場合、によって生成されるメッセージは `DlgDirListComboBox` アプリケーションのキューに格納されます。それ以外の場合は、ダイアログボックスのプロシージャに直接送信されます。

- DDL_DRIVES ドライブ。 DDL_DRIVES フラグが設定されている場合、DDL_EXCLUSIVE フラグは自動的に設定されます。 したがって、ドライブとファイルを含むディレクトリの一覧を作成するには、を2回呼び出す必要があります。1回は DDL_DRIVES フラグを設定し、もう1つは `DlgDirListComboBox` リストの残りの部分にフラグを設定します。
...
キャレットがメンバー関数によって非表示にされている場合は `BeginPaint` 、 `EndPaint` キャレットを画面に復元します。
...
*lpszClassName*<br/>
ウィンドウのクラス名 (構造体) を指定する null で終わる文字列を指し `WNDCLASS` ます。 *Lpclassname* が NULL の場合、すべてのクラス名が一致します。
...
*pszHelpFile*<br/>
指定したオブジェクトに関連付けられているファイルの完全パスを受け取る BSTR のアドレス `WinHelp` (存在する場合)。
...
*ppdispParent*<br/>
親オブジェクトのインターフェイスのアドレスを受け取り `IDispatch` ます。 親が存在しない場合、または子が親にアクセスできない場合、変数は NULL に設定されます。
...
このポインターは解放しないでください `IUnknown` 。 通常は、を使用して、コントロールの特定のインターフェイスを取得します。

によって返されたインターフェイスポインター `GetControlUnknown` は参照カウントされていません。 以前に[iunknown:: AddRef](/windows/win32/api/unknwn/nf-unknwn-iunknown-addref)を呼び出した場合を除き、ポインターに対して[Iunknown:: Release](/windows/win32/api/unknwn/nf-unknwn-iunknown-release)を呼び出さないでください。
...
成功した場合、クライアント領域のデバイスコンテキストを識別し `CWnd` ます。それ以外の場合、戻り値は NULL になります。 ポインターは一時的である可能性があり、後で使用するために格納しないでください。
...
- DCX_CLIPCHILDREN は、ウィンドウの下にあるすべての子ウィンドウの表示領域を除外 `CWnd` します。
...
- DCX_PARENTCLIP は親ウィンドウの可視領域を使用し、親ウィンドウの WS_CLIPCHILDREN と WS_PARENTDC スタイルビットを無視します。 この値により、デバイスコンテキストの原点がウィンドウの左上隅に設定され `CWnd` ます。
...
オブジェクトへのポインター `CWnd` 。子ウィンドウが見つからない場合は NULL。
...
`CWnd`関数が成功した場合は子ウィンドウの数値識別子。それ以外の場合は0。

### <a name="remarks"></a>解説

トップレベルウィンドウには ID 値がないため、 `CWnd` がトップレベルウィンドウの場合、この関数の戻り値は無効です。
...
*Bsigned* が TRUE の場合、は `GetDlgItemInt` テキストの先頭にマイナス記号 (-) があるかどうかを確認し、テキストを符号付き数値に変換します。 それ以外の場合は、符号なしの値を作成します。
...
データソースコントロールによって定義されたカーソルへのポインター。 MFC は `AddRef` ポインターに対してを呼び出します。

### <a name="remarks"></a>解説

返されたポインターを使用して、データバインドされたグリッドコントロールなど、複雑なデータバインドコントロールの ICursor プロパティを設定します。 最初のバインドされたコントロールがカーソルを要求するまで、データソースコントロールはアクティブになりません。 これは、 `GetDSCCursor` または MFC バインディングマネージャーによる暗黙的な呼び出しによって明示的に発生します。 どちらの場合でも、を呼び出してを呼び出すことにより、データソースコントロールを強制的にアクティブにすることができ `GetDSCCursor` ます。その後、 `Release` への返されたポインターに対してを呼び出し `IUnknown` ます。 アクティブ化によって、データソースコントロールが基になるデータソースへの接続を試行します。 返されたポインターは、次のコンテキストで使用される場合があります。
...
前景ウィンドウへのポインター。 これは一時的なオブジェクトである可能性があり `CWnd` ます。
...
メニューを識別します。 にメニューがない場合、値は NULL になり `CWnd` ます。 が子ウィンドウの場合、戻り値は未定義です `CWnd` 。
...
> [!NOTE]
> MFC は、ウィンドウなしの ActiveX コントロール、標準 ActiveX コントロール、およびウィンドウをサポートしているため、HWND だけで制御を参照することはできません。 オブジェクトには、次のように、ウィンドウ `COleControlSiteOrWnd` activex コントロール、ウィンドウなしの activex コントロール、またはウィンドウとしてオブジェクトを識別する情報が含まれています。
...
*pCurSiteOrWnd*<br/>
コントロールを識別 `COleControlSiteOrWnd` します。 の詳細について `COleControlSiteOrWnd` は、「 [CWnd:: GetNextDlgGroupItem](#getnextdlggroupitem)」を参照してください。
...
*n*<br/>
関数が次のウィンドウまたは前のウィンドウへのポインターを返すかどうかを指定します。 GW_HWNDNEXT のいずれかになります。これは、ウィンドウマネージャーの一覧のオブジェクトの後ろにあるウィンドウを返し `CWnd` ます。または、ウィンドウマネージャーの一覧の前のウィンドウを返す GW_HWNDPREV を返します。
...
ウィンドウのウィンドウハンドルを返します。 `CWnd`がウィンドウにアタッチされていない場合、またはが null ポインターと共に使用されている場合は、null を返し `CWnd` ます。
...
*pParent*<br/>
親ウィンドウへのポインター `CWnd` 。 NULL の場合もあります。
...
子ウィンドウのリンクリスト内のトップレベルの子ウィンドウを識別 `CWnd` します。 子ウィンドウが存在しない場合、値は NULL になります。
...
*lpRect*<br/>
`CRect`更新領域を囲む更新プログラムのクライアント座標を受け取るオブジェクトまたは[RECT 構造体](/windows/win32/api/windef/ns-windef-rect)を指します。

更新領域が内に存在するかどうかを判断するには、このパラメーターを NULL に設定し `CWnd` ます。 *LpRect* が NULL の場合、 `GetUpdateRect` メンバー関数は、更新領域が存在する場合は0以外の値を返し、それ以外の場合は0を返します。 これにより、WM_PAINT メッセージの結果が無効な領域であるかどうかを確認できます。 Windows バージョン3.0 以前では、このパラメーターを NULL に設定しないでください。
...
*nCmd*<br/>
と返されるウィンドウの間のリレーションシップを指定し `CWnd` ます。 次のいずれかの値を取ることができます。

- GW_CHILD は、 `CWnd` 最初の子ウィンドウを識別します。

- `CWnd`が子ウィンドウの場合 GW_HWNDFIRST は、最初の兄弟ウィンドウを返します。 それ以外の場合は、リスト内の最初のトップレベルウィンドウを返します。

- `CWnd`が子ウィンドウの場合は、最後の兄弟ウィンドウを返し GW_HWNDLAST ます。 それ以外の場合は、リスト内の最後のトップレベルウィンドウを返します。
...
ウィンドウ表示コンテキストは、の任意の場所での描画を許可し `CWnd` ます。これは、コンテキストの原点がクライアント領域ではなくの左上隅であるため `CWnd` です。
...
*lpwndpl*<br/>
`WINDOWPLACEMENT`状態の表示と位置の情報を受け取る構造体を指します。
...
*lpRect*<br/>
`CRect`左上隅と右下隅の画面座標を受け取るオブジェクトまたは[RECT 構造体](/windows/win32/api/windef/ns-windef-rect)を指します。
...
[](#onpaint) `CWnd` 更新領域が空ではなく、そのウィンドウのアプリケーションキューに他のメッセージがない場合は、Windows によって WM_PAINT メッセージが送信されます。
...
*lpRect*<br/>
`CRect`更新領域に追加する四角形 (クライアント座標) を格納するオブジェクトまたは[RECT 構造体](/windows/win32/api/windef/ns-windef-rect)を指します。 *LpRect* が NULL の場合、クライアント領域全体が領域に追加されます。
...
*pbParamInfo*<br/>
*Pbparaminfo* に続くパラメーターの型を指定する、null で終わる文字列のバイトを指すポインター。 使用可能な値については、「」の「解説」を参照してください `COleDispatchDriver::InvokeHelper` 。
...
関数の結果を指定します。 *PWnd* によって識別されるウィンドウがの子ウィンドウである場合、値は0以外の値です。それ以外の場合は `CWnd` 0 です。
...
が最小化されている場合は0以外 `CWnd` 。それ以外の場合は0。
...
にタッチサポートがある場合は TRUE。それ以外の場合は `CWnd` FALSE。
...
*lpszText*<br/>
`CString`表示するメッセージを格納しているオブジェクトまたは null で終わる文字列を指します。

*lpszCaption*<br/>
`CString`メッセージボックスのキャプションに使用するオブジェクトまたは null で終わる文字列を指します。 *Lpszcaption* が NULL の場合、既定のキャプション "Error" が使用されます。
...
> [!NOTE]
> 特定のコントロールの一部のスタイル (たとえば、エディットコントロールの ES_READONLY スタイル) では、 `ModifyStyle` コントロールが特殊な内部処理を実行する必要があるため、スタイルが正しく変更されない場合があります。 このような場合、スタイルを変更するための対応するメッセージが使用できるようになります (前述の例では EM_SETREADONLY)。
...
*pWndOther*<br/>
`CWnd`アクティブまたは非アクティブ化されるへのポインター。 ポインターは NULL にすることができ、一時的なものでもかまいません。

*bMinimized*<br/>
アクティブまたは非アクティブ化するの最小化された状態を指定し `CWnd` ます。 値が TRUE の場合は、ウィンドウが最小化されていることを示します。

TRUE の場合、 `CWnd` がアクティブになります。それ以外の場合は非アクティブになります。

### <a name="remarks"></a>解説

`CWnd`オブジェクトがマウスクリックでアクティブ化された場合は、 [Onmouseactivate](#onmouseactivate)メンバー関数呼び出しも受け取ります。
...
*dwThreadID*<br/>
スレッド ID の値を指定します。 *Bactive* が TRUE の場合、 *dwthreadid* は `CWnd` 非アクティブ化されているを所有しているスレッドを識別します。 *Bactive* が FALSE の場合、 *dwthreadid* は、アクティブ化されているを所有しているスレッドを識別 `CWnd` します。
...
`CWnd`呼び出しを受け取る各オブジェクト `OnChangeCbChain` は、 [SendMessage](/windows/win32/api/winuser/nf-winuser-sendmessage) Windows 関数を使用して、 [WM_CHANGECBCHAIN](/windows/win32/dataxchg/wm-changecbchain)メッセージをクリップボードビューアーチェーン (から返されたハンドル) の次のウィンドウに送信する必要があり `SetClipboardViewer` ます。 *HWndRemove* がチェーン内の次のウィンドウである場合、 *hWndAfter* によって指定されたウィンドウが次のウィンドウになり、クリップボードのメッセージがそのウィンドウに渡されます。
...
構造体には、 `CREATESTRUCT` ウィンドウの作成に使用されるパラメーターのコピーが含まれます。
...
`hwndItem`構造体のメンバーが `DELETEITEMSTRUCT` コンボボックスまたはリストボックスに属している場合 `DeleteItem` は、適切なクラスの仮想関数が呼び出されます。 `DeleteItem`項目固有のデータを削除するには、適切なコントロールのクラスのメンバー関数をオーバーライドします。
...
*lpDeviceName*<br/>
Windows 初期化ファイルに指定されているデバイス名を指します (WIN.INI)。

### <a name="remarks"></a>解説

WM_DEVMODECHANGE メッセージを処理するアプリケーションは、デバイスモード設定を再初期化することがあります。 Windows の機能を使用して `ExtDeviceMode` デバイスの設定を保存および復元するアプリケーションは、通常、この機能を処理しません。
...
*hDropInfo*<br/>
削除されたファイルを記述する内部データ構造へのポインター。 このハンドルは `DragFinish` 、、、および Windows の各関数が、削除され `DragQueryFile` `DragQueryPoint` たファイルに関する情報を取得するために使用されます。
...
*bEnable*<br/>
オブジェクトが有効か無効かを指定し `CWnd` ます。 が有効になっている場合、このパラメーターは TRUE になります。が無効になっ `CWnd` ている場合は、FALSE になり `CWnd` ます。
...
既定の実装は、 `hbrBackground` ウィンドウクラス構造体のメンバーによって指定されたウィンドウクラスの背景ブラシを使用して、背景を消去します。

`hbrBackground`メンバーが NULL の場合、オーバーライドされたのバージョンによって `OnEraseBkgnd` 背景色が消去されます。 また、このバージョンでは、最初にブラシの UnrealizeObject を `CWnd` 呼び出し、次に[](/windows/win32/api/wingdi/nf-wingdi-unrealizeobject)ブラシを選択することによって、目的のブラシの原点を座標で揃える必要があります。

メッセージを `OnEraseBkgnd` 処理して背景を消去する場合、オーバーライドされたは WM_ERASEBKGND に応答して0以外の値を返します。これは、これ以上消去する必要がないことを示します。 0を返した場合、ウィンドウは消去する必要があるとマークされたままになります。 (通常、これは、構造体のメンバーが TRUE になることを意味 `fErase` `PAINTSTRUCT` します)。
...
通常、コントロールのすべての矢印キーとタブキーの入力は、Windows によって処理さ `CWnd` れます。 をオーバーライドすることにより `OnGetDlgCode` 、 `CWnd` コントロールは特定の種類の入力を選択してそれ自体を処理することができます。
...
Windows は、さまざまな位置および寸法の既定値を指定して、ポイントの配列を設定します。 では、アプリケーションによってこれらの値が変更される可能性があり `OnGetMinMaxInfo` ます。
...
システムキー以外のキーは、ALT キーが押されていないとき、またはに入力フォーカスがあるときに押されたキーボードキーです `CWnd` 。
...
*pMenu*<br/>
選択されたへのポインターを格納し `CMenu` ます。 ポインターは一時的なものである可能性があるため、格納しないでください。
...
- オブジェクトをアクティブに MA_ACTIVATE `CWnd` ます。

- MA_NOACTIVATE オブジェクトをアクティブにしません `CWnd` 。

- `CWnd`オブジェクトをアクティブにして、マウスイベントを破棄 MA_ACTIVATEANDEAT ます。

- MA_NOACTIVATEANDEAT オブジェクトをアクティブ化せず、 `CWnd` マウスイベントを破棄しません。
...
マウスがキャプチャされていない場合は、マウスカーソルの下にあるオブジェクトによって WM_MOUSEMOVE メッセージが受信されます `CWnd` 。それ以外の場合は、マウスをキャプチャしたウィンドウにメッセージが表示されます。
...
*lpncsp*<br/>
アプリケーションが四角形[](/windows/win32/api/winuser/ns-winuser-nccalcsize_params)の新しいサイズと位置 `CWnd` (クライアント領域、境界線、キャプション、スクロールバーなど) を計算するために使用できる情報を格納している NCCALCSIZE_PARAMS データ構造体を指します。
...
*視点*<br/>
`CPoint`カーソル位置の x および y 画面座標を格納しているオブジェクトを指定します。 これらの座標は、常に画面の左上隅に対して相対的です。
...
内部 WM_PAINT メッセージは、Windows によって1回だけ送信されます。 内部 WM_PAINT メッセージがメンバー関数によってウィンドウに送信された後 `UpdateWindow` 、ウィンドウが無効化されるまで、または `RedrawWindow` RDW_INTERNALPAINT フラグが設定された状態でメンバー関数が再度呼び出されるまで、これ以上の WM_PAINT メッセージは送信または送信されません。
...
`OpenClipboard`メンバー関数または Windows 関数は、 `CloseClipboard` 内から呼び出さないで `OnRenderFormat` ください。
...
既定の実装は、処理の前に親ウィンドウを呼び出し `OnSetCursor` ます。 親ウィンドウが TRUE を返した場合、それ以降の処理は停止します。 親ウィンドウを呼び出すと、子ウィンドウでカーソルの設定を親ウィンドウで制御できるようになります。

既定の実装では、カーソルがクライアント領域に存在しない場合、または登録されているクラスカーソルである場合は、矢印に設定されます。

*NHitTest* が HTERROR で、 *message* がマウスのボタンダウンメッセージである場合は、 `MessageBeep` メンバー関数が呼び出されます。

*メッセージ* パラメーターは、がメニューモードに入ったときに0になり `CWnd` ます。
...
*pOldWnd*<br/>
`CWnd`入力フォーカスを失うオブジェクトを格納します (NULL の場合もあります)。 ポインターは一時的である可能性があり、後で使用するために格納しないでください。
...
*uFlags*<br/>
システムが呼び出しの結果としてメッセージを送信すると `SystemParametersInfo` 、このパラメーターは変更されたシステムパラメーターを示すフラグになります。 値の一覧については、Windows SDK の「 [SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) 」を参照してください。 アプリケーションがメッセージを送信する場合、このパラメーターは0である必要があります。
...
メッセージマップには、これらのマクロのうち1つだけを含める必要があります。 Windows 95/98 と Windows NT 4.0 の両方で動作するプログラムを作成するには、ON_WM_SETTINGCHANGE のハンドラーを記述します。 Windows NT 3.51 では、ハンドラーはと Uflags によって呼び出され、 `OnSettingChange` 常に0になります。 
...
ウィンドウは、メンバー関数が呼び出されたとき、重なったウィンドウが最大化または復元されたとき、または重なったウィンドウまたはポップアップウィンドウが閉じられたとき (アイコンが表示されたとき) または開いたとき `ShowWindow` (画面に表示される場合) に表示されます。 重なったウィンドウを閉じると、そのウィンドウに関連付けられているすべてのポップアップウィンドウが非表示になります。
...
- オブジェクトを SC_CLOSE 閉じ `CWnd` ます。

- アプリケーションによっ `CWnd` て指定されたホットキーに関連付けられたオブジェクトをアクティブに SC_HOTKEY ます。 *LParam* の下位ワードは、アクティブ化するウィンドウの HWND を識別します。

- SC_HSCROLL 水平にスクロールします。

- キーストロークを使用してメニューを取得 SC_KEYMENU ます。

- SC_MAXIMIZE (または SC_ZOOM) によってオブジェクトが最大化さ `CWnd` れます。

- SC_MINIMIZE (または SC_ICON) オブジェクトを最小化 `CWnd` します。

- マウスのクリックでメニューを取得 SC_MOUSEMENU ます。

- オブジェクトを移動 SC_MOVE `CWnd` ます。
...
- SC_SCREENSAVE は、SYSTEM.INI ファイルの [ブート] セクションで指定されたスクリーンセーバーアプリケーションを実行します。

- オブジェクトのサイズを SC_SIZE `CWnd` します。
...
呼び出しの場合 `OnSysKeyDown` 、キー移行ビット (ビット 15) は0です。 キーが押されている間に ALT キーが押されている場合、コンテキストコードビット (ビット 13) は1です。ウィンドウに入力フォーカスがないため、メッセージがアクティブウィンドウに送信された場合は0になります。

### <a name="remarks"></a>解説

現在入力フォーカスがあるウィンドウがない場合は、アクティブなウィンドウの `OnSysKeyDown` メンバー関数が呼び出されます。 `CWnd`メッセージを受け取るオブジェクトは、のコンテキストコードをチェックすることによって、これら2つのコンテキストを区別でき `nFlags` ます。
...
呼び出しの場合 `OnSysKeyUp` 、キー移行ビット (ビット 15) は1です。 キーが押されている間に ALT キーが押されている場合、コンテキストコードビット (ビット 13) は1です。ウィンドウに入力フォーカスがないため、メッセージがアクティブウィンドウに送信された場合は0になります。

### <a name="remarks"></a>解説

現在入力フォーカスがあるウィンドウがない場合は、アクティブなウィンドウの `OnSysKeyUp` メンバー関数が呼び出されます。 `CWnd`呼び出しを受け取るオブジェクトは、 *nFlags* のコンテキストコードをチェックすることによって、これら2つのコンテキストを区別できます。

コンテキストコードが0の場合は、によって受信された WM_SYSKEYUP メッセージを `OnSysKeyUp` [TranslateAccelerator](/windows/win32/api/winuser/nf-winuser-translateacceleratorw) Windows 関数に渡すことができます。これにより、システムキーメッセージではなく通常のキーメッセージと同じように処理されます。 これにより、アクティブウィンドウに入力フォーカスがない場合でも、アクティブウィンドウでアクセラレータ (ショートカット) キーを使用できるようになります。
...
ツールヒントが関連付けられている領域がボタンでない場合は、 `OnToolHitTest` 構造体のフラグを TTF_NOTBUTTON と TTF_CENTERTIP に設定します。

`OnToolHitTest`既定とは異なる情報を提供するようにをオーバーライドします。
...
*lpwndpos*<br/>
`WINDOWPOS`ウィンドウの新しいサイズと位置に関する情報を格納しているデータ構造体を指します。
...
[SystemParametersInfo](/windows/win32/api/winuser/nf-winuser-systemparametersinfow) Windows 関数は、 `OnWinIniChange` アプリケーションが関数を使用して WIN.INI ファイル内の設定を変更した後に、を呼び出します。
...
> [!WARNING]
> `CWnd::PreCreateWindow`は **`this`** 、メニューが NULL で、スタイルに WS_CHILD が含まれている場合に、cs の hMenu メンバーをポインターに割り当てます。 適切な機能を使用するには、ダイアログコントロールの ID が NULL でないことを確認してください。
>
> この変更により、マネージ/ネイティブの相互運用シナリオでのクラッシュが修正されます。 のトレースステートメントは、 `CWnd::Create` 問題の開発者に警告します。
...
から派生 `CWnd` した各クラスは、のオーバーライドに独自の機能を追加し `PreCreateWindow` ます。 仕様として、これらの派生 `PreCreateWindow` は記載されていません。 各クラスに適したスタイルとスタイル間の相互依存関係を判断するには、アプリケーションの基本クラスの MFC ソースコードを確認します。 オーバーライドを選択した場合は、 `PreCreateWindow,` MFC ソースコードから収集された情報を使用して、アプリケーションの基本クラスで使用されているスタイルが必要な機能を提供するかどうかを判断できます。
...
既定では、関数の影響を受けるウィンドウは、 `RedrawWindow` 指定されたウィンドウのスタイルが WS_CLIPCHILDREN かどうかによって異なります。 WS_CLIPCHILDREN ウィンドウの子ウィンドウは影響を受けません。 ただし、windows WS_CLIPCHILDREN ではないウィンドウは、WS_CLIPCHILDREN ウィンドウが検出されるまで再帰的に検証または無効化されます。 次のフラグは、どのウィンドウが関数の影響を受けるかを制御し `RedrawWindow` ます。
...
*lpRectUpdate*<br/>
`RECT`スクロールによって無効にされた四角形の境界を受け取る構造体をポイントします。 このパラメーターは NULL の場合もあります。
...
- LpRectScroll によって示される四角形と交差するすべての子ウィンドウを SW_SCROLLCHILDREN スクロールするには、 *dx* と *dy* で指定されたピクセル数をします。 Windows は、移動しない場合でも、 *lpRectScroll* と交差するすべての子ウィンドウに [WM_MOVE](/windows/win32/winmsg/wm-move)メッセージを送信します。 カーソルの位置は、子ウィンドウがスクロールされ、カーソルの四角形がスクロール用の四角形と交差するときに再配置されます。
...
`CWnd`すべてのマウス入力が不要になった場合は、他のウィンドウがマウス入力を受け取ることができるように、アプリケーションは[ReleaseCapture](/windows/win32/api/winuser/nf-winuser-releasecapture)関数を呼び出す必要があります。
...
の呼び出しが、 `SetScrollRange` SetScrollPos メンバー関数の呼び出しの直後にある場合、 メンバー関数の[](#setscrollpos)パラメーターは0になり、 `SetScrollPos` スクロールバーが2回描画されないようにする必要があります。
...
*lpfnTimer*<br/>
WM_TIMER メッセージを処理する、アプリケーションによって提供さ `TimerProc` れる[](/windows/win32/winmsg/wm-timer)コールバック関数のアドレスを指定します。 このパラメーターが NULL の場合、WM_TIMER メッセージはアプリケーションのメッセージキューに配置され、オブジェクトによって処理され `CWnd` ます。
...
間隔の値を指定すると、間隔が経過するたびに、システムは、インストールするアプリケーションのインストールメッセージキューに WM_TIMER メッセージをポストするか、アプリケーション定義のコールバック関数にメッセージを渡し `TimerProc` ます。

*LpfnTimer* callback 関数には名前を付ける必要はありません `TimerProc` が、static として宣言し、次のように定義する必要があります。
...
この例 `CWnd::SetTimer` では、、、およびを使用し `CWnd::OnTimer` て、 `CWnd::KillTimer` WM_TIMER メッセージを処理します。 最初のタイマーは、の2秒ごとにメインフレームウィンドウに WM_TIMER メッセージを送信するように設定されてい `OnStartTimer` ます。 `OnTimer`イベントハンドラーは、メインフレームウィンドウの WM_TIMER メッセージを処理します。 このメソッドにより、PC スピーカーは 2 秒ごとにビープ音を鳴らします。 2 番目のタイマーは、3.75 秒ごとにコールバック関数にメッセージを送信します。 `OnStopTimer` は、各タイマー ID の `CWnd::KillTimer` を呼び出して、両方のタイマーを停止します。
...
`ShowCaret`このメンバー関数は、現在の図形があり、連続して複数回非表示になっていない場合にのみ、キャレットを表示します。 このウィンドウがカーソルを所有していない場合、キャレットは表示されません。

キャレットを非表示にすることは累積的です。 [HideCaret](#hidecaret)メンバー関数が連続して5回呼び出されている場合、は、 `ShowCaret` カレットを表示するために5回呼び出される必要があります。
...
*nCmdShow*<br/>
を表示する方法を指定し `CWnd` ます。 次のいずれかの値を指定する必要があります。
...
`ShowWindow`[CWinApp:: m_nCmdShow](../../mfc/reference/cwinapp-class.md#m_ncmdshow)のメインウィンドウに対して、アプリケーションごとに1回だけ呼び出す必要があります。 後続のへの呼び出しでは、 `ShowWindow` で指定された値の代わりに、上記のいずれかの値を使用する必要があり `CWinApp::m_nCmdShow` ます。
...
このメンバー関数は、Windows コントロールをオブジェクトにアタッチ `CWnd` し、コントロールの `WndProc` および `AfxWndProc` 関数を置き換えます。 関数は、 `WndProc` メンバー関数によって返される位置に古いを格納し `GetSuperWndProcAddr` ます。
...
このメンバー関数は、Windows コントロールをオブジェクトにアタッチ `CWnd` し、ウィンドウの `WndProc` 関数と関数を置き換え `AfxWndProc` ます。 関数は、オブジェクト内の古いへのポインターを格納し `WndProc` `CWnd` ます。
...
この `UpdateWindow` メンバー関数は、アプリケーションキューをバイパスして WM_PAINT メッセージを直接送信します。 更新リージョンが空の場合、WM_PAINT は送信されません。
...
[Beginpaint](#beginpaint)メンバー関数は、クライアント領域全体を自動的に検証します。 `ValidateRect` [WM_PAINT](/windows/win32/gdi/wm-paint)が次に生成される前に更新領域の一部を検証する必要がある場合、または[ValidateRgn](#validatergn)メンバー関数を呼び出す必要はありません。
...
[Beginpaint](#beginpaint)メンバー関数は、クライアント領域全体を自動的に検証します。 次の[](#validaterect) `ValidateRgn` [WM_PAINT](/windows/win32/gdi/wm-paint)メッセージが生成される前に更新領域の一部を検証する必要がある場合は、ValidateRect もメンバー関数も呼び出さないでください。
...
*ulFlags*<br/>
オプションの変更を指定する一連のビットフラグ。 このフィールドには、0または次の値のいずれかを含めることができます: TWF_FINETOUCH、TWF_WANTPALM。
