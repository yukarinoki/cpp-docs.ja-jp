...
|[CRichEditCtrl::GetTextLength](#gettextlength)|この`CRichEditCtrl`オブジェクトのテキストの長さを文字数で取得します。 には、終端の null 文字は含まれません。|
...
|[CRichEditCtrl:: StreamIn](#streamin)|入力ストリームからこの`CRichEditCtrl`オブジェクトにテキストを挿入します。|
...
この`CRichEditCtrl`オブジェクトで、WYSIWYG に使用されるターゲットデバイスと線の幅を設定します (表示される内容は次のようになります)。
...
- SF_TEXT は、テキストのみを読み取ることを示します。

- SF_RTF は、テキストの読み取りと書式設定を示します。

これらの値のいずれかを SFF_SELECTION と組み合わせることができます。 場合 SFF_SELECTION が指定さ`StreamIn`れている場合、現在の選択項目を入力ストリームの内容に置き換えます。 指定されていない`StreamIn`場合、はこの`CRichEditCtrl`オブジェクトの内容全体を置き換えます。
...
- SF_TEXT は、テキストのみを書き込むことを示します。

- SF_RTF は、テキストと書式設定を記述します。

- SF_RTFNOOBJS は、テキストと書式設定を記述し、OLE 項目をスペースに置き換えます。

- SF_TEXTIZED は、OLE 項目のテキスト表現を使用してテキストと書式設定を記述します。

これらの値のいずれかを SFF_SELECTION と組み合わせることができます。 場合 SFF_SELECTION が指定さ`StreamOut`れている場合、現在の選択項目を出力ストリームに書き込みます。 指定されていない`StreamOut`場合、はこの`CRichEditCtrl`オブジェクトの内容全体を書き込みます。
