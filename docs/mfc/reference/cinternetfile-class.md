...
を使用して、 `Flush` メモリ内のすべてのデータが実際にターゲットコンピューターに書き込まれたことを確認し、ホストコンピューターとのトランザクションが完了したことを保証します。 `Flush`は `CInternetFile` 、書き込み用に開かれたオブジェクトに対してのみ有効です。
