---
description: '詳細情報: CBasePane クラス'
title: CBasePane クラス
ms.date: 11/06/2018
f1_keywords:
- CBasePane
...
|[CBasePane::D Ocktoフレームウィンドウ](#docktoframewindow)|ドッキング可能なペインをフレームにドッキングします。|
...
|`CBasePane::get_accParent`|オブジェクトの親のインターフェイスを取得するために、フレームワークによって呼び出され `IDispatch` ます。 ( [CWnd:: get_accParent](../../mfc/reference/cwnd-class.md#get_accparent)をオーバーライドします)。|
...
|[CBasePane:: InsertPane](#insertpane)|指定したペインをドッキングマネージャーに登録します。|
...
|`CBasePane::PreTranslateMessage`|[TranslateMessage](/windows/win32/api/winuser/nf-winuser-translatemessage)および[DispatchMessage](/windows/win32/api/winuser/nf-winuser-dispatchmessage) Windows 関数にディスパッチされる前に、ウィンドウメッセージを変換するためにクラス[CWinApp](../../mfc/reference/cwinapp-class.md)によって使用されます。 ( [CWnd::PreTranslateMessage](../../mfc/reference/cwnd-class.md#pretranslatemessage)をオーバーライドします)。|
...
MFC で使用できる拡張ドッキング機能をサポートするペインクラスを作成する場合は、CPane クラスから派生させる必要があり `CBasePane` ます。 [](../../mfc/reference/cpane-class.md)
...
`CBasePane` は、これらのフラグを反映するために、 [Cbasepane:: can、Losed](#canbeclosed)、 [cbasepane::](#canautohide)canの自動非表示、 [Cbasepane:: canfloat](#canfloat)の各フラグを反映するために、次の仮想ブール型メソッドを実装します。 派生クラスでオーバーライドして、その動作をカスタマイズできます。
...
- 自動的に隠すモードのペインをフローティングまたはドッキング解除する場合は、 [Cbasepane:: FloatPane](#floatpane)または[Cbasepane:: UndockPane](#undockpane)を呼び出す前に、 [CDockablePane:: setautohidemode](../../mfc/reference/cdockablepane-class.md#setautohidemode)を最初の引数として FALSE を指定して呼び出す必要があります。

## <a name="example"></a>例

`CBasePane` クラスのさまざまなメソッドの使用方法を次の例に示します。 この例では、クラスからペインを取得する方法 `CFrameWndEx` と、ドッキングモード、ペインの配置、およびペインのスタイルを設定する方法を示します。 このコードは、 [Word Pad サンプル](../../overview/visual-cpp-samples.md)からのものです。
...
これは、ドッキングマネージャーにペインを追加する便利な方法です。 このメソッドを使用すると、親フレームの型を分析するコードを記述する必要はありません。
...
## <a name="cbasepanecanautohide"></a><a name="canautohide"></a> CBasePane:: CanAutoHide に隠す
...
## <a name="cbasepanecanbeclosed"></a><a name="canbeclosed"></a> CBasePane:: Can/Losed
...
## <a name="cbasepanecanberesized"></a><a name="canberesized"></a> CBasePane:: Canのようになります。
...
このメソッドは、に既定で指定されている AFX_CBRS_RESIZE フラグをチェックし `CBasePane::OnCreate` ます。 このフラグが指定されていない場合、ドッキングマネージャーは、ドッキングではなく、ウィンドウに内部的に移動するようにフラグを付けます。
...
CDockablePane クラスなど、特定の派生型のオブジェクトのみを `CBasePane` タブ付きドキュメントに変換できます。 [](../../mfc/reference/cdockablepane-class.md)
...
## <a name="cbasepanecreatedefaultminiframe"></a><a name="createdefaultminiframe"></a> CBasePane:: CreateDefaultMiniframe フレーム
...
クラスのウィンドウを作成し `lpszClassName` ます。 WS_CAPTION を指定した場合、このメソッドは WS_CAPTION スタイルビットをクリアし、 `CBasePane::m_bHasCaption` を TRUE に設定します。これは、ライブラリがキャプション付きのペインをサポートしていないためです。
...
コントロールバーが正常にドッキングされた場合は TRUE。それ以外の場合は FALSE。
...
## <a name="cbasepanedocktoframewindow"></a><a name="docktoframewindow"></a> CBasePane::D Ocktoフレームウィンドウ
...
`EnableDocking` 内部フラグを設定 `CBasePane::m_dwEnabledAlignment` します。フレームワークは、ペインがドッキングされるときにこのフラグをチェックします。
...
`CBasePane` は、このメソッドを実装していません。 したがって、は `CBasePane::get_accHelpTopic` 常に S_FALSE を返します。
...
コントロールバーの現在の配置。 指定可能な値を次の表に示します。
...
## <a name="cbasepanegetdockingmode"></a><a name="getdockingmode"></a> CBasePane:: Getdocによるモード
...
`CBasePane::m_dockMode`が未定義 (DT_UNDEFINED) の場合、ドッキングモードはグローバルドッキングモード () から取得され `AFX_GLOBAL_DATA::m_dockModeGlobal` ます。

*M_dockMode* を設定したり、オーバーライドしたりすることによって、 `GetDockingMode` 各ペインのドッキングモードを制御できます。
...
## <a name="cbasepanegetpaneicon"></a><a name="getpaneicon"></a> CBasePane:: Getpane アイコン
...
`CDockingPanesRow`ペインがドッキングされている場合はへのポインター。フローティングの場合は NULL。

### <a name="remarks"></a>解説

ペインがドッキングされている行にアクセスするには、このメソッドを呼び出します。 たとえば、特定の行のペインを配置するには、を呼び出し、 `GetPaneRow` [CDockingPanesRow:: Arrange epanes](../../mfc/reference/cdockingpanesrow-class.md#arrangepanes)を呼び出します。

## <a name="cbasepanegetpanestyle"></a><a name="getpanestyle"></a> CBasePane:: Getpane Style
...
`GetParentMiniFrame`ペインがフローティングかどうかを判断するには、を使用します。
...
## <a name="cbasepanehideinprintpreviewmode"></a><a name="hideinprintpreviewmode"></a> CBasePane:: Hideinprintプレビューモード
...
複数ペインのフレームウィンドウでフローティングできるのは、ドッキング可能なペインだけです。 したがって、は `CBasePane::IsInFloatingMultiPaneFrameWnd` 常に FALSE を返します。
...
## <a name="cbasepaneisresizable"></a><a name="isresizable"></a> CBasePane:: IsResizable 変更可能
...
このメソッドには、クラスの機能がありません `CBasePane` 。
...
## <a name="cbasepaneonremovefromminiframe"></a><a name="onremovefromminiframe"></a> CBasePane:: OnRemoveFromMiniFrame フレーム
...
フレームワークは、ペインの状態をレジストリに保存するときに、このメソッドを呼び出します。 `SaveState`追加情報を格納するために、派生クラスでをオーバーライドします。
...
## <a name="cbasepanesetdockingmode"></a><a name="setdockingmode"></a> CBasePane:: Setdocのモード
...
初期状態では、ドッキングモードは [CDockingManager:: m_dockModeGlobal](../../mfc/reference/cdockingmanager-class.md#m_dockmodeglobal)によってグローバルに定義されます。 各ペインのドッキングモードは、メソッドを使用して個別に設定でき `SetDockingMode` ます。

## <a name="cbasepanesetpanealignment"></a><a name="setpanealignment"></a> CBasePane:: Setpane Alignment
...
## <a name="cbasepanesetpanestyle"></a><a name="setpanestyle"></a> CBasePane:: Setpane Style
...
*pWndInsertAfter*<br/>
から `CWnd` このオブジェクトの前に来るオブジェクトを `CWnd` Z オーダーで識別します。 詳細については、「 [CWnd:: SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos)」を参照してください。
...
*PWndInsertAfter* が NULL の場合、このメソッドは [CWnd:: SetWindowPos](../../mfc/reference/cwnd-class.md#setwindowpos)を呼び出します。 *PWndInsertAfter* が NULL 以外の場合、このメソッドはを呼び出し `DeferWindowPos` ます。
...
このメソッドは、ペインの表示と非表示を切り替えます。 このメソッドは、の代わりにこのメソッドを使用し `ShowWindow` ます。これは、関連するドッキングマネージャーに、ペインの表示の変更について通知するためです。
