...
カレンダー識別子では、グレゴリオ暦 (ローカライズ版)、日本語、またはイスラム暦など、地域固有の暦を指定します。 カレンダーを含むロケールがコンピューターにインストールされている場合は、`SetCalID` メソッドを使用して、 *calid*パラメーターで指定されたカレンダーを表示します。
...
`SizeMinReq` を呼び出すと、1か月の予定表のカレンダーコントロール全体が正常に表示されます。
