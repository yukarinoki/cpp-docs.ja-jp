...
*pImageList*<br/>
ボタンイメージを既定の状態で表示するために、ツールバーコントロールによって使用されるイメージを格納している `CImageList` オブジェクトへのポインター。
...
MFC での `SetImageList` の実装では、イメージリストをハンドルするのではなく、ツールバーコントロールのボタンイメージを含む `CImageList` オブジェクトを使用します。
...
*pWnd*<br/>
ツールバーコントロールの新しい所有者ウィンドウとなる `CWnd` または `CWnd`派生オブジェクトへのポインター。
...
`SetRows` メンバー関数は、常に最小のツールバーサイズになるレイアウトを選択することにも注意してください。 TBSTYLE_WRAPABLE スタイルを使用してツールバーを作成した後、コントロールのサイズを変更すると、コントロールの幅に合わせて上記のメソッドが適用されます。
