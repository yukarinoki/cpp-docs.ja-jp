...
### <a name="remarks"></a>Remarks  
 四角形の寸法には、外側の境界線上にある任意のサイズ変更ハンドルの幅と高さが含まれます。 *呼び出しから戻ると*は常に正規化されたデバイス座標の四角形。  
...
### <a name="remarks"></a>Remarks  
 WM_SETCURSOR メッセージを処理するウィンドウの関数内からこの関数を呼び出します (通常`OnSetCursor`)。  
