---
title: CComboBox クラス
description: MFC Windows コンボボックスクラスの API リファレンス [CComboBox]
ms.date: 08/27/2020
f1_keywords:
- CComboBox
- AFXWIN/CComboBox
...
*lpszText*\
入出力キューバナーテキストを受け取るバッファーへのポインター。

*cchText*\
から *LpszText* パラメーターが指すバッファーのサイズ。
...
*NIndex*が無効である場合、この関数は例外をスロー `E_INVALIDARG` します (エラーコード: `-2147024809` 、 `0x80070057` )。
...
*lpszText*\
からキューテキストを格納している null で終わるバッファーへのポインター。
