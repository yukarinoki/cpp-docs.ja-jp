...
> [!NOTE]
> 追加のバリアント定数が定義されて、すべてのバリアント型の例外を除いて`VTS_FONT`と`VTS_PICTURE`、variant データ定数へのポインターを提供します。 これらの定数の名前、`VTS_Pconstantname`規則。 たとえば、`VTS_PCOLOR`へのポインター、`VTS_COLOR`定数。
