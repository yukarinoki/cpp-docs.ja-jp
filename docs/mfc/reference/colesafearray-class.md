...
関数は、オブジェクトの VARTYPE を VT_EMPTY に設定して、セーフ配列内のデータをデタッチします。 Windows 関数を呼び出すことによって、配列を解放する呼び出し元の責任[VariantClear](/previous-versions/windows/desktop/api/oleauto/nf-oleauto-variantclear)します。
