...
### <a name="remarks"></a>Remarks  
 場合の値、 [m_dwType](#m_dwtype)データ メンバーは**DBVT_DATE**、 **DBVT_STRING**、または**DBVT_BINARY**、`Clear`メモリ解放共用体ポインター メンバーに関連付けられました。 `Clear` 設定`m_dwType`に**DBVT_NULL**です。  
  
 `CDBVariant`デストラクター呼び出し`Clear`です。  
