...
`Dump`呼び出しは、Microsoft Foundation Class ライブラリのデバッグバージョンでのみ意味があります。 呼び出し、関数宣言、および関数の実装は、 **#ifdef _DEBUG** /  `#endif` 条件付きコンパイルの #ifdef _DEBUG ステートメントで囲む必要があります。
