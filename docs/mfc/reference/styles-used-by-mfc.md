...
対応する MFC オブジェクトを作成するときに、ウィンドウまたはコントロールの外観と動作を指定するには、次のスタイルフラグを使用します。 ほとんどの場合、これらのスタイルはクラス `Create` 関数の*dwStyle*パラメーターで設定されます。

## <a name="button-styles"></a>ボタンスタイル
...
## <a name="combo-box-styles"></a>コンボボックススタイル
...
|ES_OEMCONVERT|エディットコントロールに入力したテキストは、ANSI 文字セットから OEM 文字セットに変換されてから、ANSI に戻ります。 これにより、アプリケーションが `AnsiToOem` Windows 関数を呼び出して、エディットコントロールの ANSI 文字列を OEM 文字に変換するときに、文字変換が適切に行われます。 このスタイルは、ファイル名を含むエディットコントロールに最適です。|
|ES_PASSWORD|エディットコントロールに入力されると、すべての文字がアスタリスク ( __\*__ ) として表示されます。 アプリケーションでは、`SetPasswordChar` メンバー関数を使用して、表示される文字を変更できます。|
...
## <a name="frame-window-styles-mfc"></a>フレームウィンドウスタイル
...
## <a name="list-box-styles"></a>リストボックススタイル
...
|LBS_HASSTRINGS|文字列で構成される項目を含むオーナー描画リストボックスを指定します。 リストボックスには、アプリケーションが `GetText` メンバー関数を使用して特定の項目のテキストを取得できるように、文字列のメモリとポインターが保持されます。|
|LBS_MULTICOLUMN|水平方向にスクロールされる複数列リストボックスを指定します。 `SetColumnWidth` メンバー関数は、列の幅を設定します。|
|LBS_MULTIPLESEL|文字列の選択は、ユーザーが文字列をクリックまたはダブルクリックするたびに切り替わります。 任意の数の文字列を選択できます。|
|LBS_NODATA|データのないリストボックスを指定します。 リストボックス内の項目の数が1000を超える場合に、このスタイルを指定します。 データのないリストボックスにも LBS_OWNERDRAWFIXED スタイルが必要ですが、LBS_SORT または LBS_HASSTRINGS スタイルを持つことはできません。<br/><br/> [データなし] ボックスはオーナー描画リストボックスに似ていますが、アイテムに文字列やビットマップデータが含まれていない点が異なります。 項目を追加、挿入、または削除するコマンドは、指定された項目データを常に無視します。リストボックス内の文字列の検索要求は常に失敗します。 項目を描画する必要があるときに、システムによって [オーナー] ウィンドウに WM_DRAWITEM メッセージが送信されます。 WM_DRAWITEM メッセージと共に渡される `DRAWITEMSTRUCT` 構造体の itemID メンバーは、描画される項目の行番号を指定します。 データのないリストボックスでは、WM_DELETEITEM メッセージは送信されません。|
...
|LBS_USETABSTOPS|リストボックスが文字列を描画するときにタブ文字を認識して展開できるようにします。 既定のタブ位置は、32ダイアログ単位です。 (ダイアログ単位は、水平方向または垂直方向の距離です。 水平ダイアログ単位の1つは、現在のダイアログベースの幅単位の1番目の値に相当します。 ダイアログの基本単位は、現在のシステムフォントの高さと幅に基づいて計算されます。 `GetDialogBaseUnits` Windows 関数は、現在のダイアログベース単位 (ピクセル単位) を返します。このスタイルは、LBS_OWNERDRAWFIXED では使用しないでください。|
...
## <a name="message-box-styles"></a>メッセージボックススタイル

[AfxMessageBox](../../mfc/reference/cstring-formatting-and-message-box-display.md#afxmessagebox) items には、メッセージボックスのスタイルが適用されます。 `AfxMessageBox`の*nType*パラメーターにスタイルの組み合わせを指定します。 Windows でのメッセージボックスのスタイルの詳細については、「 [MessageBox 関数 (windows)](/windows/win32/api/winuser/nf-winuser-messagebox)」を参照してください。
...
## <a name="scroll-bar-styles"></a>スクロールバーのスタイル
...
|SBS_LEFTALIGN|SBS_VERT スタイルと共に使用します。 スクロールバーの左端は、`Create` メンバー関数で指定された四角形の左端に揃えられます。 スクロールバーには、システムのスクロールバーの既定の幅が表示されます。|
|SBS_RIGHTALIGN|SBS_VERT スタイルと共に使用します。 スクロールバーの右端は、`Create` メンバー関数で指定された四角形の右端に合わせて調整されます。 スクロールバーには、システムのスクロールバーの既定の幅が表示されます。|
|SBS_SIZEBOX|サイズボックスを指定します。 SBS_SIZEBOXBOTTOMRIGHTALIGN も SBS_SIZEBOXTOPLEFTALIGN スタイルも指定されていない場合、[サイズ] ボックスには `Create` メンバー関数の高さ、幅、および位置が示されます。|
|SBS_SIZEBOXBOTTOMRIGHTALIGN|SBS_SIZEBOX スタイルと共に使用します。 [サイズ] ボックスの右下隅は、`Create` メンバー関数で指定された四角形の右下隅にあります。 [サイズ] ボックスには、[システムサイズ] ボックスの既定のサイズが設定されています。|
|SBS_SIZEBOXTOPLEFTALIGN|SBS_SIZEBOX スタイルと共に使用します。 [サイズ] ボックスの左上隅は、`Create` メンバー関数で指定された四角形の左上隅に揃えられます。 [サイズ] ボックスには、[システムサイズ] ボックスの既定のサイズが設定されています。|
|SBS_SIZEGRIP|SBS_SIZEBOX と同じですが、エッジが発生しています。|
|SBS_TOPALIGN|SBS_HORZ スタイルと共に使用します。 スクロールバーの上端は、`Create` メンバー関数で指定された四角形の上端に合わせて調整されます。 スクロールバーには、システムのスクロールバーの高さが既定で設定されています。|
|SBS_VERT|垂直スクロールバーを指定します。 SBS_RIGHTALIGN も SBS_LEFTALIGN スタイルも指定されていない場合、`Create` メンバー関数で指定された高さ、幅、および位置がスクロールバーに表示されます。|
...
|SS_ENDELLIPSIS|または SS_PATHELLIPSIS は、必要に応じて、指定された文字列の一部を省略記号に置き換えて、結果が指定した四角形に収まるようにします。 <br /><br />文字列の末尾の文字を置き換えるには SS_END_ELLIPSIS を指定し、文字列の途中で文字を置き換えるには SS_PATHELLIPSIS を指定します。 文字列に円記号 (\\) 文字が含まれている場合、SS_PATHELLIPSIS は、最後の円記号の後にできるだけ多くのテキストを保持します。|
...
## <a name="window-styles"></a>ウィンドウスタイル
...
|WS_POPUP|ポップアップウィンドウを作成します。 WS_CHILD スタイルでは使用できません。|
...
## <a name="extended-window-styles"></a>拡張ウィンドウスタイル
...
|WS_EX_LEFT|ウィンドウの汎用の左揃えプロパティを提供します。 これは既定値です。|
|WS_EX_LEFTSCROLLBAR|クライアント領域の左側に垂直スクロールバーを配置します。|
|WS_EX_LTRREADING|左から右方向の読み取り順序プロパティを使用して、ウィンドウテキストを表示します。 これは既定値です。|
...
|WS_EX_RIGHTSCROLLBAR|クライアント領域の右側に垂直スクロールバーを配置します (存在する場合)。 これは既定値です。|
