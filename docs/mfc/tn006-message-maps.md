---
title: 'TN006: メッセージ マップ |Microsoft Docs'
ms.custom: ''
ms.date: 06/25/2018
ms.technology:
- cpp-mfc
...
[を通じて](https://msdn.microsoft.com/library/windows/desktop/ms644947)関数は、システム全体で一意であることが保証される、新しいウィンドウ メッセージを定義するために使用します。 ON_REGISTERED_MESSAGE マクロは、これらのメッセージを処理するために使用されます。 このマクロの名前を受け入れる、 *UINT 近く*登録済みの windows メッセージの ID を含む変数 次に例を示します。
