...
MFC/OLE1 で`COleClientItem::GetBounds`と`SetBounds`クエリおよび項目の範囲を操作するために使用された (、`left`と`top`メンバーが 0 では常に)。 MFC/OLE 2 でより直接的でサポートされる`COleClientItem::GetExtent`と`SetExtent`、対処する、**サイズ**または`CSize`代わりにします。
...
- ズーム コンテナーに関する true WYSIWYG 動作として使用します。
