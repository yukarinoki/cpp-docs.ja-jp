...
4.  デザイン ビューでアプリケーション ボタンをクリックして、そのプロパティを表示します。 プロパティの値を次のように変更します**イメージ**に*IDB_RIBBON_MAIN*、**プロンプト**に*ファイル*、**キー** 。*f*、 **Large Images**に*IDB_RIBBON_FILELARGE*、および**Small Images**に*IDB_RIBBON_FILESMALL*です。  
  
5.  次の修正を行い、ユーザーがアプリケーション ボタンをクリックしたときに表示されるメニューを作成します。 省略記号ボタン (**.**) の横に**Main Items**を開くには、**項目エディター**です。  
  
    1.  をクリックして**追加**ボタンを追加します。 変更**キャプション**に*と新しい*、 **ID**に*ID_FILE_NEW*、**イメージ**に*0*、**イメージは大き**に*0*します。  
  
    2.  をクリックして**追加**を 2 番目のボタンを追加します。 変更**キャプション**に *& Save*、 **ID**に*ID_FILE_SAVE*、**イメージ**に*2*、および**イメージは大き**に*2*です。  
  
    3.  をクリックして**追加**を 3 番目のボタンを追加します。 変更**キャプション**に*保存 & として*、 **ID**に*ID_FILE_SAVE_AS*、**イメージ**に*3*、および**イメージは大き**に*3*です。  
  
    4.  をクリックして**追加**4 番目のボタンを追加します。 変更**キャプション**に*印刷 &*、 **ID**に*ID_FILE_PRINT*、**イメージ**に*4*、および**イメージは大き**に*4*です。  
  
    5.  変更、**項目**に入力**区切り** をクリックし、**追加**です。  
  
    6.  変更、**項目**型**ボタン**です。 をクリックして**追加**5 番目のボタンを追加します。 変更**キャプション**に*閉じる &*、 **ID**に*ID_FILE_CLOSE*、**イメージ**に*5*、および**イメージは大き**に*5*です。  
  
6.  次の修正を行い、前の手順で作成した [印刷] ボタンにサブメニューを作成します。  
  
    1.  をクリックして、**印刷**ボタンを変更、**項目**に入力**ラベル**、順にクリック**挿入**です。 変更**キャプション**に*プレビューおよびドキュメントを印刷*です。  
  
    2.  をクリックして、**印刷**ボタンを変更、**項目**に入力**ボタン**、 をクリック**挿入**です。 変更**キャプション**に*印刷 &*、 **ID**に*ID_FILE_PRINT*、**イメージ**に*4*、および**イメージは大き**に*4*です。  
  
    3.  クリックして、**印刷**ボタンをクリックし、をクリックして**挿入**ボタンを追加します。 変更**キャプション**に *& クイック印刷*、 **ID**に*ID_FILE_PRINT_DIRECT*、**イメージ**に*7*、および**イメージは大き**に*7*です。  
  
    4.  クリックして、**印刷**ボタンをクリックし、をクリックして**挿入**のボタンを追加します。 変更**キャプション**に*ビュー (&)、印刷前*、 **ID**に*ID_FILE_PRINT_PREVIEW*、**イメージ**に*6*、および**イメージは大き**に*6*です。  
...
7.  次の修正を行い、アプリケーション ボタン メニューの下部に表示される終了ボタンを作成します。  
    ...
    2.  をクリックして**追加**ボタンを追加します。 変更**キャプション**に*E & x*、 **ID**に*ID_APP_EXIT*、**イメージ**に*8*.  
...
1.  この Scribble プログラムに必要なカテゴリは 1 つだけです。 デザイン ビューで、クリックして**カテゴリ**そのプロパティを表示します。 プロパティの値を次のように変更:**キャプション**に *& ホーム*、 **Large Images**に*IDB_RIBBON_HOMELARGE*、 **Small Images**に*IDB_RIBBON_HOMESMALL*です。  
  
2.  各リボン カテゴリは名前付きパネルに整理されます。 各パネルには、関連操作を実行するコントロールのセットが含まれています。 このカテゴリには 1 つのパネルがあります。 をクリックして**パネル**、し、変更**キャプション**に*編集*と**イメージのインデックス**に*0*します。  
  
3.  **編集**パネルで、ドキュメントの内容を消去するのには、ボタンを追加します。 このボタンのメッセージ ID は、IDR_SCRIBBTYPE メニュー リソースに定義済みです。 指定*すべてクリア*ボタンのテキストとボタンを装飾するビットマップのインデックス。 開く、**ツールボックス**、し、ドラッグ、**ボタン**を**編集**パネルです。 ボタンをクリックし、変更**キャプション**に*すべてクリア*、 **ID**に*ID_EDIT_CLEAR_ALL*、**イメージ インデックス**に*0*、**大きいイメージ インデックス**に*0*します。  
