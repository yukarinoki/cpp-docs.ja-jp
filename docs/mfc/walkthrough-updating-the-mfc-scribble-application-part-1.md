...
1. 次の変更は、下のサブメニューを作成、**印刷**前の手順で作成したボタンをクリックします。
   ...
   1. 修正できました、 **Main Items**します。 クリックして**閉じる**を終了する、**項目エディター**します。
...
これで作成した、**アプリケーション**ボタン、リボン要素を追加することができます。
...
1. 各リボン カテゴリは名前付きパネルに整理されます。 各パネルには、関連操作を実行するコントロールのセットが含まれています。 このカテゴリには 1 つのパネルがあります。 クリックして**パネル**、し、変更**キャプション**に`Edit`します。

1. **編集** パネルで、ドキュメントの内容を消去するボタンを追加します。 このボタンは、メッセージ ID は既に定義されて、`IDR_SCRIBBTYPE`メニュー リソース。 指定`Clear All`ボタンのテキストとボタンを装飾するビットマップのインデックス。 開く、**ツールボックス**、し、ドラッグ、**ボタン**を**編集**パネル。 ボタンをクリックし、変更**キャプション**に`Clear All`、 **ID**に`ID_EDIT_CLEAR_ALL`、**イメージ インデックス**に`0`、 **Large Image Index**に`0`します。
...
使用する従来の Scribble 1.0 MFC サンプルを変更して、**リボン デザイナー**します。 次に「[第 2 部](../mfc/walkthrough-updating-the-mfc-scribble-application-part-2.md)します。
