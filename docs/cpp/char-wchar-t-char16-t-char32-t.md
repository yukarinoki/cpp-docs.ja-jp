...
**char** 型は、C および C++ に由来する文字型です。**unsigned char**型はC ++で組み込み型ではないバイトを表現するためによく使用されます。 **char**文字C ++で組み込み型ではないバイトを表現するためによく使用されます。**char**型は、ASCII文字セットやISO-8859文字セットのいずれかの文字、Shift-JISやUnicode文字セットのUTF-8エンコーディングなどのマルチバイト文字の個々のバイトを格納するために使用できます。**char**型の文字列は、マルチ バイト文字のエンコードに使用する場合でも、ナロー文字列と呼ばれます。 Microsoftコンパイラでは、**char** は8ビット型です。
