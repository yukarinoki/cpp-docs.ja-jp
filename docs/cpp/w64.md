...
> [!IMPORTANT]
>  [/Wp64](../build/reference/wp64-detect-64-bit-portability-issues.md)コンパイラ オプションと **_ _w64**キーワードは、Visual Studio 2010 および Visual Studio 2013 では非推奨し、Visual Studio 2013 以降では削除します。 使用する場合、`/Wp64`コマンドライン、コンパイラでコンパイラ オプションは、コマンドラインの警告 D9002 を発行します。 **_ _W64**キーワードが暗黙的に無視されます。 64 ビットの移植性の問題を検出するには、このオプションとキーワードを使用する代わりに、64 ビット プラットフォームが対象の Visual C++ コンパイラを使用してください。 詳細については、[Visual c の 64 ビット x64 構成ターゲット](../build/configuring-programs-for-64-bit-visual-cpp.md)を参照してください。
