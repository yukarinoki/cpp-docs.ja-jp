...
```  
 ...
 ただし、これが関数テンプレートと、コンパイラの型を推測`T`引数から`a`と`b`、通常の関数と同じように呼び出すことができます。  
