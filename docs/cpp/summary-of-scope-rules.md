...
メンバー選択演算子 () の後に指定された名前 **。** または **->** ) は、演算子の左側で指定されたオブジェクトのクラス型のメンバーであるか、その基底クラスのメンバーである必要があります。 メンバー選択演算子 ( **->** ) の右側に指定された名前は、別のクラス型のオブジェクトにすることもできます。これは、 **->** の左側がクラスオブジェクトであり、クラスが他のクラス型へのポインターに評価されるオーバーロードされたメンバー選択演算子 ( **->** ) を定義している場合に使用します。 (このプロビジョニングの詳細については、「[クラスメンバーアクセス](../cpp/member-access.md)」を参照してください)。
...
1. **クラス**、**構造体**、および**共用体**のキーワードを先頭に付ける名前は、**クラス**、**構造体**、または**共用体**の名前のみを検索するようにコンパイラに強制します。

1. スコープ解決演算子の左側にある名前 (`::`) には、**クラス**、**構造体**、**名前空間**、または**共用体**の名前のみを指定できます。

名前が非静的メンバーを参照しているにもかかわらず、静的メンバー関数で使用されている場合は、エラー メッセージが生成されます。 同様に、名前が外側のクラスの非静的メンバーを参照している場合は、囲まれたクラスに**この**ポインターを囲むクラスがないため、エラーメッセージが生成されます。
