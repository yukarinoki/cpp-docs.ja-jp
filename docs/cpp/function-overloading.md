...
組み込みの上位変換または変換が存在しない場合、ユーザー定義の変換が適用されます。 これらの変換は照合する引数の型に基づいて選択されます。 次のコードについて考えてみます。
...
- **`typedef`** 宣言は、新しい型を定義しません。既存の型に対してシノニムを導入します。 オーバーロード機構には影響しません。 次のコードについて考えてみます。
