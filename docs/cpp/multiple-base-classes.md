...
ms.locfileid: "81353735"
---
# <a name="multiple-base-classes"></a>複数の基本クラス

クラスは、複数の基本クラスから派生できます。 多重継承モデル (クラスが複数の基本クラスから派生) では、基本クラスは *、基本リスト*の文法要素を使用して指定されます。 たとえば、`CollectionOfBook` および `Collection` から派生する `Book` のクラス宣言は指定できます。
...
- コンストラクターで初期化が行われる順序。 コードが、`Book` パーツの前で初期化されるために、`CollectionOfBook` の `Collection` 部分に依存している場合、指定の順序が重要になります。 初期化は、クラスが*base-list*で指定された順序で実行されます。

- デストラクターがクリーンアップされるために呼び出される順序。 ここでも、他のパーツの破棄時にクラスの特定の「パーツ」が存在する必要がある場合、順序が重要になります。 デストラクターは *、base-list*で指定されたクラスの逆の順序で呼び出されます。

    > [!NOTE]
    >  基底クラスの指定の順序はクラスのメモリ レイアウトに影響を与える場合があります。 メモリの基本メンバーの順序に基づいて、プログラムの決定を行わないでください。

*ベース リスト*を指定する場合、同じクラス名を複数回指定することはできません。 ただし、間接基底クラスは複数回派生クラスとなることができます。
...
仮想基底クラスを宣言すると、**仮想**キーワードが派生クラスの基本リストに表示されます。
...
![ライン オブジェクト&#45;のシミュレートされたランチ](../cpp/media/vc38xp2.gif "ライン オブジェクト&#45;のシミュレートされたランチ") <br/>
...
**virtual**キーワードを使用すると、サブオブジェクト`Queue`のコピーが 1 つだけ含まれます (次の図を参照)。

![シミュレートされたランチ&#45;ライン オブジェクト、仮想基本クラス](../cpp/media/vc38xp3.gif "シミュレートされたランチ&#45;ライン オブジェクト、仮想基本クラス") <br/>
仮想基底クラスを使用したシミュレートされたランチラインオブジェクト

クラスは、指定された型の仮想コンポーネントと非仮想コンポーネントの両方を持つことができます。 これは、次の図に示されている条件で発生します。

![クラスの仮想および非&#45;仮想コンポーネント](../cpp/media/vc38xp4.gif "クラスの仮想および非&#45;仮想コンポーネント") <br/>
...
![オブジェクトレイアウトの仮想&非&#45;仮想継承](../cpp/media/vc38xp5.gif "オブジェクトレイアウトの仮想&非&#45;仮想継承") <br/>
仮想および非仮想継承を持つオブジェクトレイアウト

> [!NOTE]
> 仮想継承は、非仮想継承と比較してサイズに関して大きな利点があります。 ただし、余分な処理オーバーヘッドが生じる場合があります。

派生クラスが仮想基底クラスから継承する仮想関数をオーバーライドする場合、および派生基底クラスのコンストラクターまたはデストラクターが仮想基底クラスへのポインターを使用してその関数を呼び出す場合、コンパイラは仮想基底クラスを含むクラスに追加の vtordisp 隠しフィールドを導入する場合があります。 コンパイラ`/vd0`オプションは、隠し vtordisp コンストラクター/デストラクターの置き換えメンバーの追加を抑制します。 コンパイラ`/vd1`オプション (デフォルト) では、必要な場所でコンパイラ オプションを使用できます。 すべてのクラス コンストラクターとデストラクターが仮想的に仮想関数を呼び出すことが確実な場合にだけ、vtordisp をオフにしてください。

コンパイラ`/vd`オプションは、コンパイル モジュール全体に影響します。 プラグマ`vtordisp`を使用して、クラスごとにフィールドを抑制`vtordisp`し、再度有効にします。
...
1. 名前へのアクセスがメンバーのアクセス許可に違反する場合、エラー メッセージが生成されます (詳細については、「[メンバーアクセス制御](../cpp/member-access-control-cpp.md)」を参照してください)。
...
- そのオブジェクトに address-of 演算子 (**&**) を適用した場合の効果。 アドレス演算子は、常にオブジェクトのベース アドレスを指定することに注意してください。
