...
配置の新しいフォームを使用する場合、**new**オペレーターは、割り当て、コンパイラのサイズだけでなく、引数を持つフォームの配置形式をサポートしていません、**delete** 演算子場合、コンス トラクターは、例外をスローします。 例:
...
コンパイラが検出した場合、**new**型のオブジェクトを割り当てるオペレーター**型**への呼び出しを発行`type` **:: new 演算子 (sizeof (** `type`**))** またはユーザー定義されていない場合**new 演算子**が定義されている **:: new 演算子 (sizeof (** `type` **))** します。 そのため、**new** 演算子は、オブジェクトの適切なメモリ量を割り当てることができます。
