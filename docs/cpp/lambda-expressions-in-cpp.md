...
例外の指定を使用して、 **`noexcept`** ラムダ式が例外をスローしないことを示すことができます。 通常の関数と同様に、次に示すよう [](../error-messages/compiler-warnings/compiler-warning-level-1-c4297.md)に、ラムダ式で例外の指定を宣言し、 **`noexcept`** ラムダの本体が例外をスローする場合は、Microsoft C++ コンパイラによって警告 C4297 が生成されます。
