...
上記のコードで`multi`、3 次元配列の型は、**double**します。 `p2multi`型の配列を指すポインター**double**の 3 つのサイズ。 この例では、配列が 1 つ、2 つ、および 3 つの添字と共に使用されています。 `cout` ステートメントのように、すべての添字を指定する方が一般的ですが、`cout` に続くステートメントのように、配列要素の特定のサブセットを選択した方が便利なこともあります。
