...
参照型を生成する場合のみ、変換の結果が左辺値になります。 たとえば、`operator int&()` として宣言されたユーザー定義変換は、参照を返し、左辺値です。 ただし、`operator int()` として宣言された変換はオブジェクトを返し、左辺値ではありません。
...
*整数変換*は、整数型の間の変換です。 整数型は、 **char**、 **short** (または**short int**)、 **int**、 **long**、 **long long**です。 これらの型は**符号付き**または**符号なし**で修飾され、unsigned **int**の短縮形として使用すること**ができます**。
...
前の例では、**符号付き short**(`i`) が定義され、負の数に初期化されます。 式 `(u = i)` を指定すると、`u` への代入の前に `i` が**unsigned short**に変換されます。
...
前の例では、`u` は、式 `(i = u)` を評価するために符号付きの数量に変換する必要がある**符号なし短**整数オブジェクトです。 **符号付き**ので値を正しく表すことができないため、データが誤って表示されます。
...
上記の例の最初のステートメントは、2 つの整数型、`iVal` と `ulVal` の乗算を示しています。 満たされる条件は、どちらのオペランドも浮動小数点型ではないことと、1つのオペランドが**符号なし int**型であることです。そのため、もう一方のオペランドである @no__t 0 は、**符号なし int**型に変換されます。その後、結果が `dVal` に割り当てられます。 ここで条件を満たすのは、1つのオペランドが**double**型であるため、乗算の**unsigned int**の結果が**double**型に変換されることです。
