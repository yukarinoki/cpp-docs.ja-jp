...
上記の例の最初のステートメントは、2 つの整数型、`iVal` と `ulVal` の乗算を示しています。 満たされる条件は、どちらのオペランドも浮動小数点型ではないことと、1つのオペランドが**符号なし int**型であることです。そのため、もう一方のオペランドである @no__t 0 は、**符号なし int**型に変換されます。その後、結果が `dVal` に割り当てられます。 ここで条件を満たすのは、1つのオペランドが**double**型であるため、乗算の**符号なし整数**の結果が**double**型に変換されることです。
