...
C++ では、コンパイラは型の既定のコンストラクター、コピー コンストラクター、コピー代入演算子、デストラクターを自動的に生成します (型自体で宣言されていない場合)。 これらの関数は*特殊なメンバー関数*として知られており、C の構造体とC++同様に、単純なユーザー定義型が動作するようになっています。つまり、コードを追加することなく、作成、コピー、および破棄することができます。 C++11 では、移動セマンティクスが、言語に取り込まれた後、移動コンストラクターと移動代入演算子が、コンパイル時に自動生成可能な特殊なメンバー関数のリストに追加されます。
...
また、これらの規則の結果がオブジェクト階層にリークすることがあります。 たとえば、何らかの理由で基底クラスが、派生クラスから呼び出すことができる既定のコンストラクター (つまり、パラメーターを受け取らない**パブリック**または**保護さ**れたコンストラクター) を持つことができない場合、それから派生したクラスは、独自の既定のコンストラクターを自動的に生成できません。
...
前の例では、 **float**引数を使用して `call_with_true_double_only` を呼び出すとコンパイラエラーが発生することに注意してくださいが、 **int**引数を使用して `call_with_true_double_only` を呼び出すことはできません。**int**の場合、引数は**int**から**double**に上位変換され、関数の**double**バージョンを正常に呼び出すことができますが、意図したものではない可能性があります。 double 以外の引数を使用してこの関数を呼び出すとコンパイラ エラーが発生することを確認するには、削除された関数のテンプレート バージョンを宣言できます。
