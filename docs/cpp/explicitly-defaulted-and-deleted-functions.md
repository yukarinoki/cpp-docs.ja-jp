---
description: 詳細については、明示的に既定化および削除された関数
...
C++ では、コンパイラは型の既定のコンストラクター、コピー コンストラクター、コピー代入演算子、デストラクターを自動的に生成します (型自体で宣言されていない場合)。 これらの関数は *特殊なメンバー関数* と呼ばれ、C++ の単純なユーザー定義型を c の構造体のように動作させます。つまり、コードを追加することなく、作成、コピー、および破棄することができます。 C++11 では、移動セマンティクスが、言語に取り込まれた後、移動コンストラクターと移動代入演算子が、コンパイル時に自動生成可能な特殊なメンバー関数のリストに追加されます。
...
また、これらの規則の結果がオブジェクト階層にリークすることがあります。 たとえば、何らかの理由で基底クラスが、派生クラスから呼び出すことができる既定のコンストラクター (つまり、 **`public`** パラメーターを受け取らないまたはコンストラクター) を持つことができない場合、 **`protected`** そのクラスから派生したクラスは、独自の既定のコンストラクターを自動的に生成できません。
...
- コピーコンストラクターは、非表示にするためにプライベートとして宣言する必要がありますが、宣言されているため、既定のコンストラクターの自動生成は禁止されています。 したがって、既定のコンストラクターが必要な場合は、空のコンストラクターであっても何かを明示的に定義する必要があります。
...
特殊なメンバー関数は、クラスの本体の外部にある限り、既定で使用できます。
...
特殊なメンバー関数、通常のメンバー関数、および非メンバー関数を削除して、それらの関数の定義や呼び出しを防止できます。 特殊なメンバー関数を削除することで、不要な特殊なメンバー関数がコンパイラによって生成されるのを防ぐことができます。 そのような関数が宣言されていれば削除する必要があります。ただし、宣言後に既定化されている関数は削除できません。
...
前の例では、引数を使用してを呼び出すと、コンパイラエラーが発生しますが、引数を使用してを呼び出すと、が意図したものではない場合でも、引数は `call_with_true_double_only` **`float`** `call_with_true_double_only` **`int`** **`int`** からに上位変換され、 **`int`** **`double`** **`double`** 関数のバージョンが正常に呼び出されます。 非 double 引数を使用してこの関数を呼び出すとコンパイラエラーが発生するようにするには、削除された関数のテンプレートバージョンを宣言します。
