...
C++ では、コンパイラは型の既定のコンストラクター、コピー コンストラクター、コピー代入演算子、デストラクターを自動的に生成します (型自体で宣言されていない場合)。 これらの関数は*特殊なメンバー関数*と呼ばれ、C++ の単純なユーザー定義型が C の構造体と同じように動作します。つまり、追加のコーディング作業を行わずに、それらを作成、コピー、および破棄できます。 C++11 では、移動セマンティクスが、言語に取り込まれた後、移動コンストラクターと移動代入演算子が、コンパイル時に自動生成可能な特殊なメンバー関数のリストに追加されます。
...
また、これらの規則の結果がオブジェクト階層にリークすることがあります。 たとえば、何らかの理由で、基本クラスが派生クラスから呼び出し可能な既定のコンストラクター (パラメーターを受け取らない**パブリック**コンストラクターまたは**プロテクト**コンストラクター) を持たない場合、そのクラスから派生したクラスは自動的に独自の既定のコンストラクターを生成できません。
...
- コピー コンストラクターは、それを隠すためにプライベートに宣言する必要がありますが、宣言されているため、既定のコンストラクターの自動生成は禁止されます。 したがって、既定のコンストラクターが必要な場合は、空のコンストラクターであっても何かを明示的に定義する必要があります。
...
クラスの本体外で特殊なメンバー関数を既定として使用できる場合は、そのメンバー関数をインライン化できる限り、既定で使用できます。
...
特殊なメンバー関数、通常のメンバー関数、および非メンバー関数を削除して、それらの関数の定義や呼び出しを防止できます。 特殊なメンバー関数を削除すると、コンパイラが不要な特殊なメンバー関数を生成するのを防ぐ方法が提供されます。 そのような関数が宣言されていれば削除する必要があります。ただし、宣言後に既定化されている関数は削除できません。
...
前のサンプルでは **、float**引数`call_with_true_double_only`を使用して呼び出すとコンパイラ エラーが発生します`call_with_true_double_only`が **、int**引数を使用して呼び出すことはしません。**int**の場合、引数は**int**から**double**に昇格され、目的ではない場合でも **、関数**の double バージョンが正常に呼び出されます。 非倍精度引数を使用してこの関数を呼び出すとコンパイラ エラーが発生するように、削除された関数のテンプレート バージョンを宣言できます。
