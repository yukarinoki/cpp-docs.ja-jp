...
description: C++ 言語 dynamic_cast 演算子の概要について説明します。
ms.date: 02/03/2020
f1_keywords:
- dynamic_cast_cpp
helpviewer_keywords:
...
マネージコードでのの動作には、次の2つの重大な変更があり **`dynamic_cast`** ます。

- **`dynamic_cast`** ボックス化された列挙型の基になる型へのポインターへのポインターは、実行時に失敗し、変換されたポインターではなく0を返します。

- **`dynamic_cast`**`type-id`が値型への内部ポインターであり、キャストが実行時に失敗した場合、は例外をスローしなくなります。  キャストは、スローする代わりに、ポインター値 0 を返します。
...
CLR 型の場合、 **`dynamic_cast`** 変換を暗黙的に実行できる場合は no op、変換に失敗した場合はを返す MSIL 命令のいずれかになり `isinst` **`nullptr`** ます。

次の例では、を使用して **`dynamic_cast`** 、クラスが特定の型のインスタンスであるかどうかを判断します。
...
この階層構造では、`A` は仮想基底クラスです。 クラスのインスタンスとサブオブジェクトへのポインターを指定した `E` `A` 場合、 **`dynamic_cast`** へのポインターへのポインター `B` は、あいまいさのために失敗します。 最初に、完全な `E` オブジェクトにキャストし、その後、明確な方法で階層を上へとたどり、正しい `B` オブジェクトに到達する必要があります。

次の図に示すクラスの階層構造の場合を考えてみましょう。

![重複基底クラスを示すクラスの階層構造](../cpp/media/vc39013.gif "重複基底クラスを示すクラスの階層構造") <br/>
重複基底クラスを示すクラスの階層構造

`E` 型のオブジェクトと `D` サブオブジェクトへのポインターがあるとして、`D` サブオブジェクトから左端の `A` サブオブジェクトに移動するには、変換を 3 回行います。 ポインター **`dynamic_cast`** からポインターへの変換、 `D` `E` からへの変換 ( **`dynamic_cast`** または暗黙の変換)、 `E` 最後に `B` からへの暗黙的な変換 `B` `A` を実行できます。 次に例を示します。
...
演算子は、 **`dynamic_cast`** "クロスキャスト" を実行するためにも使用できます。 同じクラス階層を使用すると、完全なオブジェクトが `B` 型であれば、たとえば `D` サブオブジェクトから `E` サブオブジェクトへのように、ポインターをキャストできます。
...
Null ポインター値は、によって変換先の型の null ポインター値に変換され **`dynamic_cast`** ます。
...
例外の詳細については、「 [typeid](../cpp/typeid-operator.md) 」を参照してください `__non_rtti_object` 。
