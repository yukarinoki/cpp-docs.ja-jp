...
参照宣言を [アドレス演算子](../cpp/address-of-operator-amp.md)の使用と混同しないようにしてください。 識別子の前にやなどの型がある場合は、 `&`  **`int`** **`char`** *識別子* が型への参照として宣言されます。 識別子の前に型がない場合は、 `&` アドレス演算子の使用法が使用されます。
