...
ms.locfileid: "97268926"
---
# <a name="functions-c"></a>関数 (C++)

関数とは、何らかの操作を実行するコードのブロックです。 関数には、呼び出し元が関数に引数を渡すのに使用する入力パラメーターを必要に応じて定義できます。 関数は、必要に応じて値を出力として返すことができます。 関数は、一般的な操作を 1 つの再利用可能なブロックでカプセル化するのに役立ちます。関数には、その動作を明確に説明する名前を付けることが理想的です。 次の関数は、呼び出し元から2つの整数を受け取り、その合計を返します。 *a* と *b* は、型の *パラメーター* です **`int`** 。
...
1. 関数が返す値の型を指定する戻り値の型 **`void`** 。値が返されない場合は。 C++ 11 で **`auto`** は、は、return ステートメントから型を推論するようにコンパイラに指示する有効な戻り値の型です。 C++ 14 で `decltype(auto)` は、も使用できます。 詳細については、以下の「戻り値の型の型推論」を参照してください。
...
1. **`constexpr`** は、関数の戻り値がコンパイル時に定数値として計算できることを示します。
...
1. **`inline`**。関数のすべての呼び出しを関数コード自体に置き換えるようコンパイラに指示します。 パフォーマンスが重要なコード セクションで関数が迅速に実行され、繰り返し呼び出されるシナリオでは、インライン展開を使用すると、パフォーマンスが向上します。
...
1. **`noexcept`** 関数が例外をスローできるかどうかを指定する式。 次の例では、式がに評価される場合、関数は例外をスローしません `is_pod` **`true`** 。
...
1. (メンバー関数のみ)Cv 修飾子。関数がまたはのどちらであるかを指定し **`const`** **`volatile`** ます。

1. (メンバー関数のみ) **`virtual`** 、 **`override`** 、または **`final`** 。 **`virtual`** 派生クラスで関数をオーバーライドできることを指定します。 **`override`** は、派生クラスの関数が仮想関数をオーバーライドすることを意味します。 **`final`** は、関数を他の派生クラスでオーバーライドできないことを意味します。 詳細については、「 [仮想関数](../cpp/virtual-functions.md)」を参照してください。

1. (メンバー関数のみ) **`static`** メンバー関数に適用された場合、その関数はクラスのオブジェクトインスタンスに関連付けられていません。

1. (非静的メンバー関数のみ)Ref 修飾子。これは、暗黙的なオブジェクトパラメーター () が右辺値参照と左辺値参照のどちらであるかを選択する関数のオーバーロードをコンパイラに指定します `*this` 。 詳細については、「 [関数のオーバーロード](function-overloading.md#ref-qualifiers)」を参照してください。
...
メンバー関数をとして宣言すると、その **`const`** 関数がクラスのデータメンバーの値を変更できないように指定できます。 メンバー関数をとして宣言 **`const`** すると、コンパイラは *const の正確性* を強制的に適用できます。 として宣言された関数を使用してオブジェクトを誤って変更しようとすると **`const`** 、コンパイラエラーが発生します。 詳細については、「 [const](const-cpp.md)」を参照してください。

**`constexpr`** 生成された値がコンパイル時に決定される可能性がある関数をとして宣言します。 通常、constexpr 関数は通常の関数よりも高速に実行されます。 詳細については、「[`constexpr`](constexpr-cpp.md)」を参照してください。
...
パラメーター宣言リスト内の単一のキーワードで宣言された関数は、 **`void`** キーワード **`void`** が引数宣言リストの最初の唯一のメンバーである限り、引数を受け取りません。 リスト内の他の場所にある型の引数は、 **`void`** エラーを生成します。 次に例を示します。
...
ここに記載されている以外は引数を指定することはできませんが、 **`void`** 型から派生した型 **`void`** ( **`void`** およびの配列へのポインターなど) は、 **`void`** 引数宣言リストの任意の場所に置くことができます。
...
関数は、別の関数、または組み込み配列を返すことはできません。ただし、これらの型へのポインター、または関数オブジェクトを生成する *ラムダ* を返すことができます。 このような場合を除き、関数はスコープ内の任意の型の値を返すことも、値を返さないこともあります。この場合、戻り値の型は **`void`** です。

### <a name="trailing-return-types"></a>後続の戻り値の型

"通常" の戻り値の型は、関数のシグネチャの左側にあります。 *後続の戻り値の型* はシグネチャの右端に配置され、その前に演算子が付き **`->`** ます。 後続の戻り値の型は、関数テンプレートにおいて、戻り値の種類がテンプレート パラメーターに依存する場合、特に便利です。
...
C++ 14 では、を使用して、 **`auto`** 後続の戻り値の型を指定することなく、関数本体から戻り値の型を推論するようにコンパイラに指示できます。 **`auto`** 推測は常に値渡しで返されることに注意してください。 `auto&&` を使用すると、参照を推測するようにコンパイラに指示できます。

この例で **`auto`** は、は、lhs と rhs の合計の非定数値コピーとして推測されます。
...
で **`auto`** は、推測型の const 性が維持されないことに注意してください。 戻り値が引数の定数性または参照性を維持する必要がある転送関数の場合は、キーワードを使用でき **`decltype(auto)`** ます。この場合、 **`decltype`** 型推論規則を使用し、すべての型情報を保持します。 **`decltype(auto)`** は、左辺の通常の戻り値として、または後続の戻り値として使用できます。

次の例 ( [N3493](https://wg21.link/n3493)からのコードに基づく) は、 **`decltype(auto)`** テンプレートがインスタンス化されるまでわからない戻り値の型の関数の引数を完全に転送できるようにするために使用されています。
...
1. **Visual Studio 2017 バージョン15.3 以降** (で使用可能 [`/std:c++17`](../build/reference/std-specify-language-standard-version.md) ): 構造化バインドを使用します。 構造化バインディングの利点は、戻り値を格納する変数が、宣言されたときと同時に初期化されることです。これは、場合によっては効率が大幅に向上することがあります。 ステートメントでは `auto[x, y, z] = f();` 、かっこによって、関数ブロック全体のスコープ内にある名前が導入され、初期化されます。
...
関数 **`typedef`** ポインター型を返す関数を宣言する場合は、関数ポインター型のエイリアスを宣言するために使用することをお勧めします。  次に例を示します。
...
上記の宣言は、上記を使用した宣言と同じです **`typedef`** 。

## <a name="see-also"></a>関連項目

[関数のオーバーロード](../cpp/function-overloading.md)<br/>
[可変個引数リストを持つ関数](../cpp/functions-with-variable-argument-lists-cpp.md)<br/>
[明示的に既定化および削除された関数](../cpp/explicitly-defaulted-and-deleted-functions.md)<br/>
[関数の引数依存名 (Koenig) の参照](../cpp/argument-dependent-name-koenig-lookup-on-functions.md)<br/>
