...
符号なしの型を指定するには、`u` または `U` のいずれかのサフィックスを使用します。 長い型を指定するには、`l` または `L` のいずれかのサフィックスを使用します。 64 ビットの整数型を指定するには、サフィックスとして LL、ll を使用します。 i64 サフィックスはまだサポートされていますが、Microsoft に固有なものであり移植性がないため、使用を避けることをお勧めします。 次に例を示します。
...
浮動小数点リテラルは、既定で**double**型になります。 サフィックスとして `f` または `l` (または `F` または `L` を使用します。サフィックスの大文字と小文字は区別されません)、リテラルはそれぞれ**float**または**long double**として指定できます。

**Long double**と**double**の表現は同じですが、同じ型ではありません。 たとえば、次のようにオーバーロードされた関数を指定できます。
