...
ms.locfileid: "67894470"
---
# <a name="unions"></a>Unions

> [!NOTE]
> C++ 17 以降では、 **std::variant**クラスは、共用体のタイプ セーフな代替手段。

A**共用体**はすべてのメンバーが同じメモリ位置を共有するユーザー定義型です。 これは、任意のどの時点においても、共用体のメンバー一覧にあるオブジェクトがただ 1 つだけ共用体に含まれていることを意味します。 また、共用体のメンバー数には関係なく、最大のメンバーを格納するのに必要なメモリーだけが常に使用されることも意味します。
...
共用体の宣言を開始、**共用体**キーワード、およびメンバーのリストを中かっこで囲みます。
...
前の例では、共用体にアクセスするすべてのコードが、データを保持しているメンバーを知る必要がありました。 この問題の最も一般的な解決策は、共用体を構造体で囲み、共用体に現在格納されているデータの種類を示す列挙メンバーを同じ構造体に追加することです。 これと呼ばれますが、*判別共用体*し、次の例は、基本的なパターンを示しています。
...
![数値型共用体のデータ ストレージ](../cpp/media/vc38ul1.png "の数値型共用体データ ストレージ") <br/>
数値型共用体のデータ ストレージ

## <a name="anonymous_unions"></a> 無名共用体

無名共用体はなしで宣言される共用体、*クラス名*または*宣言リスト*します。
...
名前付きの共用体の制限事項、に加えて、無名共用体はような追加の制限は。

- として宣言する必要がありますも**静的**ファイルまたは名前空間のスコープで宣言されている場合。

- のみを持つこと**パブリック**メンバー。**プライベート**と**保護**無名共用体のメンバーは、エラーを生成します。
