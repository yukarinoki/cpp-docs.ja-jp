...
1. 宣言指定子:
   ...
   - 省略可能な**const**または**volatile**指されるオブジェクトの型に適用するキーワード。
...
1. 宣言子:
   ...
   - 省略可能な**const**または**volatile**ポインター自体に適用するキーワード。
...
ポインターの p と q が型のオブジェクトへのポイントに前の例では、 **int**されそれぞれ i と j のアドレスに初期化されます。  ストレージ クラス指定子**static**両方のポインターに適用されます。
