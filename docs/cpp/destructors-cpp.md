...
デストラクターは、オブジェクトがスコープ外に出たとき、またはの呼び出しによって明示的に破棄されたときに自動的に呼び出されるメンバー関数です **`delete`** 。 デストラクターには、クラスと同じ名前が付けられます。その前に、チルダ () が付き `~` ます。 たとえば、クラス `String` のデストラクターを宣言するには、`~String()` とします。
...
前の例では、デストラクターは演算子を使用して、 `String::~String` **`delete`** テキストストレージに対して動的に割り当てられた領域の割り当てを解除します。
...
- は **`const`** 、、 **`volatile`** 、またはとして宣言できません **`static`** 。 ただし、、、またはとして宣言されたオブジェクトを破棄するために呼び出すことができ **`const`** **`volatile`** **`static`** ます。

- はとして宣言でき **`virtual`** ます。 仮想デストラクターを使用すると、型を知らなくてもオブジェクトを破棄できます。オブジェクトの正しいデストラクターが、仮想関数のメカニズムを使用して呼び出されます。 デストラクターは、抽象クラスの純粋仮想関数として宣言することもできます。
...
- 演算子を使用して割り当てられたオブジェクト **`new`** は、を使用して明示的に解放され **`delete`** ます。
...
デストラクターを明示的に呼び出す必要はほとんどありません。 ただし、絶対アドレスにあるオブジェクトのクリーンアップを実行すると便利な場合があります。 これらのオブジェクトは、通常、配置引数を受け取るユーザー定義の演算子を使用して割り当てられ **`new`** ます。 **`delete`** このメモリは解放ストアから割り当てられていないため、このメモリの割り当てを解除できません (詳細については、「 [new および delete 演算子](../cpp/new-and-delete-operators.md)」を参照してください)。 ただし、デストラクターへの呼び出しは適切なクリーンアップを実行できます。 オブジェクトのデストラクターを明示的に呼び出すには、`s` (`String` クラス) で次のいずれかのステートメントを使用します。
...
次の例では、暗黙的に生成されたコピーコンストラクターがポインターを作成し、 `str1.text` `str2.text` 同じメモリを参照します。から戻ると、 `copy_strings()` そのメモリは2回削除されます。これは未定義の動作です。
...
[コピーコンストラクターとコピー代入演算子](../cpp/copy-constructors-and-copy-assignment-operators-cpp.md)</br>
