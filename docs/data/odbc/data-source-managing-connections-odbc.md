...
2.  **ファイル DSN** ] タブで [**追加**します。  
  
3.  **データ ソースの新規作成** ダイアログ ボックスでは、Excel ドライバーを選択し、クリックして**次**。  
  
4.  クリックして**参照**、データ ソースとして使用するファイルの名前を選択します。  
...
2.  呼び出すその`OpenEx`または`Open`メンバー関数。  
...
2.  呼び出すオブジェクトを破棄せずに、その`OpenEx`または`Open`メンバー関数をもう一度です。  
