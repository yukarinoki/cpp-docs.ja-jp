...
各クラスを開く行では、プロバイダーの各 COM オブジェクトを作成します。 プロバイダを探しを使用して、`ProgID`のプロバイダー。 取得することができます、`ProgID`システム レジストリからや MyProvider.rgs ファイル内の検索 (プロバイダーのディレクトリを開き、検索、`ProgID`キー)。  
