---
title: マイクロソフト マクロ アセンブラ BNF 文法
description: x64 の MASM の BNF 記述。
ms.date: 12/17/2019
helpviewer_keywords:
...
ms.locfileid: "81317975"
---
# <a name="microsoft-macro-assembler-bnf-grammar"></a>マイクロソフト マクロ アセンブラ BNF 文法

このページには、MASM 文法の BNF 記述が含まれています。 これは、参照の補足として提供され、完全であることが保証されていません. キーワード、パラメーター、操作などの詳細については、リファレンスを参照してください。

BNF の使用を説明するために、次の図は、非終端*`typedefDir`* から始まる TYPEDEF ディレクティブの定義を示しています。

![マスム BNF の例](media/bnf.png)

各水平中かっこの下の**`NEAR16`** エントリは、**`NEAR32`****`FAR16`** ターミナルです。 **`FAR32`** *`qualifier`* または、それらは、 、 などの*`qualifiedType`**`distance`* 非終端であり、*`protoSpec`* さらに定義することができます。 *`typedefDir`* 定義内の各斜体非終端は、BNF のエントリでもあります。 3 つの垂直ドットは、非終端の分岐定義を示しますが、これはわかりやすくするために、この図では説明しません。

BNF 文法では、再帰的な定義が可能です。 たとえば、文法では qualifiedType の定義として qualifiedType を使用します。 "|" 記号は、代替式の間の選択肢を*`endOfLine`* | *`comment`* 指定します。 二重中かっこは、省略可能なパラメータを*`macroParmList`* 指定します( など) 括弧は実際にはソースコードに現れることはありません。

## <a name="masm-nonterminals"></a>MASM 非ターミナル
...
*`character`*\
&nbsp;&nbsp;&nbsp;&nbsp;改行 (10) を除く、0 から 255 の範囲の序数を持つ任意の文字。
...
*`id`*\
&nbsp;&nbsp;&nbsp;&nbsp;識別子の最初の文字は、大文字または小文字の英字 (`[A–Za-z]`) またはこれらの 4 つの文字のいずれか`@ _ $ ?`です: 残りの文字は、同じ文字または 10`[0–9]`進数の数字 ( ) のいずれかになります。 最大長は 247 文字です。
...
*`text`*\
&nbsp;&nbsp;&nbsp;&nbsp;*`textLiteral`* | *`text`* 文字 |**`!`** *`character`* *`text`* | *`character`* | **`!`***`character`*
