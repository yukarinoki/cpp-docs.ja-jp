...
>
> 右クリックを使用することも、**リソース ビュー**ウィンドウに、コマンドのショートカット メニューを起動またはタイトル バーをドッキングするか、ウィンドウをドッキング解除 をダブルクリックします。 ウィンドウの動作を制御するための他のコマンドは、タイトル バーを右クリックします。 詳細については、次を参照してください。 [Windows 管理](/visualstudio/ide/customizing-window-layouts-in-visual-studio)します。
...
**リソースの追加** ダイアログ ボックスが含まれています、**新規カスタム リソース**C++ プロジェクトで新しいカスタム リソースを作成する以下のプロパティ ダイアログ ボックス。
...
## <a name="resource-scripts"></a>リソース スクリプト
...
1. 選択、 **Visual C**フォルダーを選択し、**リソース ファイル (.rc)** 右側のウィンドウで。

1. リソース スクリプトの名前を指定、**名前**テキスト ボックスを選択します**オープン**します。

### <a name="to-open-a-resource-script"></a>リソース スクリプトを開く

リソース スクリプトでリソースを表示するには、プロジェクトを開く必要はありません。 スクリプト ファイルではなく、ドキュメント ウィンドウに開きます、**リソース ビュー**ウィンドウ。

> [!NOTE]
> いくつかのコマンドは、ファイルが開かれているスタンドアロン (プロジェクト) の外側でのみ使用できます。 スタンドアロンのファイルを開くと、最初にプロジェクトを読み込まずに開くことを意味します。
>
> たとえば、別の形式で、または別のファイル名としてファイルを保存する (として、**付けて**コマンドは、プロジェクト内のファイルを使用できません)。 最初を使用してプロジェクトを開く場合**ファイル** > **開く** > **プロジェクト**、これらのコマンドは使用できません。

#### <a name="to-open-a-resource-script-outside-of-a-project"></a>プロジェクトの外側でリソース スクリプトを開く

1. **ファイル**メニューの **オープン**を選択し、**ファイル**します。

1. リソース スクリプト ファイルに移動し、ファイルを強調表示し、選択**オープン**します。

#### <a name="to-open-multiple-resource-scripts-outside-of-a-project"></a>プロジェクトの外側で複数のリソース スクリプトを開く

1. 2 つのリソース ファイルをスタンドアロンで開きます。 たとえば、開く*Source1.rc*と*Source2.rc*します。

1. **ファイル**] メニューの [選択**オープン**を選択し、**ファイル**します。

1. 開きたい最初のリソース スクリプト ファイルに移動します (*Source1.rc*)、ファイルを強調表示し、選択、**開く**します。

1. 2 番目の .rc ファイルを開くには、前の手順を繰り返します (*Source2.rc*)。

   別個のドキュメント ウィンドウでは両方の .rc ファイルを開くようになりました。

1. 使用して、**ウィンドウ**メニュー (または、.rc ファイルの 1 つを右クリック) 選択**水平タブ グループの新しい**または**垂直タブ グループ**します。
...
1. 選択**プログラムから開く**クリックし、ドロップダウン メニューから**ソース コード (テキスト) エディター**します。
...
> [!TIP]
> ここにショートカットがの .rc ファイルを右クリックし、**ソリューション エクスプ ローラー**、選択**プログラムから開く.** 選択**ソース コード (テキスト) エディター**します。
...
- **プロジェクト**メニューの **リソースの追加**します。

### <a name="support-for-mfc"></a>MFC のサポート

通常、Windows を使用するための Microsoft Foundation Class (MFC) アプリケーションをビルドするかどうか、 [MFC アプリケーション ウィザード](../mfc/reference/mfc-application-wizard.md)ウィザードのコア機能を含むファイル (リソース スクリプト (.rc) ファイルを含む) の基本的なセットを生成する、MFC します。 ただし、MFC ベースでない Windows アプリケーション用の .rc ファイルを編集するには、次の MFC 固有機能は利用できません。MFC コード ウィザード、メニュー プロンプト文字列は、コンボ ボックス コントロール、および ActiveX コントロールをホストしている内容を一覧表示します。

#### <a name="to-add-mfc-support-to-a-resource-script-file"></a>リソース スクリプト ファイルに MFC サポートを追加するには
...
## <a name="resource-templates"></a>リソース テンプレート
...
1. プラス記号を選択します (**+**) リソース ノードを展開し、そのリソースの使用可能なすべてのテンプレートを表示するリソースの横にあります。
