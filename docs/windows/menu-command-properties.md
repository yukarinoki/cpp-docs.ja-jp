...
- メニューコマンドにアクセス (ショートカット) キーを割り当てるには、 `&` メニュー名またはコマンド名の文字の前にアンパサンド () を入力し、対応するアクセスキーとしてその文字を指定します。
   ...
   たとえば、"&ファイル" は、 **Alt** + Microsoft Windows 用に作成されたアプリケーションの [**ファイル**] メニューのショートカットキーとして Alt**F**を設定します。
