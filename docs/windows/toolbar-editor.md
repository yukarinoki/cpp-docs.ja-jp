...
1. **[リソースの追加]** ダイアログボックスで、 **[リソースの種類]** ボックスの一覧の **[ツールバー]** を選択し、 **[新規作成]** を選択します。
   ...
   **ツールバー**のリソースの **+** 種類の横にプラス記号 () が表示されている場合は、ツールバーテンプレートが使用可能であることを意味します。 プラス記号を選択してテンプレートの一覧を展開し、テンプレートを選択して **[新規作成]** を選択します。
...
1. **ツールバー**のフォルダーを展開し、編集するツールバーを選択して、次のいずれかの操作を行います。
   ...
   - ツールバー**ビュー**ペインのツールバーの右端にある空白のボタンを選択し、描画を開始します。 既定のボタンコマンド ID が割り当てられ\<ます (ID_BUTTON n >)。
...
1. マウスポインターを **[印刷]** ツールバーボタンの上に移動`Print`すると、マウスポインターの下に "現在はフローティング" と表示されます。

1. **ワードパッド**ウィンドウの下部にあるステータスバーを見て、テキスト`Prints the active document`が表示されていることを確認します。

`Print`は、ツールヒントの名前`Prints the active document`です。は、ステータスバーのボタンの説明です。
...
[のメニューとその他のリソース](/windows/win32/menurc/resources)<br/>
