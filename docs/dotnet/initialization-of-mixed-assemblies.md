...
1. mscoree.dll と mscorwks.dll のシンボルを使用できるようにします。
   ...
   2 つ目の方法として、mscoree.dll と mscorwks.dll の PDB を Microsoft Symbol Server からダウンロードします。 Symbol Server を構成するには、シンボル検索パスのオプションのダイアログ ボックスを開きます。 (から、**ツール**] メニューの [選択**オプション**です。 左側のウィンドウで、**オプション** ダイアログ ボックスで、**デバッグ**ノードを選択して**シンボル**)。次の検索パスを検索リストに追加:http://msdl.microsoft.com/download/symbolsです。 シンボルのキャッシュ ディレクトリをシンボル サーバーのキャッシュのテキスト ボックスに追加します。 **[OK]** をクリックします。
