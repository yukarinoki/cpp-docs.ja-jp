...
3番目のメンバー関数は、where でポイントされた `first` `last` 被制御シーケンス*right*内の要素の直前に、[,) によって指定されたサブ*範囲*を挿入します。 また、 *right*によって制御されるシーケンスから元のサブ範囲も削除されます。 (の場合 `right == this` 、範囲 [,) には、が指す要素を含めることはでき `first` `last` ません。 *where*このメソッドを使用して、0個以上の要素のサブシーケンスを1つのリストから別のリストにスプライスします。
