...
このメンバー関数は、実質的に[hash_map:: erase (stl](#erase) /clr) hash_map:: begin (stl/clr) `(` [hash_map::begin (STL/CLR)](#begin) `(),` [hash_map:: end (stl/clr)](#end) `())` を呼び出します。 このメソッドを使用して、被制御シーケンスが空であることを確認します。
