---
description: 詳細については、「hash_multiset (STL/CLR)」を参照してください。
...
このメンバー関数は、実質的に[hash_multiset:: erase (stl](#erase) /clr) hash_multiset:: begin (stl/clr) `(` [](#begin) `(),` [hash_multiset:: end (stl/clr)](#end) `())` を呼び出します。 このメソッドを使用して、被制御シーケンスが空であることを確認します。
