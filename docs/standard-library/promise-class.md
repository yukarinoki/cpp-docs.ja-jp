...
|名前|説明|
|----------|-----------------|
|[とっ](#promise)|`promise` オブジェクトを構築します。|
...
## <a name="promisepromise-constructor"></a><a name="promise"></a> promise::p romise コンストラクター
...
*ウムアルクラ*\
メモリ割り当て。 詳細については、「」を参照してください [\<allocators>](../standard-library/allocators-header.md) 。
...
2番目のコンストラクターは、空 `promise` のオブジェクトを構築し、メモリの割り当てに *Al* を使用します。

3番目のコンストラクターは、オブジェクトを構築 `promise` し、関連付けられている非同期状態を *他* のから転送し、 *他の* 空のままにします。
...
[Set_exception](#set_exception)、 `set_exception_at_thread_exit` 、 [set_value](#set_value)、または[set_value_at_thread_exit](#set_value_at_thread_exit)が同じ関連付けられた非同期状態のオブジェクトに対して既に呼び出されている場合 `promise` 、このメソッドは `future_error` のエラーコードを持つをスローし `promise_already_satisfied` ます。
...
部分的特殊化で `promise<Ty&>` は、格納された値は *Val* への参照になります。
