...
ms.locfileid: "97167293"
---
# <a name="time_get-class"></a>time_get クラス

クラステンプレートは、型のシーケンスから時刻値への変換を制御するためにロケールファセットとして使用できるオブジェクトを表し `CharType` ます。
...
変換されない最初の要素を指定する反復子を返します。 変換に失敗すると、でが設定さ `ios_base::failbit` れ、最初にが `state` 返されます。 

### <a name="remarks"></a>解説

この仮想メンバー関数は、範囲 [,) 内の1つ以上の入力要素を変換およびスキップし、 `first` `last` の1つ以上のメンバーに格納されている値を確認し `*pt` ます。 変換に失敗すると、でが設定さ `ios_base::failbit` れ、最初にが `state` 返されます。  それ以外の場合、この関数は変換されていないの最初の要素を示す反復子を返します。
...
プロテクト仮想メンバー関数は、シーケンス [ `first`, `last`) の先頭から始め、空でない完全な日付入力フィールドを認識するまで、連続した要素との一致を試みます。 成功した場合、このフィールドをコンポーネント **tm:: tm \_ mon**、 **tm:: tm \_ day**、および **tm:: tm \_ year** と同等の値に変換し、結果をそれぞれ、、およびに格納し `ptm->tm_mon` `ptm->tm_day` `ptm->tm_year` ます。 これは、日付入力フィールドを超える先頭の要素を示す反復子を返します。 それ以外の場合、関数は `iosbase::failbit` *状態* をに設定します。 これは、有効な日付入力フィールドのプレフィックスを超える先頭の要素を示す反復子を返します。 どちらの場合も、戻り値が *last* と等しい場合、関数 `ios_base::eofbit` は *状態* をに設定します。
...
プロテクト仮想メンバー関数は、シーケンス [ `first`, `last`) の先頭から始め、空でない完全な月入力フィールドを認識するまで、連続した要素との一致を試みます。 成功した場合、このフィールドをコンポーネント **tm:: tm \_ mon** と同等の値に変換し、結果をに格納し `ptm->tm_mon` ます。 これは、月入力フィールドを超える先頭の要素を示す反復子を返します。 それ以外の場合、関数は `ios_base::failbit` *状態* をに設定します。 これは、有効な月入力フィールドのプレフィックスを超える先頭の要素を示す反復子を返します。 どちらの場合も、戻り値が *last* と等しい場合、関数 `ios_base::eofbit` は *状態* をに設定します。
...
の *X* 指定子によって生成される日付として文字列を解析するために呼び出されるプロテクト仮想メンバー関数 `strftime` 。
...
プロテクト仮想メンバー関数は、シーケンス [ `first`, `last`) の先頭から始め、空でない完全な時刻入力フィールドを認識するまで、連続した要素との一致を試みます。 成功した場合は、このフィールドをコンポーネント、、およびと同等の値に変換 `tm::tm_hour` `tm::tm_min` し、 `tm::tm_sec` それぞれ、、およびに結果を格納し `ptm->tm_hour` `ptm->tm_min` `ptm->tm_sec` ます。 これは、時刻入力フィールドを超える先頭の要素を示す反復子を返します。 それ以外の場合、関数は `ios_base::failbit` *状態* をに設定します。 これは、有効な時刻入力フィールドのプレフィックスを超える先頭の要素を示す反復子を返します。 どちらの場合も、戻り値が *last* と等しい場合、関数 `ios_base::eofbit` は *状態* をに設定します。
...
仮想プロテクトメンバー関数は、シーケンス [,) の *先頭* から始まり `first` `last` 、空でない完全な曜日入力フィールドを認識するまで、連続した要素との照合を試みます。 成功した場合、このフィールドをコンポーネント **tm:: tm \_ wday** と同等の値に変換し、結果をに格納し `ptm->tm_wday` ます。 これは、曜日入力フィールドを超える先頭の要素を示す反復子を返します。 それ以外の場合、関数は `ios_base::failbit` *状態* をに設定します。 これは、有効な曜日入力フィールドのプレフィックスを超える先頭の要素を示す反復子を返します。 どちらの場合も、戻り値が *last* と等しい場合、関数 `ios_base::eofbit` は *状態* をに設定します。
...
仮想プロテクトメンバー関数は、シーケンス [,) の *先頭* から `first` 空で `last` ない完全な年入力フィールドを認識するまで、連続した要素との照合を試みます。 成功した場合、このフィールドをコンポーネント **tm:: tm \_ year** と同等の値に変換し、結果をに格納し `ptm->tm_year` ます。 これは、年入力フィールドを超える先頭の要素を示す反復子を返します。 それ以外の場合、関数は `ios_base::failbit` *状態* をに設定します。 これは、有効な年入力フィールドのプレフィックスを超える先頭の要素を示す反復子を返します。 どちらの場合も、戻り値が *last* と等しい場合、関数 `ios_base::eofbit` は *状態* をに設定します。
...
時間構造体の割り当てに使用されたデータの後の最初の文字を指す反復子を返し `*ptm` ます。
...
1セント (%)形式で、次に set [EOQ #] 内の省略可能な修飾子 *mod* 、その後に変換指定子の *fmt* が、 *最初* にによって返される値に置き換えられ `do_get(first, last, iosbase, state, ptm, fmt, mod)` ます。 変換エラーにより `ios_base::failbit` *状態* がに設定され、が返されます。
...
このメンバー関数は、 [do_get_date](#do_get_date)(、、、 `first` 、) を返し `last` `iosbase` `state` `ptm` ます。
...
このメンバー関数は、 [do_get_monthname](#do_get_monthname)(、、、 `first` 、) を返し `last` `iosbase` `state` `ptm` ます。
...
の *X* 指定子によって生成される日付として文字列を解析し `strftime` ます。
...
このメンバー関数は、 [do_get_time](#do_get_time)(、、、 `first` 、) を返し `last` `iosbase` `state` `ptm` ます。
...
このメンバー関数は、 [do_get_weekday](#do_get_weekday)(、、、 `first` 、) を返し `last` `iosbase` `state` `ptm` ます。
...
このメンバー関数は、 [do_get_year](#do_get_year)(、、、 `first` 、) を返し `last` `iosbase` `state` `ptm` ます。
...
コンストラクターは、 **locale::**[facet](../standard-library/locale-class.md#facet_class)() を使用して、その基本オブジェクトを初期化し `refs` ます。
