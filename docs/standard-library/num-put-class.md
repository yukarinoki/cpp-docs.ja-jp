...
- **Iosbase. flags** の場合。 [](../standard-library/ios-base-class.md#flags)  &  `ios_base::basefield` フラグ  ==  `ios_base::`[10 月](../standard-library/ios-functions.md#oct)の変換仕様は `lo` です。
...
- **Iosbase. flags** の場合。   &  `ios_base::adjustfield` フラグ  ==  `ios_base::`[左](../standard-library/ios-functions.md#left)に、フラグ **-** が前に付加されます。 (埋め込みは、生成されたテキストの後に発生します。)
...
- **Iosbase. flags** の場合。   &  フラグ `ios_base::`[showpos](../standard-library/ios-functions.md#showpos)が0以外の場合、 **+** 変換仕様の前にフラグが付加されます。

- **Iosbase. flags** の場合。 **フラグ**  & **ios_base::**[showbase](../standard-library/ios-functions.md#showbase)が0以外の場合、 **#** 変換仕様の前にフラグが付加されます。

整数の出力フィールドの形式は、**fac.** [](../standard-library/locale-functions.md#use_facet)numpunct (iosbase. flags) の use_facet 呼び出しによって返される [ロケールファセット](../standard-library/locale-class.md#facet_class)によってさらに決定され  <  [](../standard-library/numpunct-class.md) \< **Elem**> ます。  [getloc](../standard-library/ios-base-class.md#getloc))。 具体的には、次のとおりです。
...
- **Iosbase. flags** の場合。   &  `ios_base::floatfield` フラグ  ==  `ios_base::`[修正](../standard-library/ios-functions.md#fixed)済み。変換仕様は `lf` です。

- **Iosbase. flags** の場合。 **フラグ**  & **ios_base:: floatfield**  ==  `ios_base::`[科学的](../standard-library/ios-functions.md#scientific)な変換仕様は `le` です。 **Iosbase. flags** の場合。   &  フラグ `ios_base::`[大文字](../standard-library/ios-functions.md#uppercase)は0以外で、 `e` はに置き換えられ `E` ます。
...
- **Iosbase. flags** の場合。   &  フラグ `ios_base::`[showpoint](../standard-library/ios-functions.md#showpoint)が0以外の場合、 **#** 変換仕様の前にフラグが付加されます。
