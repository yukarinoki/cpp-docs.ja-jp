...
**`true`** 指定された範囲内の各要素に条件が存在する場合、を返します。
...
*`pred`*\
テストする条件。 これは、チェックされる要素によって満たされる条件を定義するユーザー定義の述語関数オブジェクトです。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

指定された **`true`** 範囲内の各要素で条件が検出された場合、または範囲が空の場合はを返します。それ以外の場合はを返し **`false`** ます。

### <a name="remarks"></a>解説

このテンプレート関数は、 **`true`** 範囲内の各に対して述語がである場合にのみを返し `N` `[0, last - first)` `pred(*(first + N))` **`true`** ます。
...
**`true`** 指定された要素の範囲内に条件が1回以上存在する場合に、を返します。
...
*`pred`*\
テストする条件。 これは、ユーザー定義の述語関数オブジェクトによって提供されます。 述語は、テスト対象の要素によって満たされる条件を定義します。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

条件が検出されない場合は、指定された **`true`** 範囲で少なくとも1回、条件が検出された場合は、を返し **`false`** ます。

### <a name="remarks"></a>解説

このテンプレート関数は、 **`true`** `N` 範囲内の for がの場合にのみを返します。
...
*`pred`*\
1 つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 二項述語は、2つの引数を受け取り、満たされると満たされない場合はを返し **`true`** **`false`** ます。

### <a name="return-value"></a>戻り値

**`true`** 指定した値と等しいか等しい範囲内に要素が見つかった場合は。それ以外の場合は **`false`** 。
...
アルゴリズムの複雑さは、ランダムアクセス反復子の場合は対数、それ以外の場合は () のステップ数に比例して線形的です `last`  -  `first` 。
...
*`pred`*\
またはと比較するために使用される述語 *`value`* *`lower`* *`upper`* 。 比較述語は2つの引数を受け取り、 **`true`** 最初の値が2番目の意味を持つ場合はを返し、それ以外の場合はを返し **`false`** ます。

### <a name="return-value"></a>戻り値

If への参照 *`lower`* 、 `value < lower` または if への参照を返し *`upper`* `upper < value` ます。 それ以外の場合は、への参照を返し *`value`* ます。

### <a name="remarks"></a>解説

がより小さい場合、動作は未定義です *`upper`* *`lower`* 。
...
ターゲット範囲内の最後の要素の1つ後ろの位置 (つまり反復子アドレス `result` + ( *`last`*  -  *`first`* )) を示す出力反復子。

### <a name="remarks"></a>解説

ソース範囲内が有効で、コピーするすべての要素を保持する十分な領域がターゲットに必要です。

アルゴリズムは最初の要素で始まる順序でソース要素をコピーするため、ソース範囲の *最後* の位置がターゲット範囲に含まれていない場合、ターゲット範囲はソース範囲と重複する可能性があります。 `copy` は、ソースとターゲットの範囲間に重複がない場合を除いて、要素を左にシフトするために使用できますが、右側には移動できません。 任意の位置の数だけ右にシフトするには、アルゴリズムを使用し [`copy_backward`](algorithm-functions.md#copy_backward) ます。
...
ターゲット範囲内の最後の要素の1つ後ろの位置 (つまり反復子アドレス *`destEnd`* -( *`last`*  -  *`first`* )) を示す出力反復子。

### <a name="remarks"></a>解説

ソース範囲内が有効で、コピーするすべての要素を保持する十分な領域がターゲットに必要です。

アルゴリズムでは `copy_backward` 、アルゴリズムよりも厳しい要件が課され `copy` ます。 入力反復子と出力反復子はどちらも、双方向である必要があります。

`copy_backward`とアルゴリズムは、 [`move_backward`](algorithm-functions.md#move_backward) ターゲット範囲の末尾を指す反復子で出力範囲を指定する唯一の C++ 標準ライブラリアルゴリズムです。

アルゴリズムは最後の要素で始まる順序でソース要素をコピーするため、ソース範囲の *最初* の位置がターゲット範囲に含まれていない場合、ターゲット範囲はソース範囲と重複する可能性があります。 `copy_backward` は要素を右にシフトするために使用できますが、ソースとターゲットの範囲間に重複がない場合を除いて、左にはシフトできません。 任意の位置の数だけ左に移動するには、アルゴリズムを使用し [`copy`](algorithm-functions.md#copy) ます。
...
要素の範囲内で、指定した条件の要素をコピーし **`true`** ます。
...
*`pred`*\
範囲内のすべての要素がテストされる条件。 この条件は、ユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は、1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

*`dest`* 条件を満たす要素ごとに1回インクリメントされる出力反復子。 つまり、戻り値からを減算すると、 *`dest`* コピーされた要素の数と等しくなります。
...
を `[0, last - first)` の範囲で、各 `N` に対して 1 回評価し、`N` の値を最低値から厳密に 1 ずつ増やします。 *`dest`* とが *`first`* ストレージの領域を指定する場合、は *`dest`* 範囲内である必要があり `[ first, last )` ます。
...
要素のコピー先となった場所を示す出力反復子を返します。 これは、パラメーターの戻り値と同じです *`dest`* 。

### <a name="remarks"></a>解説

このテンプレート関数は、 の範囲で、`*(dest + N) = *(first + N))``[0, count)` を各 `N` に対して 1 回評価し、`N` の値を最低値から厳密に 1 ずつ増やします。 その後で `dest + N` が返されます。 *`dest`* とが *`first`* ストレージの領域を指定する場合、は *`dest`* 範囲内である必要があり `[first, last)` ます。
...
`InputIterator`値を持つ範囲 [,) 内の要素の数をカウントするの差の型 *`first`* *`last`* *`value`* 。

### <a name="remarks"></a>解説

要素と指定された値の間の一致を判断するために使用される `operator==` によって、オペランド間の等価関係が強制される必要があります。

このアルゴリズムは、テンプレート関数を使用する任意の述語を満たす要素をカウントするために一般化されてい [`count_if`](algorithm-functions.md#count_if) ます。
...
*`pred`*\
要素がカウントされる場合に満たされる条件を定義するユーザー定義の述語関数オブジェクト。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。
...
このテンプレート関数は、アルゴリズムを一般化したもので [`count`](algorithm-functions.md#count) 、述語 "特定の値に等しい" を任意の述語に置き換えます。
...
*`pred`*\
2 つの要素が等価であると見なされた場合に条件が満たされると定義する、ユーザー定義の述語関数オブジェクト。 二項述語は、2つの引数を受け取り、満たされると満たされない場合はを返し **`true`** **`false`** ます。

### <a name="return-value"></a>戻り値

**`true`** 要素によって要素を比較したときに、範囲が同一であるか、二項述語で等価である場合にのみそれ以外の場合は **`false`** 。

### <a name="remarks"></a>解説

検索対象の範囲が有効であること、すべての反復子が逆参照可能であること、さらに先頭位置からのインクリメントにより最後の位置に到達可能であることが必要です。

2 つの範囲の長さが等しい場合、アルゴリズムの時間の複雑さは、範囲に含まれる要素の数に比例します。 それ以外の場合、関数はすぐにを返し **`false`** ます。
...
*`pred`*\
1 つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 比較述語は2つの引数を受け取り、満たされた場合にはを返し、 **`true`** **`false`** 満たされない場合はを返します。

### <a name="return-value"></a>戻り値

検索対象の範囲内に含まれる、サブ範囲を指定する前方反復子のペア。すべての要素は、 *`value`* 使用される二項述語 (または既定値の小さい方) によって定義された意味でに相当し *`pred`* ます。

範囲内の要素がと等価でない場合 *`value`* 、戻り値のペアの前方反復子は等しいため、 *`value`* 範囲の順序を気にせずにを挿入できる位置を指定します。

### <a name="remarks"></a>解説

アルゴリズムによって返されたペアの最初の反復子は [`lower_bound`](algorithm-functions.md#lower_bound) で、2番目の反復子は [`upper_bound`](algorithm-functions.md#upper_bound) です。

対象範囲は `equal_range` に提供される述語に従って並べ替えられている必要があります。 たとえば、大なり (greater-than) 述語を使用する場合は、範囲を降順に並べ替える必要があります。

によって返される反復子のペアによって定義される、空の可能性があるサブ範囲の要素 `equal_range` は、使用される述語によって定義された意味の *値* に相当します。
...
*`first`*\
値が割り当てられる範囲内の最初の要素の位置を示す出力反復子 *`value`* 。

*`count`*\
値を代入する要素の数を指定する符号付きまたは符号なし整数の型。

*`value`*\
範囲 [,) 内の要素に割り当てられる *`first`* 値 *`first + count`* 。

### <a name="return-value"></a>戻り値

0の場合は、最後に入力された要素の次の要素を指す反復子 *`count`* 。それ以外の場合は最初の要素 >。
...
指定された値が検索範囲内で最初に出現する位置を示す入力反復子。 同等の値を持つ要素が見つからない場合、はを返し *`last`* ます。

### <a name="remarks"></a>解説

要素と指定された値の間の一致を判断するために使用される `operator==` によって、オペランド間の等価関係が強制される必要があります。

を使用したコード例については `find()` 、「」を参照してください [`find_if`](algorithm-functions.md#find_if) 。
...
*`pred`*\
ユーザー定義の述語関数オブジェクト、または検索対象の要素によって満たされる条件を定義する[ラムダ式](../cpp/lambda-expressions-in-cpp.md)。 単項述語は1つの引数を受け取り、 **`true`** 満たされた場合はを返し、 **`false`** 満たされていない場合はを返します。 のシグネチャは *`pred`* 、実際にはである必要があり `bool pred(const T& arg);` ます。ここで、 `T` は、逆参照 `InputIterator` するときに暗黙的に変換できる型です。 **`const`** キーワードは、関数オブジェクトまたはラムダが引数を変更できないことを示すためにのみ表示されます。

### <a name="return-value"></a>戻り値

述語によって指定された条件を満たす範囲内の最初の要素を参照する入力反復子 (述語の結果は **`true`** )。 述語を満たす要素が見つからない場合、はを返し *`last`* ます。

### <a name="remarks"></a>解説

このテンプレート関数は、アルゴリズムを一般化したもので [`find`](algorithm-functions.md#find) 、述語 "特定の値に等しい" を任意の述語に置き換えます。 論理的な反対 (述語を満たさない最初の要素を検索) については、「」を参照してください [`find_if_not`](algorithm-functions.md#find_if_not) 。
...
*`pred`*\
検索対象の要素が満たさない条件を定義する、ユーザー定義の述語関数オブジェクトまたは[ラムダ式](../cpp/lambda-expressions-in-cpp.md)。 単項述語は1つの引数を受け取り、 **`true`** 満たされた場合はを返し、 **`false`** 満たされていない場合はを返します。 のシグネチャは *`pred`* 、実際にはである必要があり `bool pred(const T& arg);` ます。ここで、 `T` は、逆参照 `InputIterator` するときに暗黙的に変換できる型です。 **`const`** キーワードは、関数オブジェクトまたはラムダが引数を変更できないことを示すためにのみ表示されます。

### <a name="return-value"></a>戻り値

述語によって指定された条件を満たさない範囲内の最初の要素を参照する入力反復子 (述語の結果は **`false`** )。 すべての要素が述語を満たす場合 (述語は **`true`** すべての要素に対してになります)、はを返し *`last`* ます。

### <a name="remarks"></a>解説

このテンプレート関数は、アルゴリズムを一般化したもので [`find`](algorithm-functions.md#find) 、述語 "特定の値に等しい" を任意の述語に置き換えます。 論理的な反対 (述語を満たす最初の要素を検索する) については、「」を参照してください [`find_if`](algorithm-functions.md#find_if) 。

に簡単に適応するコード例につい `find_if_not()` ては、「」を参照してください [`find_if`](algorithm-functions.md#find_if) 。
...
複雑さは線形的で、 `last`  -  `first` 必要なジェネレーターを正確に () 呼び出します。
...
**`true`** 最初の並べ替えられた範囲に2番目の並べ替えられた範囲のすべての要素が含まれている場合は。それ以外の場合は **`false`** 。
...
アルゴリズムの複雑さは線形的で、空でない `2 * ((last1 - first1) - (last2 - first2)) - 1` ソース範囲に対して最も多くの比較があります。
...
*`pred`*\
1 つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 比較述語は2つの引数を受け取り、 **`true`** 最初の要素が2番目の要素より小さい場合はを返し、それ以外の場合はを返し **`false`** ます。
...
複雑さは、アルゴリズムがメモリを一時バッファーに割り当てる際に使用可能なメモリによって異なります。 十分なメモリを使用できる場合、最適なケースは比較による線形です `(last - first) - 1` 。補助メモリが使用できない場合は、最悪の場合は、の場合はと `N log(N)` *`N`*  =  *`last`*  -  *`first`* なります。
...
**`true`** 指定された範囲の要素がヒープを形成する場合はを返します。
...
*`pred`*\
要素の並べ替えをテストするための条件。 比較述語は2つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

**`true`** 指定された範囲内の要素がヒープを形成する場合は、それ以外の場合はを返し **`false`** ます。

### <a name="remarks"></a>解説

最初のテンプレート関数は、を返し [`is_heap_until`](algorithm-functions.md#is_heap_until) `(first , last) == last` ます。
...
範囲 [,) のうち、ヒープの順序付け条件を満たしていない最初の要素に配置された反復子を返し `first` `last` *`end`* ます。または、範囲がヒープを形成している場合はを返します。
...
*`pred`*\
ヒープを定義する厳密弱順序の条件を指定する二項述語。 既定の述語は、が指定されてい `std::less<>` ない場合です *`pred`* 。

### <a name="return-value"></a>戻り値

*`last`* 指定された範囲がヒープを形成するか、または1つ以下の要素が含まれている場合は、を返します。 それ以外の場合、ヒープの条件を満たさない最初の要素の反復子を返します。

### <a name="remarks"></a>解説

1 つ目のテンプレート関数は、`[first, last)` のうち、`[first, next)` が関数オブジェクト `std::less<>` によって順序付けられるヒープである場合の最後の反復子 `next` を返します。 距離 `last - first` が2未満の場合、関数はを返し *`last`* ます。

2番目のテンプレート関数は、 *`pred`* `std::less<>` ヒープの順序付け条件としてではなく述語を使用する点を除いて、1番目の関数と同じように動作します。

## <a name="is_partitioned"></a><a name="is_partitioned"></a> is_partitioned

**`true`** 条件をテストする特定の範囲のすべての要素が、テスト対象の要素の前にある場合は、を返し **`true`** **`false`** ます。
...
*`pred`*\
テストする条件。 これは、検索対象の要素によって満たされる条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

**`true`** 条件をテストする特定の範囲のすべての要素が、テスト対象の要素の前にある場合はを返し **`true`** **`false`** ます。それ以外の場合はを返し **`false`** ます。

### <a name="remarks"></a>解説

このテンプレート関数は、 **`true`** 内のすべての要素が pred によってパーティション分割されている場合にのみを返します。つまり、が true であるのすべての要素は `[first, last)` 、がである `X` `[first, last)` `pred (X)` すべての要素の前に発生し `Y` `pred (Y)` **`false`** ます。
...
**`true`** 比較演算子の述語に従って同一になるように範囲を再配置できる場合は。それ以外の場合は **`false`** 。

### <a name="remarks"></a>解説

`is_permutation` は、最悪の場合に 2 次の複雑さを持ちます。

1つ目のテンプレート関数は、で指定された範囲内にあるとおり、 *first2* で始まる範囲内に要素が複数あることを前提としてい `[first1, last1)` ます。 2 つ目の範囲にそれよりも多くの要素がある場合、それらの要素は無視されます。2 つ目の範囲にそれよりも少ない要素がある場合は、未定義の動作が発生します。 3 つ目のテンプレート関数 (C++ 14 以降) では、この想定を行っていません。 **`true`** は、で指定された範囲内の各要素 x に対して、 `[first1, last1)` またはで始まる範囲内にある x = = y と同じ範囲内の要素 Y が1つだけの場合にのみを返し *`first2`* `[first2, last2)` ます。 ここで、は、 `operator==` オペランド間のペアの比較を実行する必要があります。
...
指定した **`true`** 範囲の要素が並べ替えられた順序である場合はを返します。
...
*`pred`*\
2 つの要素間の順序を決定するテストの条件。 比較述語は2つの引数を受け取り、 **`true`** またはを返し **`false`** ます。 これは `operator<` と同じタスクを実行します。

### <a name="remarks"></a>解説

最初のテンプレート関数は、を返し [`is_sorted_until`](#is_sorted_until) `( first, last ) == last` ます。 関数は、 `operator<` 順序の比較を実行します。

2 番目のテンプレート関数は、`is_sorted_until( first, last , pred ) == last` を返します。 *`pred`* 述語関数は、順序の比較を実行します。

## <a name="is_sorted_until"></a><a name="is_sorted_until"></a> `is_sorted_until`

指定した範囲から並べ替え順序で最後の要素に設定されている `ForwardIterator` を返します。

2番目のバージョンでは、指定した **`true`** 2 つの要素が並べ替えられた順序である場合にを返す比較関数オブジェクトを指定できます。それ以外の場合はを返し **`false`** ます。
...
*`pred`*\
2 つの要素間の順序を決定するテストの条件。 比較述語は2つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

並べ替え順序で最後の要素に設定された `ForwardIterator` を返します。 並べ替えられたシーケンスはから始まり *`first`* ます。

### <a name="remarks"></a>解説

1 つ目のテンプレート関数は、 が  によって並べ替えられたシーケンス順序になるように、`[first, last]``[first, next)``operator<` の中で最後の反復子 `next` を返します。 が2未満の場合 `distance()` 、関数はを返し *`last`* ます。
...
`swap` は、旧バージョンとの互換性のために C++ 標準に含まれていた **iter_swap** に優先して使用する必要があります。 `Fit1`と `Fit2` が前方反復子の場合、はと `iter_swap( Fit1, Fit2 )` 同じに `swap( *Fit1, *Fit2 )` なります。
...
**`true`** 1番目の範囲が辞書式未満の場合は。それ以外の場合は **`false`** 。
...
- 不等ずが見つかりません。シーケンスの要素数が同じであるため、シーケンスは等しく、比較の結果はになり **`false`** ます。
...
ヒープは、優先順位キューを実装するのに最適な方法です。C++ 標準ライブラリ コンテナー アダプター [priority_queue Class](priority-queue-class.md) の実装に使用されます。

複雑さは線形的で、 `3 * (last - first)` 比較が必要です。
...
2 つのオブジェクトのうち大きい方のオブジェクト。大きいオブジェクトが存在しない場合は、2 つのオブジェクトのうち 1 番目のオブジェクトが返されます。 の場合は `initializer_list` 、リスト内の最大のオブジェクトを返します。

### <a name="remarks"></a>解説

`max` アルゴリズムでは、パラメーターとして渡されるオブジェクトを使うことはほとんどありません。 C++ 標準ライブラリの多くのアルゴリズムは、パラメーターとして渡された反復子によって位置が指定されている要素の範囲で動作します。 要素の範囲で動作する関数が必要な場合は、代わりにを使用 [`max_element`](algorithm-functions.md#max_element) します。 Visual Studio 2017 **`constexpr`** では、を受け取るオーバーロードでが有効になり `initializer_list` ます。
...
複雑さは線形的で、空でない `(last - first) - 1` 範囲に対して比較が必要です。
...
各範囲内の要素の相対順序はターゲット範囲内でも維持されるため、操作は安定しています。 ソース範囲はアルゴリズムによって変更されません `merge` 。
...
アルゴリズムの複雑さは線形的で、最大の `(last1 - first1) - (last2 - first2) - 1` 比較があります。

[ `list` クラス](list-class.md)は、 `merge` 2 つのリストの要素を結合するメンバー関数を提供します。
...
*`inlist`*\
`initializer_list`比較するメンバーを格納している。

### <a name="return-value"></a>戻り値

2 つのオブジェクトのうち小さい方のオブジェクト。小さいオブジェクトが存在しない場合は、2 つのオブジェクトのうち 1 番目のオブジェクトが返されます。 の場合 `initializer_list` 、リスト内の最小のオブジェクトが返されます。

### <a name="remarks"></a>解説

`min` アルゴリズムでは、パラメーターとして渡されるオブジェクトを使うことはほとんどありません。 C++ 標準ライブラリの多くのアルゴリズムは、パラメーターとして渡された反復子によって位置が指定されている要素の範囲で動作します。 要素の範囲を使用する関数が必要な場合は、を使用 [`min_element`](algorithm-functions.md#min_element) します。 [`constexpr`](../cpp/constexpr-cpp.md) は、 `initializer_list` Visual Studio 2017 のオーバーロードで有効になりました。
...
*`pred`*\
1つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 比較述語は2つの引数を受け取り、 **`true`** 最初のが2番目の引数より小さい場合はを返し、それ以外の場合はを返し **`false`** ます。
...
最初のテンプレート関数 `pair<const Type&, const Type&>( right, left )` は、 *`right`* がより小さい場合にを返し *`left`* ます。 それ以外の場合は `pair<const Type&, const Type&>( left, right )`を返します。

2番目のメンバー関数は、1番目の要素が小さい方のペアを返します。2番目の要素は、述語によって比較された場合に大きくなり *`pred`* ます。

残りのテンプレート関数は同じように動作しますが、パラメーターとパラメーターをに置き換えている点が異なり *`left`* *`right`* *`inlist`* ます。
...
*`pred`*\
各範囲内の現在の要素を比較し、それらが等しいかどうかを判定するための、ユーザー定義の述語関数オブジェクト。 このメソッド **`true`** は、満たされている場合はを返し、 **`false`** 満たされない場合はを返します。
...
このテンプレート関数は、 の範囲で、`*(dest + N) = move(*(first + N))``[0, last - first)` を各 `N` に対して 1 回評価し、`N` の値を最低値から厳密に 1 ずつ増やします。 その後で `dest + N` が返されます。 `dest`とが *`first`* ストレージの領域を指定する場合、は *`dest`* 範囲内である必要があり `[first, last)` ます。
...
このテンプレート関数は、 の範囲で、`*(destEnd - N - 1) = move(*(last - N - 1))``[0, last - first)` を各 `N` に対して 1 回評価し、`N` の値を最低値から厳密に 1 ずつ増やします。 その後で `destEnd - (last - first)` が返されます。 *`destEnd`* とが *`first`* ストレージの領域を指定する場合、は *`destEnd`* 範囲内である必要があり `[first, last)` ます。
...
*`pred`*\
順序内の次の要素によって満たされる比較条件を定義するユーザー定義の述語関数オブジェクト。 二項述語は、2つの引数を受け取り、満たされると満たされない場合はを返し **`true`** **`false`** ます。

### <a name="return-value"></a>戻り値

**`true`** 辞書式次の順列が存在し、範囲の元の順序を置き換えた場合は。それ以外 **`false`** の場合は、順序付けが辞書式の最小の順列に変換されます。

### <a name="remarks"></a>解説

参照されている範囲が有効であり、すべてのポインターが逆参照可能であって、かつシーケンス内で先頭位置からのインクリメントにより最後の位置に到達可能である必要があります。

既定の二項述語はより小さく、次の順列が適切に定義されていることを保証するために、範囲内の要素は比較可能である必要があります。

複雑さは線形的で、最大 `(last - first) / 2` スワップがあります。
...
要素の範囲を分割し、その範囲内のシーケンスの *n* 番目の要素を正しく検索します。これにより、その前にあるすべての要素がそれ以下で、シーケンス内の後続のすべての要素がその値以上になります。
...
*`pred`*\
順序内の次の要素によって満たされる比較条件を定義するユーザー定義の述語関数オブジェクト。 比較述語は2つの引数を受け取り、満たされた場合にはを返し、 **`true`** **`false`** 満たされない場合はを返します。

### <a name="remarks"></a>解説

参照されている範囲が有効であり、すべてのポインターが逆参照可能であって、かつシーケンス内で先頭位置からのインクリメントにより最後の位置に到達可能である必要があります。

アルゴリズムでは、 `nth_element` *n* 番目の要素の両側の要素が並べ替えられることは保証されません。 そのため、一定の選択した要素を下回る範囲内の要素を並べ替える `partial_sort` よりも保証は少なくなり、下の範囲の要素を並べ替えが不要な場合に `partial_sort` のより高速な代替として使用することができます。

どちらの要素も他方より小さくない場合、要素は同等ですが、必ずしも等しいわけではありません。

並べ替えの複雑さの平均は、に対して線形的です *`last - first`* 。
...
**`true`** 指定された範囲内の要素間に条件が存在しない場合はを返します。
...
*`pred`*\
テストする条件。 これは、条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="return-value"></a>戻り値

指定された **`true`** 範囲で少なくとも1回条件が検出されなかった場合、および条件が検出された場合は、を返し **`false`** ます。

### <a name="remarks"></a>解説

このテンプレート関数は、 **`true`** 範囲内の一部の `N` 述語が常にである場合にのみを返し `[0, last - first)` `pred(*(first + N))` **`false`** ます。
...
どちらの要素も他方より小さくない場合、要素は同等ですが、必ずしも等しいわけではありません。 `sort`アルゴリズムが安定しておらず、同等の要素の相対順序が保持されることは保証されません。 アルゴリズム `stable_sort` は元の順序を保持します。

部分的な並べ替えの複雑さの平均は *O*(( `last` -  `first` ) log ( `sortEnd` -  `first` )) です。
...
*`pred`*\
要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクト。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。
...
要素 *`a`* とは *`b`* 等価ですが、必ずしも等しいとは限りません。が false であり、が false である場合、 `pred( a, b )` `pred( b, a )` *`pred`* はパラメーター指定の述語です。 `partition`アルゴリズムが安定しておらず、同等の要素の相対順序が保持されることは保証されません。 アルゴリズム `stable_partition` は元の順序を保持します。

複雑さは線形的であり、 `(last - first)` のアプリケーションと最大スワップ数のアプリケーションがあり *`pred`* `(last - first)/2` ます。
...
条件が **`true`** 1 つの宛先にあり、条件が別の変換先になる要素をコピーし **`false`** ます。 要素は指定された範囲に含まれている必要があります。
...
*`dest1`*\
*Pred* を使用してテストされた条件に対して true を返す要素をコピーするために使用される出力反復子。

*`dest2`*\
*Pred* を使用してテストされた条件に対して false を返す要素をコピーするために使用される出力反復子。

*`pred`*\
テストする条件。 これは、テストする条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は1つの引数を受け取り、 **`true`** またはを返し **`false`** ます。

### <a name="remarks"></a>解説

このテンプレート関数は、 `X` `[first,last)` が true の場合は、の各要素をにコピー `*dest1++` `pred(X)` `*dest2++` します。それ以外の場合はにコピーします。 `pair<OutputIterator1, OutputIterator2>(dest1, dest2)` を返します。
...
`ForwardIterator` *Pred* によってテストされた条件を満たさない最初の要素を参照するを返します。見つからない場合は、 *last* を返します。

### <a name="remarks"></a>解説

このテンプレート関数は、がであるの最初の反復子を検索し `it` `[first, last)` `pred(*it)` **`false`** ます。 シーケンスはによって並べ替えられる必要があり *`pred`* ます。
...
複雑さは対数で、ほとんどの比較が必要 `log (last - first)` です。
...
**`true`** 辞書式前の順列が存在し、範囲の元の順序を置き換えた場合は。それ以外 **`false`** の場合は、順序付けが辞書式最大の順列に変換されます。

### <a name="remarks"></a>解説

参照されている範囲が有効であり、すべてのポインターが逆参照可能であって、かつシーケンス内で先頭位置からのインクリメントにより最後の位置に到達可能である必要があります。

既定の二項述語はよりも小さく、範囲内の要素は、前の順列が適切に定義されていることを確認するために比較可能である必要があります。
...
`std::random_shuffle()`関数は非推奨とされ、に置き換えられました [`std::shuffle`](algorithm-functions.md#shuffle) 。 コード例と詳細については、「」および Stack Overflow を参照してください [`<random>`](random.md) 。 [ `std::random_shuffle` c++ 14 でメソッドが非推奨とされる理由](https://go.microsoft.com/fwlink/p/?linkid=397954)について説明します。
...
複雑さは線形的です。( `last`  -  `first` ) の等価比較があります。

[ `list` クラス](list-class.md)には、のより効率的なメンバー関数バージョンが用意されて `remove` おり、ポインターもエディットコンティニュます。
...
複雑さは線形的です。( `last`  -  `first` ) の等価比較と、最大で () の割り当てがあり `last`  -  `first` ます。
...
複雑さは線形的であり、( `last`  -  `first` ) の等価比較と、最大で () の割り当てがあり `last`  -  `first` ます。

これらの関数の動作については、「[チェックを行う反復子](checked-iterators.md)」を参照してください。
...
複雑さは線形的です。() の `last`  -  `first` 等価比較と、最大で ( `last`  -  `first` ) の新しい値の割り当てがあります。
...
アルゴリズムはアルゴリズムを一般化したもので、指定され `replace_if` `replace` た定数値と等価ではなく、任意の述語を指定できます。
...
*`pred`*\
1 つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 二項述語は2つの引数を受け取り、 **`true`** 最初の要素が2番目の要素より小さい場合はを返し、それ以外の場合はを返し **`false`** ます。
...
アルゴリズムの複雑さは線形的で、最大の `2 * ((last1 - first1) - (last2 - first2)) - 1` 比較があります。
...
の詳細と、を使用するコードサンプルについ `shuffle()` ては、「」を参照してください [`<random>`](random.md) 。
...
*`pred`*\
順序内の次の要素によって満たされる比較条件を定義するユーザー定義の述語関数オブジェクト。 この二項述語は2つの引数を受け取り、 **`true`** 2 つの引数が順序付けされている場合はを返し、それ以外の場合はを返し **`false`** ます。 この比較子関数は、シーケンスからの要素のペアで厳密弱順序を強制する必要があります。 詳細については、「[アルゴリズム](algorithms.md)」をご覧ください。
...
並べ替えの複雑さの平均はです `O( N log N )` 。ここで、 *N* は最初に指定  =    -  します。
...
ヒープは優先順位の高いキューを実装するのに最適な方法であり、C++ 標準ライブラリのコンテナーアダプター [ `priority_queue` クラス](priority-queue-class.md)の実装で使用されます。

参照されている範囲が有効であり、すべてのポインターが逆参照可能であって、かつシーケンス内で先頭位置からのインクリメントにより最後の位置に到達可能である必要があります。

複雑さは最大で `N log N` 、N は最初に *N* に  =    -  なります。
...
*`pred`*\
要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクト。 単項述語は1つの引数を受け取り、 **`true`** 満たされた場合はを返し、 **`false`** 満たされていない場合はを返します。
...
要素 *a* と *b* は同等ですが、両方とも false で、が false である場合、は `pred( a, b )` `pred( b, a )` *`pred`* パラメーター指定の述語です。 `stable_partition`アルゴリズムは安定しており、同等の要素の相対順序が保持されることを保証します。 このアルゴリズムで `partition` は、必ずしもこの順序を維持するわけではありません。
...
の実行時の複雑さは、 `stable_sort` 使用可能なメモリの量によって異なりますが、最適なケース (十分なメモリがある場合) はで、 `O(N log N)` 最悪のケースはです `O(N (log N)^2)` 。ここで、 *N* は最初に指定し  =    -  ます。 通常、 `sort` アルゴリズムはよりも大幅に高速です `stable_sort` 。
...
最初のバージョンのアルゴリズムで *result* がと同じに設定されている場合、 *`first1`* ソースとターゲットの範囲は同じになり、シーケンスが適切に変更されます。 ただし、では、 *`result`* 範囲 [ `first1` + 1,) 内の位置をアドレス指定することはできません `last1` 。
...
参照されている範囲が有効であり、すべてのポインターが逆参照可能であって、かつシーケンス内で先頭位置からのインクリメントにより最後の位置に到達可能である必要があります。 シーケンス内の要素の数はアルゴリズムによって変更されず、変更され `unique` たシーケンスの末尾を越えた要素はかつインクリメントますが、指定されていません。

複雑さは線形的で、 `(last - first) - 1` 比較が必要です。
...
*`pred`*\
1つの要素が別の要素より小さいという意味を定義するユーザー定義の比較述語関数オブジェクト。 比較述語は2つの引数を受け取り、満たされた場合にはを返し、 **`true`** **`false`** 満たされない場合はを返します。
...
並べ替えられた範囲は、およびの使用の前提条件であり、 `upper_bound` 順序の基準は比較述語によって指定されたものと同じです。
