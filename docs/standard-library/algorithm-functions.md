...
このテンプレート関数は**true** 、範囲内の各に対して `N` `[0, last - first)` 述語 `pred(*(first + N))` が**true**の場合にのみ true を返します。
...
このテンプレート関数は、範囲内の for がの場合にのみ**true**を返します。 `N`
...
*pred*\
ユーザー定義の述語関数オブジェクト、または検索対象の要素によって満たされる条件を定義する[ラムダ式](../cpp/lambda-expressions-in-cpp.md)。 単項述語は1つの引数を受け取り、満たされた場合は**true**を返し、満たされていない場合は**false**を返します。 *Pred*のシグネチャは、実際にはにする必要があり `bool pred(const T& arg);` ます。ここで、 `T` は、逆参照 `InputIterator` するときに暗黙的に変換できる型です。 **Const**キーワードは、関数オブジェクトまたはラムダが引数を変更できないことを示すためにのみ表示されます。
...
*pred*\
検索対象の要素が満たさない条件を定義する、ユーザー定義の述語関数オブジェクトまたは[ラムダ式](../cpp/lambda-expressions-in-cpp.md)。 単項述語は1つの引数を受け取り、満たされた場合は**true**を返し、満たされていない場合は**false**を返します。 *Pred*のシグネチャは、実際にはにする必要があり `bool pred(const T& arg);` ます。ここで、 `T` は、逆参照 `InputIterator` するときに暗黙的に変換できる型です。 **Const**キーワードは、関数オブジェクトまたはラムダが引数を変更できないことを示すためにのみ表示されます。
...
指定された範囲の要素がヒープを形成する場合に**true**を返します。
...
*pred*\
要素の並べ替えをテストするための条件。 比較述語は2つの引数を受け取り、 **true**または**false**を返します。
...
条件の**true**をテストする特定の範囲のすべての要素が、 **false**をテストする要素の前にある場合に**true**を返します。
...
*pred*\
テストする条件。 これは、検索対象の要素によって満たされる条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は1つの引数を取り、 **true**または**false**を返します。
...
このテンプレート関数は、内のすべての要素が pred によってパーティション分割されている場合にのみ**true**を返します。つまり、が true のすべての要素は `[first, last)` 、 *pred* `X` `[first, last)` `pred (X)` `Y` が false のすべての要素の前に発生し `pred (Y)` ます。 **false**
...
1つ目のテンプレート関数は、で指定された範囲内にあるとおり、 *first2*で始まる範囲内に要素が複数あることを前提としてい `[first1, last1)` ます。 2 つ目の範囲にそれよりも多くの要素がある場合、それらの要素は無視されます。2 つ目の範囲にそれよりも少ない要素がある場合は、未定義の動作が発生します。 3 つ目のテンプレート関数 (C++ 14 以降) では、この想定を行っていません。 両方とも**true**が返されるのは、によって指定された範囲内の各要素 x に対して、 `[first1, last1)` *first2*またはから始まる範囲内にある x = = y と同じ範囲内の要素 Y が1つだけである場合のみ `[first2, last2)` です。 ここで、は、 `operator==` オペランド間のペアの比較を実行する必要があります。
...
指定された範囲の要素が並べ替えられた順序である場合に**true**を返します。
...
2番目のバージョンでは、2つの指定した要素が並べ替えられた順序である場合に**true**を返す比較関数オブジェクトを指定できます。それ以外の場合は**false**を返します。
...
*pred*\
2 つの要素間の順序を決定するテストの条件。 比較述語は2つの引数を受け取り、 **true**または**false**を返します。
...
`max` アルゴリズムでは、パラメーターとして渡されるオブジェクトを使うことはほとんどありません。 C++ 標準ライブラリの多くのアルゴリズムは、パラメーターとして渡された反復子によって位置が指定されている要素の範囲で動作します。 要素の範囲で動作する関数が必要な場合は、代わりに [max_element](algorithm-functions.md#max_element) を使用してください。 Visual Studio 2017 では、initializer_list を受け取るオーバーロードで**constexpr**が有効になります。
...
*pred*\
1つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 比較述語は2つの引数を受け取り、最初の引数が2番目の引数より小さい場合は**true**を、それ以外の場合は**false**を返します。
...
指定された範囲内の要素間に条件が存在しない場合に**true**を返します。
...
*pred*\
テストする条件。 これは、条件を定義するユーザー定義の述語関数オブジェクトによって提供されます。 単項述語は1つの引数を取り、 **true**または**false**を返します。
...
このテンプレート関数は**true** 、範囲の一部 `N` で `[0, last - first)` 述語 `pred(*(first + N))` が常に**false**である場合にのみ true を返します。
...
*pred*\
要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクト。 単項述語は1つの引数を取り、 **true**または**false**を返します。
...
条件が**true**である要素を1つの変換先にコピーし、条件を**false**にします。 要素は指定された範囲に含まれている必要があります。
...
テンプレート関数は、が false であるの最初の反復子を検索し `it` `[first, last)` `pred(*it)` ます。 **false** シーケンスは、 *pred*によって順序付けされている必要があります。
...
*pred*\
1 つの要素が別の要素より小さいという意味を定義するユーザー定義の述語関数オブジェクト。 2 項述語は 2 つの引数を受け取り、最初の要素が 2 番目の要素より小さい場合は **true** 、そうでない場合は **false** を返す必要があります。
...
*pred*\
順序内の次の要素によって満たされる比較条件を定義するユーザー定義の述語関数オブジェクト。 この二項述語は2つの引数を受け取り、2つの引数が順序付けられている場合は**true**を返し、それ以外の場合は**false**を返します。 この比較子関数は、シーケンスからの要素のペアで厳密弱順序を強制する必要があります。 詳細については、「[アルゴリズム](algorithms.md)」をご覧ください。
...
*pred*\
要素が分類される場合に満たされる条件を定義するユーザー定義の述語関数オブジェクト。 単項述語は1つの引数を受け取り、満たされた場合は**true**を返し、満たされていない場合は**false**を返します。
...
*pred*\
1つの要素が別の要素より小さいという意味を定義するユーザー定義の比較述語関数オブジェクト。 比較述語は2つの引数を受け取り、満たされた場合は**true**を返し、満たされていない場合は**false**を返します。
