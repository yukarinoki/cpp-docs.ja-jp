...
|演算子|説明|
|-|-|
|[unordered_map::演算子[]](#op_at)|指定したキーを持つ要素を検索または挿入します。|
|[unordered_map::演算子=](#op_eq)|ハッシュ テーブルをコピーします。|

## <a name="remarks"></a>解説

このオブジェクトは、このオブジェクトが制御するシーケンスを、格納されている 2 つのオブジェクト ([unordered_map::key_equal](#key_equal) 型の比較関数オブジェクトと、[unordered_map::hasher](#hasher) 型のハッシュ関数オブジェクト) を呼び出すことによって並べ替えます。 最初の格納されたオブジェクトにアクセスする場合は、メンバー関数[unordered_map::key_eq](#key_eq)`()`を呼び出します。2 番目のストアド オブジェクトにアクセスする場合は、メンバー関数[unordered_map::hash_function](#hash)`()`を呼び出します。 具体的には、`X` 型のすべての値 `Y` と `Key` について、`key_eq()(X, Y)` が呼び出され、2 つの引数値の大小関係が等しい場合は true が返されます。`hash_function()(keyval)` の呼び出しからは、`size_t` 型の値の分布が生成されます。 クラス テンプレート[unordered_multimap Class](../standard-library/unordered-multimap-class.md)とは`unordered_map`異なり`key_eq()(X, Y)`、型のオブジェクトは、被制御シーケンスの任意の 2 つの要素に対して常に false になります。 キーの重複は許されません。

このオブジェクトには、さらに、適切とされるバケットあたりの最大平均要素数を指定する最大テーブル占有率が格納されます。 要素を挿入すると[、unordered_map::load_factor](#load_factor)`()`が最大負荷係数を超える場合、コンテナーはバケット数を増やし、必要に応じてハッシュ テーブルを再構築します。
...
被制御シーケンスに対するストレージの割り当ておよび解放は、格納されている [unordered_map::allocator_type](#allocator_type) 型のアロケーター オブジェクトを介して行われます。 このようなアロケーター オブジェクトは、 型`allocator`のオブジェクトと同じ外部インターフェイスを持つ必要があります。 コンテナー オブジェクトを代入しても、格納されているアロケーター オブジェクトはコピーされない点に注意してください。
...
## <a name="unordered_mapbegin"></a><a name="begin"></a>unordered_map::開始
...
## <a name="unordered_mapbucket"></a><a name="bucket"></a>unordered_map::バケツ
...
## <a name="unordered_mapclear"></a><a name="clear"></a>unordered_map::クリア
...
## <a name="unordered_mapcount"></a><a name="count"></a>unordered_map::カウント
...
## <a name="unordered_mapemplace"></a><a name="emplace"></a>unordered_map::エムプレイス
...
bool コンポーネント`pair`が true を返す場合、挿入が行`unordered_map`われ、キーに同等の値が付いた要素が既に含まれ、反復子コンポーネントが新しい要素が挿入された場所または要素が既に配置されている場所のアドレスを返す要素が既に含まれている場合は false を返します。 **bool**
...
## <a name="unordered_mapend"></a><a name="end"></a>unordered_map::終了
...
## <a name="unordered_maperase"></a><a name="erase"></a>unordered_map::消去
...
## <a name="unordered_mapfind"></a><a name="find"></a>unordered_map::検索
...
メンバー関数は[unordered_map::equal_range](#equal_range)`(keyval).first`を返します。
...
## <a name="unordered_maphasher"></a><a name="hasher"></a>unordered_map::ハッシャー
...
## <a name="unordered_mapinsert"></a><a name="insert"></a>unordered_map:挿入
...
|*ヴァルティ*|value_type の要素を構築するために unordered_map使用できる引数の型を指定する[value_type](../standard-library/map-class.md#value_type)テンプレート パラメーターvalue_type、引数として*Val*を完全に転送します。|
|*まずは*|コピーされる最初の要素の位置。|
|*前の*|コピーされる最後の要素の次の位置。|
...
単一要素メンバー関数 (1) と (2) は、挿入が行われた場合は**bool**コンポーネントが true の[ペア](../standard-library/pair-structure.md)を返し、unordered_mapにキーの順序に同等の値を持つ要素が既に含まれている場合は false を返します。 戻り値ペアの反復子コンポーネントは **、bool**コンポーネントが true の場合は新しく挿入された要素を指し **、bool**コンポーネントが false の場合は既存の要素を指します。
...
## <a name="unordered_mapiterator"></a><a name="iterator"></a>unordered_map::反復器
...
メンバー関数は`(float)`[、unordered_map::size](#size)`() / (float)`[unordered_map::bucket_count](#bucket_count)`()`を返します。
...
## <a name="unordered_mapoperator"></a><a name="op_at"></a>unordered_map::演算子[]
...
`operator[]`*m*[*Key*] =`DataValue`を使用してマップ*m*に要素を挿入するために使用できます。は`DataValue`Key のキー`mapped_type`値を持つ要素の値*です*。
...
このメンバー関数は、[unordered_map::insert](#insert)`(` [unordered_map::value_type](#value_type)`(keyval, Ty())` の戻り値として反復子 `where` を特定します。 (要素が存在しない場合は、指定したキーを持つ要素を挿入します)。次に、 への参照`(*where).second`を返します。

## <a name="unordered_mapoperator"></a><a name="op_eq"></a>unordered_map::演算子=
...
|パラメーター|説明|
|-|-|
|*そうです*|演算子関数がそこからコンテンツを割り当てる unordered_map。|

### <a name="remarks"></a>解説

最初のバージョンでは、すべての要素が*右*からこのunordered_mapにコピーされます。

2 番目のバージョンでは、すべての要素が*右*からこのunordered_mapに移動します。
...
## <a name="unordered_mappointer"></a><a name="pointer"></a>unordered_map::pオインター
...
## <a name="unordered_mapreference"></a><a name="reference"></a>unordered_map::参照
...
## <a name="unordered_maprehash"></a><a name="rehash"></a>unordered_map::再ハッシュ
...
## <a name="unordered_mapsize"></a><a name="size"></a>unordered_map::サイズ
...
## <a name="unordered_mapswap"></a><a name="swap"></a>unordered_map::スワップ
...
メンバー関数は、 と*の*間`*this`で制御されたシーケンスを入れ替えます。 [unordered_map::get_allocator](#get_allocator)`() == right.get_allocator()`場合、一定の時間で例外がスローされ、格納されている type`Tr`の特性オブジェクトをコピーした結果、例外がスローされ、2 つの被制御シーケンス内の要素を指定する参照、ポインター、または反復子は無効になります。 それ以外の場合、2 つの被制御シーケンス内の要素数に比例した回数、要素の割り当てとコンストラクター呼び出しが実行されます。
...
バケットの最小数は、引数*Bucket_count*が存在する場合は、 です。それ以外の場合は、実装定義値としてここで説明されるデフォルト`N0`値です。
