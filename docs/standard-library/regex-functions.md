...
|[スワップ](#swap)|2つ `basic_regex` または複数の `match_results` オブジェクトを交換します。|
...
*BidIt*\
サブマッチ用の反復子の型。 一般的なケースでは `string::const_iterator` 、、 `wstring::const_iterator` 、 `const char*` またはのいずれか `const wchar_t*` です。

*割り当て*\
一致結果のアロケーター クラス。

*Elem*\
一致させる要素の型。 一般的なケースでは `string` 、、 `wstring` 、 `char*` または `wchar_t*` です。
...
2つ `basic_regex` または複数の `match_results` オブジェクトを交換します。
