...
ms.locfileid: "68451374"
---
# <a name="regular-expressions-c"></a>正規表現 (C++)

標準C++ライブラリでは、複数の正規表現文法がサポートされています。 このトピックでは、正規表現を使用するときに使用できる文法のバリエーションについて説明します。

## <a name="regexgrammar"></a> 正規表現の文法

使用する正規表現文法は、 `std::regex_constants::syntax_option_type`列挙値のいずれかを使用して指定します。 これらの正規表現文法は、std:: regex_constants で定義されています。

- `ECMAScript` :これは、JavaScript および .NET 言語で使用される文法に最も近いものです。
- `basic`:POSIX の基本的な正規表現または BRE。
- `extended` :POSIX 拡張正規表現またはの場合は、
- `awk`:これは`extended`ですが、印刷以外の文字に対する追加のエスケープがあります。
- `grep` :これは`basic`ですが、改行文字 (' \n ') を使用して代替を分離することもできます。
- `egrep`:これは`extended`ですが、改行文字を分離することもできます。

既定では、文法が指定され`ECMAScript`ていない場合、が想定されます。 1つの文法のみを指定できます。

文法に加えて、いくつかのフラグを適用できます。
- `icase`:一致する場合は大文字と小文字を区別しません。
- `nosubs` :マークされた一致 (つまり、かっこ内の式) を無視します。置換は格納されません。
- `optimize`:構築時間が長くなる可能性があるため、照合速度を上げることができます。
- `collate`:ロケールに依存する照合順序を使用します (たとえば、"[a-z]" という形式の範囲)。

0個以上のフラグを文法と組み合わせて、正規表現エンジンの動作を指定できます。 フラグだけを指定した`ECMAScript`場合、は文法と見なされます。
...
*肯定アサート*、*否定アサート*、*アンカー*の要素が繰り返し数を配置できます。 最も一般的な種類の繰り返し数は、"{`min`,`max`}" の形式になります。または、`basic` および `grep` では、"\\{`min`,`max`\\}" の形式になります。 後にこの形式の繰り返し数が続く要素は、要素に一致するシーケンスが少なくとも `min` 回連続して発生し、最大で `max` 回連続して発生する場合に一致します。 たとえば、"a{2,3}" は、ターゲットシーケンス "aa" およびターゲットシーケンス "aaa" と一致しますが、ターゲットシーケンス "a" やターゲットシーケンス "aaaa" とは一致しません。
...
- "a{2}" は、ターゲットシーケンス "aa" と一致しますが、ターゲットシーケンス "a" やターゲットシーケンス "aaa" とは一致しません。

- "a{2,}" は、ターゲットシーケンス "aa"、ターゲットシーケンス "aaa" などと一致しますが、ターゲットシーケンス "a" とは一致しません。
...
で`ECMAScript`は、繰り返し回数の後に文字 "?" を使用して、*最短一致の繰り返し*を指定できます。
...
- "a\\\*" はターゲットシーケンス "aaa" と一致しませんが、ターゲットシーケンス "a\*" と一致します。
...
- '\*' は、正規表現内の最初の文字であるか、正規表現内の最初の ' ^ ' に続く最初の文字であるか、またはキャプチャグループの最初の文字または最初の文字である場合を除き、すべての場合に特別な意味を持ちます。キャプチャグループの先頭の ' ^ ' に従います。
...
|"$$"||"$"|
||"\\&"|"&"|
|"$\`" (ドル記号とそれに続くバッククォート)||正規表現と一致するサブシーケンスの直前の文字シーケンス (`[match.prefix().first, match.prefix().second)`)|
|"$'" (ドル記号とそれに続く単一引用符)||正規表現と一致するサブシーケンスの直後の文字シーケンス (`[match.suffix().first, match.suffix().second)`)|
|"$n"|"\n"|位置`n`でキャプチャ`[match[n].first, match[n].second)`グループと一致する文字シーケンス。は0から9()までの数値です。`n`|
||"\\\n"|"\n"|
|"$nn"||位置`nn`でキャプチャグループと一致する文字シーケンス。ここ`nn`で、は 10 ~ 99 (`[match[nn].first, match[nn].second)`) の数値です。|
