...
- `optimize`: 可能なコストの大きい構築時に高速で一致するようにします。
- `collate`: ロケールに依存する照合順序 (たとえば、フォーム"[a ~ z]"の範囲) を使用します。
...
- "a \b"と表示されます。 ターゲット シーケンスと一致する"、~"、ターゲット シーケンス"ab"とは一致しません。
...
|"$$"||"$"|
||"\\&"|"&"|
|"$\`"(ドル記号に続く逆引用符)||正規表現と一致するサブシーケンスの直前の文字シーケンス (`[match.prefix().first, match.prefix().second)`)|
