...
`atomic_fetch_or_explicit` 関数は、`read-modify-write` 操作を実行して、指定された[memory_order](../standard-library/atomic-enums.md#memory_order_enum)の制約内で、 *atom*の格納さ*れている*値をビットごとの `or`*値*および*atom*に格納されている現在の値に置き換えます。
