---
description: '詳細情報: extreme_value_distribution クラス'
...
*URNG*\
乱数ジェネレーターエンジン。 使用できる型については、「」を参照してください [\<random>](../standard-library/random.md) 。

## <a name="remarks"></a>解説

クラステンプレートは、極端な値分布に従って分布した、ユーザー指定の浮動小数点型の値または型の値 (指定がない場合) を生成する分布を表し **`double`** ます。 次の表は、個々のメンバーに関する記事にリンクしています。
