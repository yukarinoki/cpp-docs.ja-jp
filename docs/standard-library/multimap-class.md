...
|[ポインター (pointer)](#pointer)|内の要素へのポインターを提供する型 **`const`** `multimap` 。|
...
|[](#contains)<sup>c++ 20</sup>を含む|指定したキーを持つ要素が内に存在するかどうかを確認し `multimap` ます。|
...
Multimap *m* では、2つの要素 *e1*(*k1*、 *d1*) と *e2*(*k2*、 *d2*) が型のオブジェクト `value_type` であり、 *k1* と *k2* が型のキーであり、d1 と d2 が型のデータである場合、 `key_type` はと  `mapped_type` `m.value_comp(e1, e2)` 同じに `m.key_comp(k1, k2)` なります。
