...
メンバー関数は **`true`** 、メンバーの以前の使用では、 `operator=` **subf**_-> の呼び出しが eof を返した場合、を返し `sputc` ます。 **eof**
...
この演算子は、出力反復子式 i x でのみ機能し、 \* *i*  =  *x*ストリームバッファーに文字を出力します。 Ostreambuf 反復子に適用され、反復子を返します。** \* iter**は**iter**を返します。
...
演算子は、出力反復子式 i x を実装するために使用され \* *i*  =  *x*ます。
...
出力 \* *i*  =  ストリームに書き込むための出力反復子式 i*x*を実装するために使用される代入演算子。
