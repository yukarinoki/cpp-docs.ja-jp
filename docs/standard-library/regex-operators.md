...
型がクラステンプレートの特殊化である `match_results` か、または `sub_match` メンバー関数を呼び出すことによって変換される引数 `str` 。

型がクラステンプレートの特殊化である引数 `basic_string` は変更されません。

その他のすべての引数の型は、クラステンプレートの適切な特殊化のために引数の値をコンストラクターに渡すことによって変換され `basic_string` ます。
