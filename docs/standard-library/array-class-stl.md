...
コピーコンストラクターは、 `array(const array& right)` シーケンス [*right* `.begin()` , *right*) を使用して被制御シーケンスを初期化し `.end()` ます。 これを使用して、配列オブジェクト *right* によって制御されるシーケンスのコピーである最初の被制御シーケンスを指定します。
