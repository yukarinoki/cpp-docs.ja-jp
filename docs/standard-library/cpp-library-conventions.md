...
- 標準 C ライブラリの関数の名前には、extern "C++" または extern "C" リンケージを含めることができます。 ライブラリ エンティティをインラインで宣言するのではなく、適切な標準 C ヘッダーをインクルードします。
