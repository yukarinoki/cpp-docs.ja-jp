...
str で始まるシーケンスの長さ N = `traits_type::` [長さ](../standard-library/char-traits-struct.md#length)() を決定 `str` し、シーケンスを挿入します。  N < `_Ostr.`[width](../standard-library/ios-base-class.md#width) の場合は、関数はまた `_Ostr.width` - N 充填文字の繰り返しを挿入します。 (の場合、繰り返しはシーケンスの前に置かれ `_Ostr` ます。 [](../standard-library/ios-base-class.md#flags)  &  フラグ `adjustfield`! = [left](../standard-library/ios-functions.md#left)。 それ以外の場合、繰り返しはシーケンスに後続します。 関数は *_Ostr* を返します。
...
ただし、 *str* で始まるシーケンスの各要素 *_Ch* は、 `Elem` `_Ostr.` [put](../standard-library/basic-ostream-class.md#put)(拡張 `_Ostr.` [](../standard-library/basic-ios-class.md#widen)()) を呼び出すことによって、型のオブジェクトに変換され `_Ch` ます。
