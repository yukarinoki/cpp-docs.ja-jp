---
description: '詳細情報: exponential_distribution クラス'
...
クラステンプレートは、指数分布に従って分布した、ユーザー指定の整数型の値または型の値 (指定がない場合) を生成する分布を表し **`double`** ます。 次の表は、個々のメンバーに関する記事にリンクしています。
