...
|[int_type](#int_type)|この型を`basic_filebuf`*、Tr*スコープ内の同じ名前の型と同じスコープ内にします。|
|[off_type](#off_type)|この型を`basic_filebuf`*、Tr*スコープ内の同じ名前の型と同じスコープ内にします。|
|[pos_type](#pos_type)|この型を`basic_filebuf`*、Tr*スコープ内の同じ名前の型と同じスコープ内にします。|
...
|swap||
|[アンダー フロー](#underflow)|入力ストリームから現在の要素を抽出するプロテクト仮想メンバー関数。|
...
最初のコンストラクターは、入力バッファーと出力バッファーを制御するすべてのポインターに Null ポインターを格納します。 詳細については、[basic_streambuf クラス](../standard-library/basic-streambuf-class.md)の「コメント」セクションを参照してください。 また *、_Mode*を文字列ビュー モードとして格納します。 詳細については、[basic_stringbuf クラス](../standard-library/basic-stringbuf-class.md)の「コメント」セクションを参照してください。

2 番目のコンストラクターは、文字列オブジェクト*str*によって制御されるシーケンスのコピーを割り当てます。 `_Mode & ios_base::in` が 0 以外の場合、シーケンスの先頭から読み取りを開始するように入力バッファーを設定します。 `_Mode & ios_base::out` が 0 以外の場合、シーケンスの先頭から書き込みを開始するように出力バッファーを設定します。 また *、_Mode*を文字列ビュー モードとして格納します。 詳細については、[basic_stringbuf クラス](../standard-library/basic-stringbuf-class.md)の「コメント」セクションを参照してください。
...
この型をbasic_filebufのスコープ内で、スコープ内の同じ名前の型と`Tr`同じものにします。
...
## <a name="basic_stringbufoverflow"></a><a name="overflow"></a>basic_stringbuf::オーバーフロー
...
* \_Meta*が**traits_type::**[eof](../standard-library/char-traits-struct.md#eof)と等しくない場合、プロテクト仮想メンバー関数は、要素**traits_type::**[to_char_type](../standard-library/char-traits-struct.md#to_char_type)(*\_Meta*) を出力バッファーに挿入しようとします。 これはさまざまな方法で行うことができます。
...
## <a name="basic_stringbufpbackfail"></a><a name="pbackfail"></a>basic_stringbuf::pバックフェイル
...
*_Meta*が**traits_type::**[eof](../standard-library/char-traits-struct.md#eof)と等しい場合、プッシュバックする要素は、事実上、現在の要素の前に既にストリーム内にある要素になります。 それ以外の場合、その要素は**バイト** = **traits_type:**[to_char_type](../standard-library/char-traits-struct.md#to_char_type)(_ *Meta*) に置き換えられます。 この関数は、さまざまな方法で要素を戻すことができます。
...
## <a name="basic_stringbufseekoff"></a><a name="seekoff"></a>basic_stringbuf::シークオフ
...
*_Off*\
*_Way*に対して相対的に求める位置。 詳細については、「[basic_stringbuf::off_type](#off_type)」を参照してください。
...
- の`_Way` == `ios_base::beg`場合、新しい位置はストリームの先頭に *_Off*を加えたものになります。

- の`_Way` == `ios_base::cur`場合、新しい位置は現在のストリーム位置*に _Off*を加えた位置になります。

- の`_Way` == `ios_base::end`場合、新しい位置はストリームの末尾にストリームを加えた *_Off。*

`_Mode & ios_base::in` が 0 以外の場合、関数は入力バッファー内の次に読み取る位置を変更します。 `_Mode & ios_base::out` が 0 以外の場合、関数は出力バッファー内の次に書き込む位置を変更します。 影響を受けるストリームの場合、そのバッファーが存在する必要があります。 配置操作を正常に完了するためには、結果のストリームの位置が被制御シーケンス内になければなりません。 関数が両方のストリーム位置に影響を与える`ios_base::beg`場合`ios_base::end`*、_Way*は、または両方のストリームが同じ要素に配置されている必要があります。 それ以外の場合 (またはどちらも位置も影響を受けない場合)、配置操作は失敗します。

関数がいずれかまたは両方のストリームの位置の変更に成功すると、結果のストリームの位置が返されます。 それ以外の場合は、失敗し、無効なストリームの位置が返されます。

## <a name="basic_stringbufseekpos"></a><a name="seekpos"></a>basic_stringbuf::シーポス
...
2 番目のメンバー関数は、**\*現在 this**によって制御されているシーケンスを割り当て解除します。 次に *、_Newstr*によって制御されるシーケンスのコピーを割り当てます。 **mode & ios_base::in** が 0 以外の場合、シーケンスの先頭から読み取りを開始するように入力バッファーを設定します。 **mode & ios_base::out** が 0 以外の場合、シーケンスの先頭から書き込みを開始するように出力バッファーを設定します。
...
関数が成功できない場合は **、traits_type:eof**[eof](../standard-library/char-traits-struct.md#eof)を返します。 それ以外の場合、変換された入力ストリームの現在の要素が返されます。

### <a name="remarks"></a>解説

プロテクト仮想メンバー関数は、入力バッファーから現在`byte`の要素を抽出し、現在のストリーム位置を進め、traits_type:: **traits_type::**[to_int_type](../standard-library/char-traits-struct.md#to_int_type)( **byte**) として要素を返そうとします。 これは、ある方法で行うことができます:読み取り位置が使用可能な場合`byte`、読み取り位置に格納されている要素として受け取り、入力バッファの次のポインタを進めます。
...
## <a name="basic_stringbufoperator"></a><a name="op_eq"></a>basic_stringbuf::演算子=
