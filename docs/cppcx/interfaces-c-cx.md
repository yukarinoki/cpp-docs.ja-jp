...
- プロパティ、メソッド パラメーター、または戻り値として使用される型は、Windows ランタイム型にする場合に限られます。これには、基本型と列挙型クラス型が含まれます。
...
- template 宣言された ref クラスは、プライベートである必要があります。 汎用インターフェイスを実装でき、テンプレート パラメータ*T*をジェネリック引数*T*に渡すことができます。テンプレート化 ref クラスのインスタンス化は、それ自体が ref クラスです。
