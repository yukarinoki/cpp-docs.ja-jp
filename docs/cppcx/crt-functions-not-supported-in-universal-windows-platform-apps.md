...
ユニバーサル Windows プラットフォーム (UWP) アプリをビルドする場合、多くの C ランタイム (CRT) 関数は使用できません。 場合によっては、Windows ランタイムまたは Win32 Api を使用するなどの回避策を使用できます。 それ以外の場合は、対応する機能またはサポート Api が UWP アプリに適用されないため、CRT 関数が禁止されています。 Windows ランタイムでサポートされている別の方法を探すには、「 [UWP アプリの Windows api に代わる](/uwp/win32-and-com/alternatives-to-windows-apis-uwp)方法」を参照してください。

次の表は、UWP アプリをビルドするときに使用できない CRT 関数の一覧です。 これは、適用されるすべての回避策を示します。
...
|`getpid` `_getpid` | これらは古い関数です。|Win32 API `GetCurrentProcessId`を使用します。|
