...
Windows ランタイムは、コレクションおよび関連する型のインターフェイスを定義し、C++/CX はコレクション .h ヘッダーファイル内の具体的な C++ 実装を提供します。 この図は、コレクション型間のリレーションシップを示しています。

![コレクション型の C&#43;&#43;&#47;CX 継承ツリー](../cppcx/media/cppcxcollectionsinheritancetree.png "コレクション型の C&#43;&#43;&#47;CX 継承ツリー")
...
- C++/CX コレクション型は、STL コンテナーがサポートするのと同じスレッドセーフの保証をサポートしています。
...
クラスがシーケンスコンテナーを別の Windows ランタイムコンポーネントに渡す必要がある場合は、 [Windows:: Foundation:: collections:: \<T> ivector](/uwp/api/windows.foundation.collections.ivector-1)をパラメーターまたは戻り値の型として使用し、 [Platform:: collections:: Vector \<T> ](../cppcx/platform-collections-vector-class.md)を具象実装として使用します。 パブリックの戻り値またはパラメーターで `Vector` 型を使用しようとすると、コンパイラ エラー C3986 が発生します。 このエラーは、 `Vector` を `IVector`に変更することで解決できます。
...
を使用する既存のコードがあり、 `std::vector` それを Windows ランタイムコンポーネントで再利用する場合は、または反復子のペアを受け取るコンストラクターの1つを使用して、 `Vector` ABI を介してコレクションを `std::vector` `Vector` 渡すポイントにを構築します。 次の例は、 `Vector` からの効率的な初期化のために `std::vector`移動コンストラクターを使用する方法を示しています。 移動操作の後、元の `vec` 変数は有効でなくなります。
...
![VectorProxy for ループに基づく範囲&#45;](../cppcx/media/vectorproxy-1.png "VectorProxy for ループに基づく範囲&#45;")

プロキシオブジェクトをコーディングする必要があるシナリオの1つは、要素 **`dynamic_cast`** コレクションで特定の型の XAML オブジェクトを探しているときなど、要素に対してを実行する必要がある場合です `UIElement` 。 この場合は、最初に [Platform::Object](../cppcx/platform-object-class.md)^ に要素をキャストし、その後に動的キャストを実行する必要があります。
...
`Map` と `MapView` は、 `<K, V, C = std::less<K>>`で template 宣言されるので、比較子をカスタマイズできます。  さらに、 `Vector` と `VectorView` は `<T, E = std::equal_to<T>>` で template 宣言されるので、 `IndexOf()`の動作をカスタマイズできます。 これは、値の構造体の `Vector` と `VectorView` について特に重要です。 たとえば、Vector を作成するには、 \<Windows::Foundation::DateTime> カスタム比較子を指定する必要があります。これは、DateTime が = = 演算子をオーバーロードしないためです。
...
|Iterators|関数|
|---------------|---------------|
|[Platform:: Collections:: VectorIterator\<T>](../cppcx/platform-collections-vectoriterator-class.md)<br /><br /> ( [Windows:: Foundation:: Collections:: IVector \<T> ](/uwp/api/windows.foundation.collections.ivector-1)および int を内部に格納します)。|[開始](../cppcx/begin-function.md) / [end](../cppcx/end-function.md)([Windows:: Foundation:: Collections:: ivector \<T> ](/uwp/api/windows.foundation.collections.ivector-1))|
|[Platform:: Collections:: VectorViewIterator\<T>](../cppcx/platform-collections-vectorviewiterator-class.md)<br /><br /> ( [IVectorView \<T> ](/uwp/api/windows.foundation.collections.ivectorview-1)^ と int を内部に格納します)。|[開始](../cppcx/begin-function.md) / [end](../cppcx/end-function.md) ([IVectorView \<T> ](/uwp/api/windows.foundation.collections.ivectorview-1)^)|
|[Platform:: Collections:: InputIterator\<T>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> ( [Iiterator \<T> ](/uwp/api/windows.foundation.collections.iiterator-1)^ と T を内部に格納します)。|[開始](../cppcx/begin-function.md) / [end](../cppcx/end-function.md) ([IIterable \<T> ](/uwp/api/windows.foundation.collections.iiterable-1))|
|[Platform:: Collections:: InputIterator<Ikeyvaluepair<k,\<K, V>^>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> ( [Iiterator \<T> ](/uwp/api/windows.foundation.collections.iiterator-1)^ と T を内部に格納します)。|[開始](../cppcx/begin-function.md) / [終了](../cppcx/end-function.md)([IMap \<K,V> ](/uwp/api/windows.foundation.collections.imap-2)。|
|[Platform:: Collections:: InputIterator<Ikeyvaluepair<k,\<K, V>^>](../cppcx/platform-collections-inputiterator-class.md)<br /><br /> ( [Iiterator \<T> ](/uwp/api/windows.foundation.collections.iiterator-1)^ と T を内部に格納します)。|[開始](../cppcx/begin-function.md) / [end](../cppcx/end-function.md) ([Windows:: Foundation:: Collections:: IMapView](/uwp/api/windows.foundation.collections.imapview-2))|
