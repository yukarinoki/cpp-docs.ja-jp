...
Unicode 文字の配列の先頭へのポインター `const char16` ( `char16` はの typedef **`wchar_t`** )。
...
**`true`**`str`が現在のオブジェクトと等しい場合は。それ以外の場合は **`false`** 。
...
**`true`** 現在のオブジェクトが高速である場合は `String` 。それ以外の場合は **`false`** 。
...
**`true`** の内容 `str1` がと等しい場合は `str2` 。それ以外の場合は **`false`** 。
...
**`true`** の値 `str1` がの値より大きい場合は `str2` 。それ以外の場合は **`false`** 。
