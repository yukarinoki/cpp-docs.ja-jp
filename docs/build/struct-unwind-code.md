 ...
 **アンワインド コードの操作**  
 ...
 UWOP_SAVE_XMM128 と UWOP_SAVE_XMM128_FAR を除くすべてのオペレーション コードは、オフセットは必ず、8 の倍数 (スタック自体は 16 バイトでアラインでは常に) 8 バイト境界に関心のあるすべてのスタックの値が格納されるためです。 短いオフセット (512 K 未満) を使用するオペレーション コード、このコードのノードの最後の USHORT は 8 で割った値のオフセットを保持します。 時間のオフセットを使用するオペレーション コードを (512 K < = < 4 GB のオフセット)、このコードの最後の 2 つ USHORT ノード (リトル エンディアン形式) でのオフセットを保持します。  
  
 オペコード UWOP_SAVE_XMM128 と UWOP_SAVE_XMM128_FAR、オフセットは常に 16 の倍数でために 128 ビットのすべての XMM 操作は、16 バイトのアラインされたメモリで発生する必要があります。 そのため、16 のスケール ファクターより小さい 1 m 分のオフセットが許可 UWOP_SAVE_XMM128 に使用されます。  
  
 アンワインド操作コードは、次のいずれかです。  
  
 UWOP_PUSH_NONVOL (0) 1 つのノード  
  
 不揮発性の整数レジスタ、8 でデクリメント RSP をプッシュします。 操作情報は、レジスタの数です。 エピローグ上の制約のため UWOP_PUSH_NONVOL アンワインド コードする必要があります、プロローグ内の先頭に注意してください同様に、アンワインド コード配列の最後します。 この相対順序は、UWOP_PUSH_MACHFRAME を除くその他のすべてのアンワインド コードに適用されます。  
...
|RSP|エラー コード|  
 ...
 このアンワインド コードは、これが実際に実行されるが、代わりに、割り込みルーチンの実際のエントリ ポイントの前に表示され、マシン フレームのプッシュをシミュレートする場所を提供することのみが存在するダミーのプロローグで常に表示されます。 UWOP_PUSH_MACHFRAME を記録するシミュレーションでは、コンピューターが、次の概念的にが実行を示します。  
  
 RIP のリターン アドレスにスタックの一番上からのポップ*Temp*  
 ...
 (Op 情報では、1 と等しい) 場合は、エラー コードをプッシュします。  
  
 40 でシミュレートされた UWOP_PUSH_MACHFRAME 操作デクリメント RSP (op 情報は、0 に等しい) または 48 (op 情報が 1 と等しくなります)。  
