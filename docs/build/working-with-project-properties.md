...
IDE では、プロジェクトをビルドするために必要なすべての情報は "*プロパティ*" として公開されます。 この情報には、アプリケーションの名前、拡張子 (DLL、LIB、EXE)、コンパイラ オプション、リンカー オプション、デバッガーの設定、カスタム ビルド ステップ、その他多くの情報が含まれます。 通常、*プロパティページ*を使用して、これらのプロパティを表示および変更します。 プロパティページにアクセスするには、メインメニューから [**プロジェクト** >  **_プロジェクト名_ のプロパティ**] を選択するか、**ソリューションエクスプローラー**でプロジェクトノードを右クリックし、 **[プロパティ]** を選択します。
