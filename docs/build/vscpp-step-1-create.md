...
Visual Studioで新しい空のプロジェクトを作成し、ソースコードファイルに追加するアプリケーションの種類に特殊化する準備をします。 その後、次のことを行います。
...
1. **ソリューション エクスプ ローラー**で、HelloWorld プロジェクトを選択します。 メニューバー上で、次のように選択します。**プロジェクト**、**新しい項目の追加** と順に選択すると、**新しい項目の追加** ダイアログが開きます。 
...
> [C++ プロジェクト のビルドおよび実行](vscpp-step-2-build.md)
