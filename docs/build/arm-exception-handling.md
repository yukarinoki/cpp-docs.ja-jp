...
|ワード オフセット|Bits|目的|
|-----------------|----------|-------------|
|0|0-31|*関数の開始 rva*は、関数の開始の32ビット rva です。 関数に Thumb コードが含まれている場合は、このアドレスの下位ビットを設定する必要があります。|
|1|0-1|*Flag*は、2番目の pdata の残りの30ビットを解釈する方法を示す2ビットフィールドです。 *フラグ*が0の場合、残りのビットは*例外情報 RVA*を形成します (下位2ビットが暗黙的に0になります)。 *フラグ*が0以外の場合、残りのビットは、パックされた*アンワインドデータ*構造体を形成します。|
|1|2-31|*例外情報の RVA*または*パックアンワインドデータ*。<br /><br /> *例外情報 RVA*は、.xdata セクションに格納されている可変長例外情報構造体のアドレスです。 このデータは、4 バイトでアラインされている必要があります。<br /><br /> パックされた*アンワインドデータ*は、正規の形式を想定して、関数からのアンワインドに必要な操作の圧縮された説明です。 この場合、.xdata レコードは必要ありません。|
...
|-----------------|----------|-------------|
|0|0-31|*関数の開始 rva*は、関数の開始の32ビット rva です。 関数に Thumb コードが含まれている場合は、このアドレスの下位ビットを設定する必要があります。|
|1|0-1|*フラグ*は、次の意味を持つ2ビットフィールドです。<br /><br />-00 = パックされたアンワインドデータは使用されません。残りのビットは、.xdata レコードを指します。<br />-01 = パックしたアンワインドデータ。<br />-10 = パックされたアンワインドデータ。関数にプロローグがないと見なされます。 関数の開始から分離している関数フラグメントの記述に役立ちます。<br />-11 = 予約済み。|
|1|2-12|*関数の長さ*は、関数全体の長さを2で割った値をバイト単位で提供する11ビットのフィールドです。 関数が 4KB より大きい場合、フル .xdata レコードを代わりに使用する必要があります。|
|1|13-14|*Ret*は、関数がを返す方法を示す2ビットフィールドです。<br /><br />-00 = pop {pc} を使用して返します (この場合、 *L*フラグビットを1に設定する必要があります)。<br />-01 = 16 ビットの分岐を使用してを返します。<br />-10 = 32 ビットの分岐を使用して返されます。<br />-11 = エピローグがありません。 プロローグのみが含まれ、エピローグは他の場所にある分離された関数フラグメントの記述に役立ちます。|
|1|15|*H*は、関数が整数パラメーターレジスタ (r0-r3) を関数の先頭にプッシュすることによって "ホーム" にするかどうかを示す1ビットフラグです。この値を返す前に、スタックの16バイトを解放します。 (0 = レジスタを元に戻しません、1 = レジスタを元に戻します。)|
|1|16-18|*Reg*は、最後に保存された非 volatile レジスタのインデックスを示す3ビットフィールドです。 *R*ビットが0の場合は、整数レジスタのみが保存され、r4 の範囲内であると見なされます。ここで、N は 4 + *Reg*に相当します。*R*ビットが1の場合は、浮動小数点レジスタだけが保存されます。この場合、N は 8 + *Reg*に等しいと見なされます。*R* = 1 と*Reg* = 7 の特別な組み合わせにより、レジスタが保存されていないことが示されます。|
|1|19|*R*は、保存されていない不揮発性レジスタが整数レジスタ (0) または浮動小数点レジスタ (1) であるかどうかを示す1ビットフラグです。 *R*が1に設定され、 *Reg*フィールドが7に設定されている場合、非 volatile レジスタはプッシュされませんでした。|
|1|20|*L*は、関数が LR を保存/復元するかどうかを示す1ビットフラグであり、 *Reg*フィールドによって示される他のレジスタと共に使用されます。 (0 = 保存/復元を行わない、1 = 保存/復元を行う。)|
|1|21|*C*は、高速スタックウォーク (1) または not (0) のフレームチェーンを設定するための追加命令が関数に含まれているかどうかを示す1ビットフラグです。 このフラグが設定されている場合、保存された非 volatile 整数レジスタのリストに r11 が暗黙的に追加されています。 ( *C*フラグが使用されている場合は、以下の制限事項を参照してください)。|
|1|22-31|*Stack アジャスト*は、この関数に割り当てられているスタックのバイト数を4で割った値を示す10ビットフィールドです。 ただし、0x000 ～ 0x3F3 の間の値のみを直接エンコード可能です。 4,044 バイトを超えるスタックを割り当てる関数は、フル .xdata レコードを使用する必要があります。 *スタック調整*フィールドが0x3f4 以上の場合、下位4ビットには特別な意味があります。<br /><br />-ビット0-1 は、スタック調整の単語数 (1-4) から1を引いた値を示します。<br />プロローグがこの調整をプッシュ操作に結合した場合は、-Bit 2 が1に設定されます。<br />エピローグがこの調整を pop 操作に結合した場合、-Bit 3 は1に設定されます。|

上のエンコーディングに冗長性がある可能性があるため、次の制限が適用されます。

- *C*フラグが1に設定されている場合は、次のようになります。

   - フレームチェーンは r11 と LR の両方を必要とするため、 *L*フラグも1に設定する必要があります。
...
1. 例外データの後 (ヘッダーの*E*ビットが0に設定されている場合) は、エピローグスコープに関する情報の一覧です。このスコープは、1語にパックされ、開始オフセットが増加する順に格納されます。 各スコープには、次のフィールドが含まれます。
...
例外が関数本体のみで発生し、プロローグまたはエピローグでは絶対に発生しないことが保証されている場合、必要なアンワインド シーケンスは 1 つのみです。 ただし、Windows アンワインド モデルでは、部分的に実行されたプロローグまたはエピローグからアンワインドする機能が求められます。 この要件に対応するため、アンワインド コードはプロローグおよびエピローグ内の関連オペコードへの明確な 1 対 1 のマッピングを持つように慎重に設計されています。 これには複数の影響があります。
...
|80-BF|00-FF|||32|`pop   {r0-r12, lr}`<br /><br /> 対応するビットがコード & 0x1FFF で設定されている場合、LR が & ポップされます。|
|C0-CF||||16|`mov   sp,rX`<br /><br /> X はコード & 0x0F|
|D0-D7||||16|`pop   {r4-rX,lr}`<br /><br /> ここで X は (コード & 0x03) + 4、コード & 0x04 の場合、LR がポップされます。|
|D8-DF||||32|`pop   {r4-rX,lr}`<br /><br /> ここで X は (コード & 0x03) + 8 で、コード & 0x04 の場合は LR がポップされます。|
|E0-E7||||32|`vpop  {d8-dX}`<br /><br /> X は (Code & 0x07) + 8|
|E8-EB|00-FF|||32|`addw  sp,sp,#X`<br /><br /> X は (コード & 0x03FF) \* 4|
|EC-ED|00-FF|||16|`pop   {r0-r7,lr}`<br /><br /> 対応するビットがコード & 0x00FF で設定されている場合、LR & がポップされます。|
|EE|00-0F|||16|Microsoft 固有の仕様|
|EE|10-FF|||16|利用可能|
|EF|00-0F|||32|`ldr   lr,[sp],#X`<br /><br /> X は (コード & 0x000F) \* 4|
|EF|10-FF|||32|利用可能|
|F0-F4||||-|利用可能|
|F5|00-FF|||32|`vpop  {dS-dE}`<br /><br /> S は (Code & 0x00F0) > > 4 および E はコード & 0x000F です。|
|F6|00-FF|||32|`vpop  {dS-dE}`<br /><br /> S は ((コード & 0x00F0) > > 4) + 16 および E は (コード & 0x000F) + 16|
...
これは、アンワインドコード*コード*内の各バイトの16進値の範囲を示します。これには、オペコード Size *opsize*と、それに対応する元の命令解釈が含まれます。 空白のセルは、短いアンワインド コードを示しています。 複数のバイトをカバーする大きな値を持つ命令では、最上位ビットが最初に格納されます。 *Opsize*フィールドは、各 Thumb 2 操作に関連付けられた暗黙のオペコードサイズを示します。 表内の異なるエンコーディングの明らかな重複項目は、異なるオペコード サイズの区別に使用されます。
...
シュリンク ラップされた関数は、一般的には通常のプロローグ内に追加のレジスタ保存用領域を事前に割り当てて、次に `str` ではなく `stm` または `push` を使用してレジスタ保存を実行することが想定されています。 これにより、関数の元のプロローグでスタックポインターの操作がすべて維持されます。
...
- Word 0
   ...
   - *関数の開始 RVA* = 0x00088 c24 (= 0x00488C24-0x00400000)
