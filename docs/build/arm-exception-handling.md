...
ARM 上の Windows は *、アンワインド コード*を使用して[、構造化例外処理](/windows/win32/debug/structured-exception-handling)(SEH) の間にスタック アンワインドを制御します。 アンワインド コードは、実行可能イメージの .xdata セクションに格納されるバイトのシーケンスです。 関数のプロローグとエピローグ コードの操作を抽象的な方法で記述し、呼び出し元のスタック フレームに戻す準備として関数のプロローグの効果を元に戻すことができます。
...
ARM 版 Windows 用の実行可能イメージでは、移植可能な実行可能 (PE) 形式が使用されます。 詳細については、[マイクロソフトの PE および COFF 仕様](https://go.microsoft.com/fwlink/p/?linkid=84140)を参照してください。 例外処理情報は、イメージの .pdata および .xdata セクションに格納されます。

例外処理メカニズムでは、ARM 版 Windows の ABI に従うコードについて、次のことを想定しています。

- 関数の本体内で例外が発生した場合、プロローグの操作が取り消されるか、エピローグの操作が順方向に実行されるかは関係ありません。 両方の場合で同じ結果となります。
...
|ワード オフセット|Bits|目的|
|-----------------|----------|-------------|
|0|0-31|*関数開始 RVA*は、関数の開始の 32 ビット RVA です。 関数に Thumb コードが含まれている場合は、このアドレスの下位ビットを設定する必要があります。|
|1|0-1|*フラグ*は、2 番目の .pdata ワードの残りの 30 ビットを解釈する方法を示す 2 ビット フィールドです。 *Flag*が 0 の場合、残りのビットは*例外情報 RVA*を形成します (下位の 2 ビットは暗黙的に 0 になります)。 *Flag*が 0 以外の場合、残りのビットは*パックアンワインド データ*構造体を形成します。|
|1|2-31|*例外情報 RVA*または*パックアンワインド データ*。<br /><br /> *例外情報 RVA*は、.xdata セクションに格納されている可変長例外情報構造体のアドレスです。 このデータは、4 バイトでアラインされている必要があります。<br /><br /> *パックアンワインドデータ*は、正規形を想定して、関数からアンワインドするために必要な操作の圧縮された説明です。 この場合、.xdata レコードは必要ありません。|
...
|-----------------|----------|-------------|
|0|0-31|*関数開始 RVA*は、関数の開始の 32 ビット RVA です。 関数に Thumb コードが含まれている場合は、このアドレスの下位ビットを設定する必要があります。|
|1|0-1|*フラグ*は、次の意味を持つ 2 ビット フィールドです。<br /><br />- 00 = パックされたアンワインドデータは使用されません。残りのビットは .xdata レコードを指します。<br />- 01 = パックされたアンワインドデータ。<br />- 10 = 関数がプロローグを持たないと仮定される、パックされたアンワインドデータ。 関数の開始から分離している関数フラグメントの記述に役立ちます。<br />- 11 = 予約済み。|
|1|2-12|*関数長*は、11 ビットのフィールドで、関数全体の長さをバイト数で 2 で割った値で表します。 関数が 4KB より大きい場合、フル .xdata レコードを代わりに使用する必要があります。|
|1|13-14|*Ret*は、関数がどのように返されるかを示す 2 ビット フィールドです。<br /><br />- 00 = ポップ {pc} 経由で戻ります (この場合は*L*フラグ ビットを 1 に設定する必要があります)。<br />- 01 = 16 ビットブランチを使用して戻ります。<br />- 10 = 32 ビットブランチを使用して戻ります。<br />- 11 = エピローグはまったくありません。 プロローグのみが含まれ、エピローグは他の場所にある分離された関数フラグメントの記述に役立ちます。|
|1|15|*H*は、関数が整数パラメーター レジスタ (r0-r3) をプッシュして関数を "ホーム" するかどうかを示す 1 ビット フラグであり、戻る前にスタックの 16 バイトを割り当て解除します。 (0 = レジスタを元に戻しません、1 = レジスタを元に戻します。)|
|1|16-18|*Reg*は、最後に保存された不揮発性レジスタのインデックスを示す 3 ビット フィールドです。 *R*ビットが 0 の場合、整数レジスタのみが保存され、r4-rN の範囲内であると見な*されます。**R*ビットが 1 の場合、浮動小数点レジスタのみが保存され、d8-dN の範囲内にあると見な*されます。**R* = 1 と*Reg* = 7 の特別な組み合わせは、レジスタが保存されなくて済むことを示します。|
|1|19|*R*は、保存された不揮発性レジスタが整数レジスタ (0) であるか、浮動小数点レジスタ (1) であるかを示す 1 ビット フラグです。 *R*が 1 に設定され *、Reg*フィールドが 7 に設定されている場合、不揮発性レジスタはプッシュされませんでした。|
|1|20|*L*は、関数が LR を保存/復元するかどうか *、Reg*フィールドで示される他のレジスタと共に示す 1 ビット フラグです。 (0 = 保存/復元を行わない、1 = 保存/復元を行う。)|
|1|21|*C*は、高速スタック ウォーク (1) または設定しない (0) のフレーム チェーンを設定する追加の命令を関数に含めるかどうかを示す 1 ビット フラグです。 このフラグが設定されている場合、保存された非 volatile 整数レジスタのリストに r11 が暗黙的に追加されています。 (C フラグを使用する*C*場合は、以下の制限を参照してください。|
|1|22-31|*スタック調整*は、この関数に割り当てられているスタックのバイト数を 4 で割った 10 ビットのフィールドです。 ただし、0x000 ～ 0x3F3 の間の値のみを直接エンコード可能です。 4,044 バイトを超えるスタックを割り当てる関数は、フル .xdata レコードを使用する必要があります。 *スタック調整*フィールドが 0x3F4 以上の場合、下位 4 ビットには特別な意味があります。<br /><br />- ビット 0 から 1 は、スタック調整 (1 - 4) から 1 を引いたワード数を示します。<br />- プロローグがこの調整をプッシュ操作に組み合わせた場合、ビット 2 は 1 に設定されます。<br />- この調整をポップ操作にエピローグが組み合わせた場合、ビット 3 は 1 に設定されます。|

上のエンコーディングに冗長性がある可能性があるため、次の制限が適用されます。

- *C*フラグが 1 に設定されている場合:

  - フレームチェーンには r11 と LR の両方が必要なため *、L*フラグも 1 に設定する必要があります。

  - *reg*で記述されるレジスタのセットに r11 を含める必要はありません。つまり、r4-r11 がプッシュされた場合 *、Reg*は r4-r10 のみを*C*記述する必要があります。

- *Ret*フィールドが 0 に設定されている場合 *、L*フラグは 1 に設定する必要があります。

これらの制限に違反すると、サポートされないシーケンスが発生します。

以下の説明の目的のために、2 つの擬似フラグは*スタック調整*から派生します。

- *PF*または「プロローグ折りたたみ」は *、スタック調整*が 0x3F4 以上であり、ビット 2 が設定されていることを示します。

- *EF*または 「エピローグ折りたたみ」は *、スタック調整*が 0x3F4 以上であり、ビット 3 が設定されていることを示します。
...
|3a|*C*==1 と (*L*==0 および*R*==1 および PF==0)|16|`mov r11,sp`|C0-CF/FB|
|3b|*C*==1 および (*L*==1 または*R*==0 または PF=1)|32|`add r11,sp,#xx`|FC|
|4|*R*==1 と*Reg* != 7|32|`vpush {d8-dE}`|E0-E7|
|5|*スタック調整*!= 0 と PF==0|16/32|`sub sp,sp,#xx`|00-7F/E8-EB|

*H*ビットが 1 に設定されている場合、命令 1 は常に存在します。

フレームチェーンを設定するには *、C*ビットが設定されている場合、命令3aまたは3bのいずれかが存在する。 r11 および LR 以外のレジスタがプッシュされていない場合、16 ビット `mov` となり、その他の場合は 32 ビット `add` となります。

折りたたまれない調整が指定された場合、命令 5 は明示的なスタック調整です。

命令 2 および 4 は、プッシュが必要かどうかに基づいて設定されます。 次の表は *、C*、 *L*、 R 、*および* *PF*フィールドに基づいて保管されるレジスターを要約したものです。 すべての場合において *、N*は*Reg* + *4、E*は*Reg* + *8、S*は (~*スタック調整*) & 3 と等しくなります。
...
|-----------------|-----------------------------------|----------|------------|
|6|*スタック調整*!=0 と*EF*==0|16/32|`add   sp,sp,#xx`|
|7|*R*==1 と*Reg!=7*|32|`vpop  {d8-dE}`|
|8|*C*==1 または *(L*==1 と*H*==0)または*R==0*または*EF*==1|16/32|`pop   {registers}`|
|9a|*H*==1 と*L*==0|16|`add   sp,sp,#0x10`|
|9b|*H*==1 および*L*==1|32|`ldr   pc,[sp],#0x14`|
|10a|*Ret*==1|16|`bx    reg`|
|10b|*Ret*==2|32|`b     address`|

折りたたまれない調整が指定された場合、命令 6 は明示的なスタック調整です。 *PF*は*EF*から独立しているので、命令6なしで、または逆の命令5を持つことができます。

命令 7 と 8 は、プロローグと同じロジックを使用して、スタックから復元されるレジスタを判別しますが、この 2 つの変更を行う場合は、*まず、EF*が PF の代わりに使用されます。 *EF*第 2 に *、Ret* = 0 の場合、LR はレジスター・リストの PC に置き換えられ、エピローグは直ちに終了します。

*H*が設定されている場合は、命令 9a または 9b のいずれかが存在します。 L*が*0 の場合、LR がスタック上にないことを示すために、命令 9a が使用されます。 この場合、スタックは手動で調整され *、Ret*は明示的な戻り値を指定するために 1 または 2 である必要があります。 命令 9b は *、L*が 1 の場合に使用され、エピローグの早期終了を示し、スタックを戻して同時に調整します。

エピローグがまだ終了していない場合は *、Ret*の値に基づいて 16 ビットまたは 32 ビットの分岐を示す命令 10a または 10b のいずれかが存在します。
...
1. .xdata 構造体全体のサイズを記述し、主な関数データを提供する 1 ～ 2 ワードのヘッダー。 2 番目の単語は、*エピローグ数*と*コードワード*の両方のフィールドが 0 に設定されている場合にのみ存在します。 次の表で、各フィールドについて詳細に説明します。

   |Word|Bits|目的|
   |----------|----------|-------------|
   |0|0-17|*関数長*は、関数の全長をバイト数で 2 で割った値を示す 18 ビット・フィールドです。 関数が 512 KB より大きい場合、複数の .pdata および .xdata レコードを使用して関数を記述する必要があります。 詳細については、このドキュメントの「大きな関数」セクションを参照してください。|
   |0|18-19|** は、残りの .xdata のバージョンを示す2ビットフィールドです。 現在、バージョン 0 のみが定義されています。値 1 ～ 3 は予約済みです。|
   |0|20|*X*は、例外データの存在 (1) または不在 (0) を示す 1 ビット・フィールドです。|
   |0|21|*E*は、1 つのエピローグを記述する情報が、後でスコープワードを追加する必要なくヘッダー (1) にパックされることを示す 1 ビット・フィールドです (0)。|
   |0|22|*F*は、このレコードが関数フラグメント (1) または全関数 (0) を記述していることを示す 1 ビット・フィールドです。 フラグメントは、プロローグが存在せず、すべてのプロローグ処理が無視されることを意味します。|
   |0|23-27|*エピローグカウント*は *、E*ビットの状態に応じて、2 つの意味を持つ 5 ビットフィールドです。<br /><br /> - *E*が 0 の場合、このフィールドはセクション 3 で説明されている例外スコープの合計数です。 関数内に 31 を超えるスコープが存在する場合、このフィールドと*コードワード*フィールドの両方を 0 に設定して、拡張ワードが必要であることを示す必要があります。<br />- *E*が 1 の場合、このフィールドは、唯一のエピローグを記述する最初のアンワインド コードのインデックスを指定します。|
   |0|28-31|*コードワード*は、セクション 4 のすべてのアンワインドコードを含むために必要な 32 ビットワードの数を指定する 4 ビットフィールドです。 63 を超えるアンワインド コード バイトに 15 ワード以上が必要な場合、このフィールドと*Epilogue Count*フィールドの両方を 0 に設定して、拡張ワードが必要であることを示す必要があります。|
   |1|0-15|*拡張エピローグ カウント*は、異常に多数のエピローグをエンコードするためのより多くの領域を提供する 16 ビット フィールドです。 このフィールドを含む拡張ワードは、最初のヘッダーワードの*エピローグ数*フィールドと*コードワード*フィールドの両方が 0 に設定されている場合にのみ存在します。|
   |1|16-23|*拡張コード ワード*は 8 ビット のフィールドで、非常に大量のアンワインド コード ワードをエンコードするための領域を広げます。 このフィールドを含む拡張ワードは、最初のヘッダーワードの*エピローグ数*フィールドと*コードワード*フィールドの両方が 0 に設定されている場合にのみ存在します。|
   |1|24-31|予約済み|

1. 例外データの後 (ヘッダーの*E*ビットが 0 に設定されている場合)、エピローグ スコープに関する情報のリストが、単語に 1 つパックされ、開始オフセットを増加する順序で格納されます。 各スコープには、次のフィールドが含まれます。

   |Bits|目的|
   |----------|-------------|
   |0-17|*エピローグ開始オフセット*は、関数の開始に対する 2 で割った、エピローグのオフセットをバイト数で表す 18 ビットのフィールドです。|
   |18-19|*Res*は将来の拡張のために予約された 2 ビット・フィールドです。 この値は 0 である必要があります。|
   |20-23|*Condition*は、エピローグが実行される条件を示す 4 ビット・フィールドです。 条件を伴わないエピローグの場合、このフィールドは 0xE、つまり "常時" に設定する必要があります。 (エピローグは、全体が条件付きであるか全体が条件なしである必要があります。また、Thumb-2 モードでは、エピローグは IT オペコードの後の最初の命令で開始します。)|
   |24-31|*エピローグ開始インデックス*は、このエピローグを記述する最初のアンワインド コードのバイト インデックスを示す 8 ビット フィールドです。|
...
1. ヘッダーの*X*フィールドが 1 の場合、アンワインド コードのバイトの後に例外ハンドラー情報が続きます。 これは、例外ハンドラーのアドレスを含む 1 つの*例外ハンドラー RVA*で構成され、その直後に例外ハンドラーで必要なデータ量 (可変長) が続きます。
...
プロローグと各エピローグにはアンワインド コードへのインデックスがありますが、テーブルはそれらの間で共有されます。 これらすべてが同じアンワインド コードを共有できることは、ほとんどありません。 コンパイラの作成者がこの状況に合わせて最適化することをお勧めします。これは、指定可能な最も大きいインデックスは 255 であり、このことによって、特定の関数に対して許容されるアンワインド コードの合計数が制限されるためです。
...
|------------|------------|------------|------------|------------|-----------------|
|00-7F||||16|`add   sp,sp,#X`<br /><br /> X は (コード & 0x7F) \* 4|
|80-BF|00-FF|||32|`pop   {r0-r12, lr}`<br /><br /> コード & 0x2000 および r0-r12 がコード & 0x1FFF に設定されている場合にポップされると LR がポップされます。|
|C0-CF||||16|`mov   sp,rX`<br /><br /> ここで X はコード & 0x0F です|
|D0-D7||||16|`pop   {r4-rX,lr}`<br /><br /> ここで X は (コード & 0x03) + 4、コードが 0x04 の場合は LR がポップ&|
|D8-DF||||32|`pop   {r4-rX,lr}`<br /><br /> ここで X は (コード & 0x03) + 8、コードが 0x04 の場合は LR がポップ&|
|E0-E7||||32|`vpop  {d8-dX}`<br /><br /> X は (コード & 0x07) + 8|
|E8-EB|00-FF|||32|`addw  sp,sp,#X`<br /><br /> X は (コード & 0x03FF) \* 4|
|EC-ED|00-FF|||16|`pop   {r0-r7,lr}`<br /><br /> コード & 0x0100 および r0-r7 がコード & 0x00FF に設定されている場合に、コードがポップされると LR がポップされます。|
|EE|00-0F|||16|Microsoft 固有の仕様|
|EE|10-FF|||16|使用可能|
|EF|00-0F|||32|`ldr   lr,[sp],#X`<br /><br /> X は (コード & 0x000F) \* 4|
|EF|10-FF|||32|使用可能|
|F0-F4||||-|使用可能|
|F5|00-FF|||32|`vpop  {dS-dE}`<br /><br /> S は (コード & 0x00F0)  >> 4、E はコード & 0x000F です。|
|F6|00-FF|||32|`vpop  {dS-dE}`<br /><br /> ここで S は ((コード & 0x00F0) >> 4) + 16、E は (コード & 0x000F) + 16|
|F7|00-FF|00-FF||16|`add   sp,sp,#X`<br /><br /> X は (コード & 0x00FFFF) \* 4|
|F8|00-FF|00-FF|00-FF|16|`add   sp,sp,#X`<br /><br /> X は (コード & 0x00FFFFFF) \* 4|
|F9|00-FF|00-FF||32|`add   sp,sp,#X`<br /><br /> X は (コード & 0x00FFFF) \* 4|
|FA|00-FF|00-FF|00-FF|32|`add   sp,sp,#X`<br /><br /> X は (コード & 0x00FFFFFF) \* 4|
|FB||||16|nop (16 ビット)|
|FC||||32|nop (32 ビット)|
...
これは、アンワインド コード*Code*の各バイトの 16 進値の範囲と、opcode*サイズ Opsize*と対応する元の命令の解釈を示します。 空白のセルは、短いアンワインド コードを示しています。 複数のバイトをカバーする大きな値を持つ命令では、最上位ビットが最初に格納されます。 *Opsize*フィールドには、各 Thumb-2 操作に関連付けられた暗黙的なオペコードサイズが表示されます。 表内の異なるエンコーディングの明らかな重複項目は、異なるオペコード サイズの区別に使用されます。
...
アンワインド コード 0xFD および 0xFE は、通常の終了コード 0xFF と同等ですが、エピローグの場合は 16 ビットまたは 32 ビットの特別な nop オペコードに相当します。 プロローグでは、コード 0xFD、0xFE および 0xFF は正確に同等です。 これは、同等のプロローグ命令を持たない`bx lr`一`b <tailcall-target>`般的なエピローグの終わりまたはを説明します。 これにより、アンワインド シーケンスをプロローグとエピローグの間で共有できる機会が増えます。
...
各オペコードの次には、この演算を記述する適切なアンワインド コードがあります。 プロローグのアンワインド コード シーケンスは、エピローグのアンワインド コードのミラー イメージになっています。 このケースは一般的であり、プロローグのアンワインド コードが常にプロローグの実行順序とは逆の順序で保存されると想定される理由です。 これにより、次のアンワインドコードの共通セットが提供されます。
...
この例では、プロローグとエピローグの間の関数本体の実行中に例外が発生した場合、アンワインドはエピローグの場合で、エピローグ コード内のオフセット 0 で開始されます。 これは、例のオフセット 0x140 に対応します。 クリーンアップは行われていないため、アンワインダーは、アンワインド シーケンス全体を実行します。 一方、例外がエピローグ コードの開始後の 1 つの命令で発生した場合、アンワインダーは最初のアンワインド コードをスキップして正常にアンワインドできます。 opcodes とアンワインド コードの間の 1 対 1 のマッピングを指定すると、エピローグ内の命令*n*からアンワインドする場合、アンワインダーは最初の*n*アンワインド コードをスキップする必要があります。

プロローグでは同様のロジックが逆に機能します。 プロローグ内のオフセット 0 からのアンワインドの場合、処理を実行する必要はありません。 プロローグ内のある命令からのアンワインドの場合、プロローグ アンワインド コードは逆順で格納されているため、アンワインド シーケンスは 1 つのアンワインド コードを末尾から開始する必要があります。 一般的なケースでは、プロローグ内の命令*n*からアンワインドする場合、アンワインドコードの実行は、コードのリストの最後から*n*アンワインドコードで開始する必要があります。
...
1 番目の場合では、プロローグのみを記述する必要があります。 これは、通常のプロローグを記述し、エピローグを示さない*Ret*値 3 を指定することで、コンパクトな .pdata 形式で行うことができます。 フル .xdata 形式では、インデックス 0 に通常どおりプロローグ アンワインド コードを指定して、エピローグ カウントに 0 を指定することでこのことを実現できます。

2 番目の場合は、通常の関数です。 フラグメント内にエピローグが 1 つしかなく、フラグメントの末尾にある場合は、コンパクトな .pdata レコードを使用できます。 その他の場合は、フル .xdata レコードを使用する必要があります。 エピローグ開始用に指定されたオフセットは、フラグメントの開始からの相対値であり、元の関数からの相対値ではない点に留意してください。

3 番目と 4 番目のケースは、プロローグを含まないことを除いて、それぞれ 1 番目と 2 番目のケースのバリアントです。 これらの状況においては、エピローグの開始前にコードがあることが想定され、そのコードは関数本体の一部と見なされます。通常、これはプロローグの効果を元に戻すことでアンワインドされます。 したがって、これらの場合は、擬似プロローグと共にエンコードする必要があります。擬似プロローグは本体内からアンワィンドする方法を記述しますが、フラグメントの開始で部分アンワインドを実行するかどうかを決定するときに 0 長として扱われます。 この擬似プロローグは、エピローグと同じアンワインド コードを使用して記述することもできます。これは、これらのコードが同等の演算を実行すると想定されるためです。

3 番目と 4 番目のケースでは、疑似プロローグの存在は、コンパクト .pdata レコードの*Flag*フィールドを 2 に設定するか、.xdata ヘッダーの*F*フラグを 1 に設定することによって指定されます。 両方の場合で、部分的なプロローグ アンワインドのチェックが無視され、すべての非エピローグ アンワインドは完全なものであると見なされます。
...
関数フラグメントのより複雑な特殊なケースは、レジスタの保存を必要としない単純なケースを最適化するために、関数の先頭から関数の後半に保存されたレジスタを保留する手法である*シュリンクラッピング*です。 これは、スタック領域を割り当てるが最小セットのレジスタを保存する外部の領域、および追加のレジスタの保存と復元を行う内部の領域として記述できます。
...
シュリンク ラップされた関数は、一般的には通常のプロローグ内に追加のレジスタ保存用領域を事前に割り当てて、次に `str` ではなく `stm` または `push` を使用してレジスタ保存を実行することが想定されています。 これにより、関数の元のプロローグ内のすべてのスタック ポインター操作が保持されます。
...
ここで重要なことは、各命令の境界で、スタックは該当領域のアンワインド コードと完全に整合しているということです。 この例の内部のプッシュの前でアンワインドが発生した場合、これは領域 A の一部と見なされ、領域 A のプロローグのみがアンワインドされます。 内部プッシュの後にアンワインドが発生した場合、それはプロローグを持たない領域 B の一部と見なされますが、内側のプッシュと領域 A からの元のプロローグの両方を記述するアンワインド コードがあります。
...
- Word 0
  ...
  - *関数開始 RVA* = 0x000535F8 (= 0x004535F8-0x00400000)

- Word 1

  - *フラグ*= 1(正規のプロローグとエピローグ形式を示す)

  - *関数の長さ*= 0x31 (= 0x62/2)

  - *Ret* = 1(16 ビットブランチリターンを示す)

  - *H* = 0(パラメータがホームに設定されていないことを示す)

  - *R*=0 および*Reg* = 1(r4-r5 のプッシュ/ポップを示す)

  - *L* = 0(LR の保存/復元がないことを示す)
...
- Word 0
  ...
  - *関数開始 RVA* = 0x000533AC (= 0x004533AC -0x0040000)
...
- Word 1
  ...
  - *Ret* = 0(ポップ {pc} が返ることを示す)

  - *H* = 0(パラメータがホームに設定されていないことを示す)

  - *R*=0 および*Reg* = 3(r4-r7 のプッシュ/ポップを示す)

  - *L* = 1、LR が保存/復元されたことを示します。
...
- Word 0
  ...
  - *関数開始 RVA* = 0x00053988 (= 0x00453988-0x00400000)
...
- Word 1
  ...
  - *Ret* = 0、ポップ {pc}スタイルのリターンを示します (この場合は ldr pc,[sp],#0x14リターン)

  - *H* = 1、パラメーターがホームされたことを示す
...
- Word 0
  ...
  - *関数開始 RVA* = 0x000592F4 (= 0x004592F4-0x00400000)

- Word 1

  - *フラグ*= 0(.xdata レコードが存在することを示す)(複数のエピローグが原因で必要)
...
- Word 0
  ...
  - *関数長 =* 0x0001A3 (= 0x000346/2)

  - *Vers* = 0(Xdata の最初のバージョンを示す)

  - *例外*データがないことを示す X = 0

  - *E* = 0(エピローグスコープのリストを示す)

  - *F* = 0、プロローグを含む全関数の説明を示す

  - *エピローグ数*= 0x04、4 つの合計エピローグ スコープを示します。

  - *コードワード*= 0x01(アンワインドコードの1つの32ビットワードを示す)
...
- Word 0
  ...
  - *関数開始 RVA* = 0x00085A20 (= 0x00485A20-0x0040000)
...
- Word 0
  ...
  - *関数開始 RVA* = 0x00088C24 (= 0x00488C24-0x00400000)
...
- Word 0
  ...
  - *エピローグ数*= 0x00、エピローグアンワインドコードがオフセット 0x00 から始まる

  - *コードワード*= 0x02(アンワインドコードの2つの32ビットワードを示す)
...
- Word 3 は例外ハンドラ = 0x0019A7ED (= 0x0059A7ED - 0x0040000) を指定します。
...
- Word 0
  ...
  - *関数開始 RVA* = 0x00088C72 (= 0x00488C72-0x00400000)
...
- Word 1
  ...
  - *R*=0 および*Reg* = 7(レジスタが保存/復元されなかったことを示す)
