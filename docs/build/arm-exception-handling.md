...
|1|15|*H*は、関数が整数パラメーターを「ホーム」しているかどうかを示す 1 ビット フラグは、関数の開始時にプッシュして (r0 r3) を登録し、戻る前にスタックの 16 バイトの割り当てを解除します。 (0 = レジスタを元に戻しません、1 = レジスタを元に戻します。)|
...
|80-BF|00-FF|||32|`pop   {r0-r12, lr}`<br /><br /> コード & 0x2000 と r0 r12 がポップされます。 対応するビットが Code & 0x1FFF で設定されている場合がある場合 LR がポップされます。|
|C0-CF||||16|`mov   sp,rX`<br /><br /> X は Code & 0x0F です。|
|D0-D7||||16|`pop   {r4-rX,lr}`<br /><br /> X は (Code & 0x03) + 4 と LR がポップされます場合 Code & 0x04。|
|D8-DF||||32|`pop   {r4-rX,lr}`<br /><br /> X は (Code & 0x03) + 8 と LR がポップされます場合 Code & 0x04。|
|E0-E7||||32|`vpop  {d8-dX}`<br /><br /> X は (Code & 0x07) + 8|
|E8-EB|00-FF|||32|`addw  sp,sp,#X`<br /><br /> X は (Code & 0x03FF) \* 4|
|EC-ED|00-FF|||16|`pop   {r0-r7,lr}`<br /><br /> コード & 0x0100 および r0 r7 がポップされます。 対応するビットが Code & 0x00FF で設定されている場合がある場合 LR がポップされます。|
