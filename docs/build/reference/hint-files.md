...
ms.locfileid: "65446311"
---
# <a name="hint-files"></a>ヒント ファイル

*ヒント ファイル*には C++ 参照データベース パーサーによってコードの領域がスキップされる原因となるマクロが含まれています。 Visual Studio を開くとC++プロジェクト、パーサーは、プロジェクト内の各ソース ファイル内のコードを分析し、すべての識別子に関する情報を含むデータベースを作成します。 それらの情報は**クラス ビュー** ブラウザーや**ナビゲーション バー**などのコード参照機能をサポートするために、IDE によって使用されます。
...
- ソース ファイルのルート ディレクトリからソース ファイル自体を含むディレクトリへのパス。 一般的な Visual Studio でC++プロジェクト ルート ディレクトリには、ソリューションまたはプロジェクト ファイルが含まれています。
...
図が Visual Studio での物理ディレクトリの一部を示しますC++プロジェクト。 ヒント ファイルは、`vcpackages`、`Debug`、`A1`、および `A2` のディレクトリにあります。
