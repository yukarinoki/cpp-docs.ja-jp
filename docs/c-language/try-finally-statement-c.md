...
**`__leave** keyword is valid within a `try-finally` statement block. The effect of **`__leave** の効果は、`try-finally` ブロックの末尾に移動することです。 終了ハンドラーはすぐに実行されます。 **`goto`** ステートメントを使用しても同じ結果が得られますが、 **`goto`** ステートメントではスタックがアンワインドされます。 **`__leave** ステートメントは、スタック アンワインドが関与しないため、より効率的です。
