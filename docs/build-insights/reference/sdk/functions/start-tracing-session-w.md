...
description: C++ Build Insights SDK の StartTracingSessionW 関数のリファレンス。
ms.date: 02/12/2020
helpviewer_keywords:
- C++ Build Insights
...
`StartTracingSessionW` 関数により、トレース セッションが開始されます。 この関数を呼び出す実行可能ファイルには、管理者権限が必要です。
...
*sessionName*\
開始するトレース セッションの名前。 [StopTracingSessionW](stop-tracing-session-w.md) または他のトレース停止関数を呼び出すときは、同じ名前を使用します。
