...
オプティマイザーでは、`expression` で表される条件は、このキーワードが指定された時点で true であり、`expression` が変更されるまで (変数への代入などにより) true のままであると想定します。 によってオプティマイザーに渡されるヒントを選択すると、 **`__assume`** 最適化が向上します。

ステートメントが **`__assume`** 矛盾 (常に false に評価される式) として記述されている場合は、常にとして扱われ `__assume(0)` ます。 コードが期待どおりに動作しない場合は、前述のとおり、定義した `expression` が有効かつ true であることを確認してください。 想定される `__assume(0)` の動作の詳細については、以降の解説を参照してください。

> [!WARNING]
> プログラムには、到達可能なパスに無効なステートメントを含めることはできません **`__assume`** 。 コンパイラが無効なステートメントに至る可能性がある場合は **`__assume`** 、プログラムが予期しない動作や危険につながる可能性があります。
...
コンパイラはに基づいてコードを生成 **`__assume`** するため、ステートメント内の式 **`__assume`** が実行時に false である場合、そのコードは正常に実行されない可能性があります。 実行時に式が常に true となることが確認できない場合は、`assert` 関数を使用するとコードを保護することができます。
