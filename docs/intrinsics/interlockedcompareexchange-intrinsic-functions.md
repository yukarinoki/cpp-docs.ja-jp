---
description: 詳細については、_InterlockedCompareExchange 組み込み関数」を参照してください。
title: 組み込み関数の _InterlockedCompareExchange
ms.date: 09/02/2019
f1_keywords:
...
ms.locfileid: "97168190"
---
# <a name="_interlockedcompareexchange-intrinsic-functions"></a>組み込み関数の _InterlockedCompareExchange
...
`_InterlockedCompareExchange` 値と値をアトミックに比較 `Destination` `Comparand` します。 `Destination` 値が `Comparand` 値と等しい場合、`Exchange` 値は `Destination` で指定されたアドレスに格納されます。 それ以外の場合は、操作は実行されません。
...
には `_InterlockedCompareExchange` 、関連するデータ型や、プロセッサ固有の取得または解放のセマンティクスが使用されているかどうかによって、いくつかのバリエーションがあります。

関数は `_InterlockedCompareExchange` 長整数値で動作しますが、は8ビット整数値で動作し、short 整数値に対して演算を実行 `_InterlockedCompareExchange8` `_InterlockedCompareExchange16` し、 `_InterlockedCompareExchange64` 64 ビット整数値で動作します。
