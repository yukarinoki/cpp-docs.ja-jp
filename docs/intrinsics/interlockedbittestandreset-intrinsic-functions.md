---
description: 詳細については、_interlockedbittestandreset 組み込み関数」を参照してください。
title: 組み込み関数の _interlockedbittestandreset
ms.date: 09/02/2019
f1_keywords:
...
ms.locfileid: "97168216"
---
# <a name="_interlockedbittestandreset-intrinsic-functions"></a>組み込み関数の _interlockedbittestandreset

**Microsoft 固有の仕様**

アドレスのビットをゼロに設定し、 `b` 元の値を返す命令を生成し `a` ます。
...
X86 および x64 プロセッサでは、これらの組み込みは、 `lock btr` 分割不可能な操作で指定されたビットを読み取り、設定する命令を使用します。
